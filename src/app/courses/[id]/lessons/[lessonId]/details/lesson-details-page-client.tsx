...
import { ResourceMetadataHero } from '@/app/content/components/resource-metadata-hero'
...
interface LessonDetailsPageClientProps {
  courseId: string
  lessonId: string
}
...
function LessonContent({ 
  ...
  const { count: viewCount } = useViews({
    ns: 'content',
    id: viewResourceId,
    enabled: socialReady && Boolean(viewResourceId)
  })

  const interactionData = useCommentThreads(resourceNote?.id, {
    enabled: socialReady && Boolean(resourceNote?.id) && resourcePurchased,
    realtime: true,
...
export default function LessonDetailsPageClient({
  courseId,
  lessonId
}: LessonDetailsPageClientProps) {
  return (
    <MainLayout>
      <Section spacing="lg">
        <div className="space-y-6">
