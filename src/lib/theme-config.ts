/**
 * Complete theme packages for shadcn with integrated fonts, styles, and radius
 * Each theme is a complete package - no manual selection needed
 * Based on shadcn/ui theming system with optional runtime Google Fonts loading
 */
import { isRemoteFontLoadingEnabled } from "./font-loading-policy"

export type ThemeName = 
  | "amber-minimal"
  | "amethyst"
  | "amethyst-haze"
  | "astral"
  | "blaze"
  | "blue"
  | "bold-tech"
  | "bubblegum"
  | "caffeine"
  | "calypso"
  | "candyland"
  | "canvas"
  | "catppuccin"
  | "citrus"
  | "claude"
  | "claymorphism"
  | "clean-slate"
  | "cosmic-night"
  | "cyberpunk"
  | "darkmatter"
  | "default"
  | "doom64"
  | "elegant-luxury"
  | "emerald"
  | "forest"
  | "graphite"
  | "gray"
  | "green"
  | "miami"
  | "midnight-bloom"
  | "mocha-mousse"
  | "modern-minimal"
  | "mono"
  | "nature"
  | "neo-brutalism"
  | "neutral"
  | "northern-lights"
  | "notebook"
  | "ocean-breeze"
  | "orange"
  | "perpetuity"
  // LOCAL CUSTOMIZATION NOTE:
  // `plebdevs` is a deployment-specific theme for plebdevs.com.
  // When syncing from upstream pleb.school, keep this value and corresponding
  // theme object below, or resolve merge conflicts in favor of retaining it.
  | "plebdevs"
  | "quick-pink"
  | "razzmatazz"
  | "red"
  | "retro-arcade"
  | "rose"
  | "santa-fe"
  | "sky"
  | "slate"
  | "soft-pop"
  | "solar"
  | "solar-dusk"
  | "spooky"
  | "spring-bouquet"
  | "starry-night"
  | "stone"
  | "sunset-horizon"
  | "supabase"
  | "tangerine"
  | "twitter"
  | "typewriter"
  | "underground"
  | "vercel"
  | "vintage-paper"
  | "violet"
  | "violet-bloom"
  | "xanadu"
  | "yellow"
  | "zinc"

export interface CompleteTheme {
  name: string
  value: ThemeName
  description: string
  fontFamily: string
  fontWeight: string
  googleFontUrl?: string
  borderRadius: string
  style: "default" | "new-york"
  lightColors: Record<string, string>
  darkColors: Record<string, string>
}

export const defaultThemeName: ThemeName = "default"

// Available fonts for manual override
export interface FontConfig {
  name: string
  value: string
  fontFamily: string
  fontWeight: string
  googleFontUrl?: string
}

export const availableFonts: FontConfig[] = [
  {
    name: "System Default",
    value: "system",
    fontFamily: "ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif",
    fontWeight: "400"
  },
  {
    name: "Inter",
    value: "inter",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap"
  },
  {
    name: "Roboto",
    value: "roboto",
    fontFamily: "Roboto, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap"
  },
  {
    name: "Poppins",
    value: "poppins",
    fontFamily: "Poppins, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&display=swap"
  },
  {
    name: "Playfair Display",
    value: "playfair",
    fontFamily: "Playfair Display, ui-serif, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;500;600;700&display=swap"
  },
  {
    name: "Georgia",
    value: "georgia",
    fontFamily: "Georgia, ui-serif, serif",
    fontWeight: "400"
  },
  {
    name: "Crimson Text",
    value: "crimson",
    fontFamily: "Crimson Text, ui-serif, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Crimson+Text:wght@400;600;700&display=swap"
  },
  {
    name: "JetBrains Mono",
    value: "jetbrains",
    fontFamily: "JetBrains Mono, ui-monospace, monospace",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;600;700&display=swap"
  },
  {
    name: "Fira Code",
    value: "fira",
    fontFamily: "Fira Code, ui-monospace, monospace",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Fira+Code:wght@400;500;600&display=swap"
  },
  {
    name: "Source Sans Pro",
    value: "source-sans",
    fontFamily: "Source Sans Pro, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Source+Sans+Pro:wght@400;600;700&display=swap"
  },
  {
    name: "IBM Plex Sans",
    value: "ibm-plex",
    fontFamily: "IBM Plex Sans, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@400;500;600;700&display=swap"
  },
  {
    name: "Nunito",
    value: "nunito",
    fontFamily: "Nunito, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Nunito:wght@400;500;600;700&display=swap"
  },
  {
    name: "Comfortaa",
    value: "comfortaa",
    fontFamily: "Comfortaa, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Comfortaa:wght@400;500;600;700&display=swap"
  },
  {
    name: "Orbitron",
    value: "orbitron",
    fontFamily: "Orbitron, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Orbitron:wght@400;500;600;700;900&display=swap"
  },
  {
    name: "System Mono",
    value: "system-mono",
    fontFamily: "ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace",
    fontWeight: "400"
  },
  {
    name: "Space Mono",
    value: "space-mono",
    fontFamily: "Space Mono, ui-monospace, SFMono-Regular, 'SF Mono', Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Space+Mono:wght@400;700&display=swap"
  },
  {
    name: "Lora",
    value: "lora",
    fontFamily: "Lora, ui-serif, Georgia, Cambria, 'Times New Roman', Times, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Lora:wght@400;500;600;700&display=swap"
  },
  {
    name: "Merriweather",
    value: "merriweather",
    fontFamily: "Merriweather, ui-serif, Georgia, Cambria, 'Times New Roman', Times, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Merriweather:wght@400;500;600;700&display=swap"
  },
  {
    name: "Space Grotesk",
    value: "space-grotesk",
    fontFamily: "Space Grotesk, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&display=swap"
  },
  {
    name: "Open Sans",
    value: "open-sans",
    fontFamily: "Open Sans, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;500;600;700&display=swap"
  },
  {
    name: "Press Start 2P",
    value: "press-start",
    fontFamily: "Press Start 2P, ui-monospace, SFMono-Regular, 'SF Mono', Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Press+Start+2P&display=swap"
  },
  {
    name: "Quicksand",
    value: "quicksand",
    fontFamily: "Quicksand, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Quicksand:wght@400;500;600;700&display=swap"
  },
  {
    name: "Raleway",
    value: "raleway",
    fontFamily: "Raleway, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Raleway:wght@400;500;600;700&display=swap"
  },
  {
    name: "Libre Baskerville",
    value: "libre-baskerville",
    fontFamily: "Libre Baskerville, ui-serif, Georgia, Cambria, 'Times New Roman', Times, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Libre+Baskerville:wght@400;500;600;700&display=swap"
  }
]

// Complete theme packages with everything integrated
export const completeThemes: CompleteTheme[] = [
  {
    name: "Amber Minimal",
    value: "amber-minimal",
    description: "Minimal amber theme with Inter",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap",
    borderRadius: "0.375rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1.0000 0 0)",
      "--foreground": "oklch(0.2686 0 0)",
      "--card": "oklch(1.0000 0 0)",
      "--card-foreground": "oklch(0.2686 0 0)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.2686 0 0)",
      "--primary": "oklch(0.7686 0.1647 70.0804)",
      "--primary-foreground": "oklch(0 0 0)",
      "--secondary": "oklch(0.9670 0.0029 264.5419)",
      "--secondary-foreground": "oklch(0.4461 0.0263 256.8018)",
      "--muted": "oklch(0.9846 0.0017 247.8389)",
      "--muted-foreground": "oklch(0.5510 0.0234 264.3637)",
      "--accent": "oklch(0.9869 0.0214 95.2774)",
      "--accent-foreground": "oklch(0.4732 0.1247 46.2007)",
      "--destructive": "oklch(0.6368 0.2078 25.3313)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.9276 0.0058 264.5313)",
      "--input": "oklch(0.9276 0.0058 264.5313)",
      "--ring": "oklch(0.7686 0.1647 70.0804)",
      "--chart-1": "oklch(0.7686 0.1647 70.0804)",
      "--chart-2": "oklch(0.6658 0.1574 58.3183)",
      "--chart-3": "oklch(0.5553 0.1455 48.9975)",
      "--chart-4": "oklch(0.4732 0.1247 46.2007)",
      "--chart-5": "oklch(0.4137 0.1054 45.9038)",
      "--sidebar": "oklch(0.9846 0.0017 247.8389)",
      "--sidebar-foreground": "oklch(0.2686 0 0)",
      "--sidebar-primary": "oklch(0.7686 0.1647 70.0804)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.9869 0.0214 95.2774)",
      "--sidebar-accent-foreground": "oklch(0.4732 0.1247 46.2007)",
      "--sidebar-border": "oklch(0.9276 0.0058 264.5313)",
      "--sidebar-ring": "oklch(0.7686 0.1647 70.0804)",
    },
    darkColors: {
      "--background": "oklch(0.2046 0 0)",
      "--foreground": "oklch(0.9219 0 0)",
      "--card": "oklch(0.2686 0 0)",
      "--card-foreground": "oklch(0.9219 0 0)",
      "--popover": "oklch(0.2686 0 0)",
      "--popover-foreground": "oklch(0.9219 0 0)",
      "--primary": "oklch(0.7686 0.1647 70.0804)",
      "--primary-foreground": "oklch(0 0 0)",
      "--secondary": "oklch(0.2686 0 0)",
      "--secondary-foreground": "oklch(0.9219 0 0)",
      "--muted": "oklch(0.3200 0 0)",
      "--muted-foreground": "oklch(0.7155 0 0)",
      "--accent": "oklch(0.4732 0.1247 46.2007)",
      "--accent-foreground": "oklch(0.9243 0.1151 95.7459)",
      "--destructive": "oklch(0.6368 0.2078 25.3313)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.3715 0 0)",
      "--input": "oklch(0.3715 0 0)",
      "--ring": "oklch(0.7686 0.1647 70.0804)",
      "--chart-1": "oklch(0.8369 0.1644 84.4286)",
      "--chart-2": "oklch(0.6658 0.1574 58.3183)",
      "--chart-3": "oklch(0.4732 0.1247 46.2007)",
      "--chart-4": "oklch(0.5553 0.1455 48.9975)",
      "--chart-5": "oklch(0.4732 0.1247 46.2007)",
      "--sidebar": "oklch(0.1684 0 0)",
      "--sidebar-foreground": "oklch(0.9219 0 0)",
      "--sidebar-primary": "oklch(0.7686 0.1647 70.0804)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.4732 0.1247 46.2007)",
      "--sidebar-accent-foreground": "oklch(0.9243 0.1151 95.7459)",
      "--sidebar-border": "oklch(0.3715 0 0)",
      "--sidebar-ring": "oklch(0.7686 0.1647 70.0804)",
    }
  },
  {
    name: "Amethyst",
    value: "amethyst",
    description: "Amethyst theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "1rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9868 0.0029 259.84)",
      "--foreground": "oklch(0.278 0.0277 258.85)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.278 0.0277 258.85)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.278 0.0277 258.85)",
      "--primary": "oklch(0.5709 0.1808 306.89)",
      "--secondary": "oklch(0.811 0.0589 201.14)",
      "--muted": "oklch(0.9692 0.0058 262.54)",
      "--muted-foreground": "oklch(0.446 0.0335 258.8)",
      "--accent": "oklch(0.9692 0.0058 262.54)",
      "--accent-foreground": "oklch(0.278 0.0277 258.85)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9253 0.0117 262.53)",
      "--input": "oklch(0.8692 0.0175 260.34)",
      "--ring": "oklch(0.5709 0.1808 306.89)",
      "--chart-1": "oklch(0.5709 0.1808 306.89)",
      "--chart-2": "oklch(0.811 0.0589 201.14)",
      "--chart-3": "oklch(0.8281 0.078 309.73)",
      "--chart-4": "oklch(0.8611 0.0504 198.03)",
      "--chart-5": "oklch(0.947 0.0212 309.77)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.278 0.0277 258.85)",
      "--sidebar-primary": "oklch(0.5709 0.1808 306.89)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9868 0.0029 259.84)",
      "--sidebar-accent-foreground": "oklch(0.278 0.0277 258.85)",
      "--sidebar-border": "oklch(0.9253 0.0117 262.53)",
      "--sidebar-ring": "oklch(0.5709 0.1808 306.89)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.278 0.0277 258.85)"
    },
    darkColors: {
      "--background": "oklch(0.13 0.0219 261.69)",
      "--foreground": "oklch(0.9253 0.0117 262.53)",
      "--card": "oklch(0.21 0.024 262.66)",
      "--card-foreground": "oklch(0.9253 0.0117 262.53)",
      "--popover": "oklch(0.21 0.024 262.66)",
      "--popover-foreground": "oklch(0.9253 0.0117 262.53)",
      "--primary": "oklch(0.5709 0.1808 306.89)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.811 0.0589 201.14)",
      "--secondary-foreground": "oklch(0 0 0)",
      "--muted": "oklch(0.278 0.0277 258.85)",
      "--muted-foreground": "oklch(0.8692 0.0175 260.34)",
      "--accent": "oklch(0.278 0.0277 258.85)",
      "--accent-foreground": "oklch(0.9253 0.0117 262.53)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.278 0.0277 258.85)",
      "--input": "oklch(0.373 0.0306 259.73)",
      "--ring": "oklch(0.5709 0.1808 306.89)",
      "--chart-1": "oklch(0.5709 0.1808 306.89)",
      "--chart-2": "oklch(0.811 0.0589 201.14)",
      "--chart-3": "oklch(0.8281 0.078 309.73)",
      "--chart-4": "oklch(0.8611 0.0504 198.03)",
      "--chart-5": "oklch(0.947 0.0212 309.77)",
      "--sidebar": "oklch(0.21 0.024 262.66)",
      "--sidebar-foreground": "oklch(0.9253 0.0117 262.53)",
      "--sidebar-primary": "oklch(0.5709 0.1808 306.89)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.278 0.0277 258.85)",
      "--sidebar-accent-foreground": "oklch(0.9253 0.0117 262.53)",
      "--sidebar-border": "oklch(0.278 0.0277 258.85)",
      "--sidebar-ring": "oklch(0.5709 0.1808 306.89)"
    }
  },
  {
    name: "Amethyst Haze",
    value: "amethyst-haze",
    description: "Mystical amethyst theme with Geist",
    fontFamily: "Geist, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Geist:wght@400;500;600;700&display=swap",
    borderRadius: "0.5rem",
    style: "new-york",
    lightColors: {
      "--background": "oklch(0.9777 0.0041 301.4256)",
      "--foreground": "oklch(0.3651 0.0325 287.0807)",
      "--card": "oklch(1.0000 0 0)",
      "--card-foreground": "oklch(0.3651 0.0325 287.0807)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.3651 0.0325 287.0807)",
      "--primary": "oklch(0.6104 0.0767 299.7335)",
      "--primary-foreground": "oklch(0.9777 0.0041 301.4256)",
      "--secondary": "oklch(0.8957 0.0265 300.2416)",
      "--secondary-foreground": "oklch(0.3651 0.0325 287.0807)",
      "--muted": "oklch(0.8906 0.0139 299.7754)",
      "--muted-foreground": "oklch(0.5288 0.0375 290.7895)",
      "--accent": "oklch(0.7889 0.0802 359.9375)",
      "--accent-foreground": "oklch(0.3394 0.0441 1.7583)",
      "--destructive": "oklch(0.6332 0.1578 22.6734)",
      "--destructive-foreground": "oklch(0.9777 0.0041 301.4256)",
      "--border": "oklch(0.8447 0.0226 300.1421)",
      "--input": "oklch(0.9329 0.0124 301.2783)",
      "--ring": "oklch(0.6104 0.0767 299.7335)",
      "--chart-1": "oklch(0.6104 0.0767 299.7335)",
      "--chart-2": "oklch(0.7889 0.0802 359.9375)",
      "--chart-3": "oklch(0.7321 0.0749 169.8670)",
      "--chart-4": "oklch(0.8540 0.0882 76.8292)",
      "--chart-5": "oklch(0.7857 0.0645 258.0839)",
      "--sidebar": "oklch(0.9554 0.0082 301.3541)",
      "--sidebar-foreground": "oklch(0.3651 0.0325 287.0807)",
      "--sidebar-primary": "oklch(0.6104 0.0767 299.7335)",
      "--sidebar-primary-foreground": "oklch(0.9777 0.0041 301.4256)",
      "--sidebar-accent": "oklch(0.7889 0.0802 359.9375)",
      "--sidebar-accent-foreground": "oklch(0.3394 0.0441 1.7583)",
      "--sidebar-border": "oklch(0.8719 0.0198 302.1690)",
      "--sidebar-ring": "oklch(0.6104 0.0767 299.7335)",
    },
    darkColors: {
      "--background": "oklch(0.2166 0.0215 292.8474)",
      "--foreground": "oklch(0.9053 0.0245 293.5570)",
      "--card": "oklch(0.2544 0.0301 292.7315)",
      "--card-foreground": "oklch(0.9053 0.0245 293.5570)",
      "--popover": "oklch(0.2544 0.0301 292.7315)",
      "--popover-foreground": "oklch(0.9053 0.0245 293.5570)",
      "--primary": "oklch(0.7058 0.0777 302.0489)",
      "--primary-foreground": "oklch(0.2166 0.0215 292.8474)",
      "--secondary": "oklch(0.4604 0.0472 295.5578)",
      "--secondary-foreground": "oklch(0.9053 0.0245 293.5570)",
      "--muted": "oklch(0.2560 0.0320 294.8380)",
      "--muted-foreground": "oklch(0.6974 0.0282 300.0614)",
      "--accent": "oklch(0.3181 0.0321 308.6149)",
      "--accent-foreground": "oklch(0.8391 0.0692 2.6681)",
      "--destructive": "oklch(0.6875 0.1420 21.4566)",
      "--destructive-foreground": "oklch(0.2166 0.0215 292.8474)",
      "--border": "oklch(0.3063 0.0359 293.3367)",
      "--input": "oklch(0.2847 0.0346 291.2726)",
      "--ring": "oklch(0.7058 0.0777 302.0489)",
      "--chart-1": "oklch(0.7058 0.0777 302.0489)",
      "--chart-2": "oklch(0.8391 0.0692 2.6681)",
      "--chart-3": "oklch(0.7321 0.0749 169.8670)",
      "--chart-4": "oklch(0.8540 0.0882 76.8292)",
      "--chart-5": "oklch(0.7857 0.0645 258.0839)",
      "--sidebar": "oklch(0.1985 0.0200 293.6639)",
      "--sidebar-foreground": "oklch(0.9053 0.0245 293.5570)",
      "--sidebar-primary": "oklch(0.7058 0.0777 302.0489)",
      "--sidebar-primary-foreground": "oklch(0.2166 0.0215 292.8474)",
      "--sidebar-accent": "oklch(0.3181 0.0321 308.6149)",
      "--sidebar-accent-foreground": "oklch(0.8391 0.0692 2.6681)",
      "--sidebar-border": "oklch(0.2847 0.0346 291.2726)",
      "--sidebar-ring": "oklch(0.7058 0.0777 302.0489)",
    }
  },
  {
    name: "Astral",
    value: "astral",
    description: "Astral theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.2561 0.018 266.26)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2561 0.018 266.26)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2561 0.018 266.26)",
      "--primary": "oklch(0.5512 0.1177 218.91)",
      "--secondary": "oklch(0.5079 0.0441 228.53)",
      "--muted": "oklch(0.9883 0.0019 258.27)",
      "--muted-foreground": "oklch(0.4465 0.0218 264.54)",
      "--accent": "oklch(0.9883 0.0019 258.27)",
      "--accent-foreground": "oklch(0.2561 0.018 266.26)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9321 0.0076 259.94)",
      "--input": "oklch(0.8715 0.0114 261.25)",
      "--ring": "oklch(0.5512 0.1177 218.91)",
      "--chart-1": "oklch(0.5512 0.1177 218.91)",
      "--chart-2": "oklch(0.5079 0.0441 228.53)",
      "--chart-3": "oklch(0.8632 0.1432 207.1)",
      "--chart-4": "oklch(0.8501 0.0455 218.77)",
      "--chart-5": "oklch(0.9536 0.0508 205.46)",
      "--sidebar": "oklch(0.9717 0.0038 259.34)",
      "--sidebar-foreground": "oklch(0.2561 0.018 266.26)",
      "--sidebar-primary": "oklch(0.5512 0.1177 218.91)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9321 0.0076 259.94)",
      "--sidebar-accent-foreground": "oklch(0.2561 0.018 266.26)",
      "--sidebar-border": "oklch(0.9321 0.0076 259.94)",
      "--sidebar-ring": "oklch(0.5512 0.1177 218.91)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2561 0.018 266.26)"
    },
    darkColors: {
      "--background": "oklch(0.1291 0.0142 268.92)",
      "--foreground": "oklch(0.9321 0.0076 259.94)",
      "--card": "oklch(0.1291 0.0142 268.92)",
      "--card-foreground": "oklch(0.9321 0.0076 259.94)",
      "--popover": "oklch(0.1291 0.0142 268.92)",
      "--popover-foreground": "oklch(0.9321 0.0076 259.94)",
      "--primary": "oklch(0.5512 0.1177 218.91)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.5079 0.0441 228.53)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.2081 0.0156 268.02)",
      "--muted-foreground": "oklch(0.7056 0.0171 264.11)",
      "--accent": "oklch(0.2081 0.0156 268.02)",
      "--accent-foreground": "oklch(0.9321 0.0076 259.94)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2561 0.018 266.26)",
      "--input": "oklch(0.3722 0.0199 264.57)",
      "--ring": "oklch(0.5512 0.1177 218.91)",
      "--chart-1": "oklch(0.5512 0.1177 218.91)",
      "--chart-2": "oklch(0.5079 0.0441 228.53)",
      "--chart-3": "oklch(0.8632 0.1432 207.1)",
      "--chart-4": "oklch(0.8501 0.0455 218.77)",
      "--chart-5": "oklch(0.9536 0.0508 205.46)",
      "--sidebar": "oklch(0.2081 0.0156 268.02)",
      "--sidebar-foreground": "oklch(0.9321 0.0076 259.94)",
      "--sidebar-primary": "oklch(0.5512 0.1177 218.91)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.2561 0.018 266.26)",
      "--sidebar-accent-foreground": "oklch(0.9321 0.0076 259.94)",
      "--sidebar-border": "oklch(0.2561 0.018 266.26)",
      "--sidebar-ring": "oklch(0.5512 0.1177 218.91)"
    }
  },
  {
    name: "Blaze",
    value: "blaze",
    description: "Blaze theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.3rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9848 0.0019 286.64)",
      "--foreground": "oklch(0.274 0.0116 285.83)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.274 0.0116 285.83)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.274 0.0116 285.83)",
      "--primary": "oklch(0.5827 0.2187 36.98)",
      "--secondary": "oklch(0.9199 0.0076 286.14)",
      "--muted": "oklch(0.9668 0.0038 286.2)",
      "--muted-foreground": "oklch(0.442 0.0148 285.58)",
      "--accent": "oklch(0.9668 0.0038 286.2)",
      "--accent-foreground": "oklch(0.274 0.0116 285.83)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9199 0.0076 286.14)",
      "--input": "oklch(0.8709 0.0114 286.09)",
      "--ring": "oklch(0.5827 0.2187 36.98)",
      "--chart-1": "oklch(0.5827 0.2187 36.98)",
      "--chart-2": "oklch(0.8926 0.076 41.45)",
      "--chart-3": "oklch(0.7319 0.2017 39.84)",
      "--chart-4": "oklch(0.8245 0.1307 40.37)",
      "--chart-5": "oklch(0.9441 0.0382 42.16)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.274 0.0116 285.83)",
      "--sidebar-primary": "oklch(0.5827 0.2187 36.98)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9848 0.0019 286.64)",
      "--sidebar-accent-foreground": "oklch(0.274 0.0116 285.83)",
      "--sidebar-border": "oklch(0.9199 0.0076 286.14)",
      "--sidebar-ring": "oklch(0.5827 0.2187 36.98)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.274 0.0116 285.83)"
    },
    darkColors: {
      "--background": "oklch(0.1409 0.0059 285.64)",
      "--foreground": "oklch(0.9199 0.0076 286.14)",
      "--card": "oklch(0.2108 0.0078 285.71)",
      "--card-foreground": "oklch(0.9199 0.0076 286.14)",
      "--popover": "oklch(0.2108 0.0078 285.71)",
      "--popover-foreground": "oklch(0.9199 0.0076 286.14)",
      "--primary": "oklch(0.5827 0.2187 36.98)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.37 0.0136 285.61)",
      "--secondary-foreground": "oklch(0.9848 0.0019 286.64)",
      "--muted": "oklch(0.274 0.0116 285.83)",
      "--muted-foreground": "oklch(0.8709 0.0114 286.09)",
      "--accent": "oklch(0.274 0.0116 285.83)",
      "--accent-foreground": "oklch(0.9199 0.0076 286.14)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.274 0.0116 285.83)",
      "--input": "oklch(0.37 0.0136 285.61)",
      "--ring": "oklch(0.5827 0.2187 36.98)",
      "--chart-1": "oklch(0.5827 0.2187 36.98)",
      "--chart-2": "oklch(0.8926 0.076 41.45)",
      "--chart-3": "oklch(0.7319 0.2017 39.84)",
      "--chart-4": "oklch(0.8245 0.1307 40.37)",
      "--chart-5": "oklch(0.9441 0.0382 42.16)",
      "--sidebar": "oklch(0.2108 0.0078 285.71)",
      "--sidebar-foreground": "oklch(0.9199 0.0076 286.14)",
      "--sidebar-primary": "oklch(0.5827 0.2187 36.98)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.274 0.0116 285.83)",
      "--sidebar-accent-foreground": "oklch(0.9199 0.0076 286.14)",
      "--sidebar-border": "oklch(0.274 0.0116 285.83)",
      "--sidebar-ring": "oklch(0.5827 0.2187 36.98)"
    }
  },
  {
    name: "Blue",
    value: "blue",
    description: "Ocean blue theme with Inter font",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap",
    borderRadius: "0.65rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.141 0.005 285.823)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.141 0.005 285.823)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.141 0.005 285.823)",
      "--primary": "oklch(0.623 0.214 259.815)",
      "--primary-foreground": "oklch(0.97 0.014 254.604)",
      "--secondary": "oklch(0.967 0.001 286.375)",
      "--secondary-foreground": "oklch(0.21 0.006 285.885)",
      "--muted": "oklch(0.967 0.001 286.375)",
      "--muted-foreground": "oklch(0.552 0.016 285.938)",
      "--accent": "oklch(0.967 0.001 286.375)",
      "--accent-foreground": "oklch(0.21 0.006 285.885)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.92 0.004 286.32)",
      "--input": "oklch(0.92 0.004 286.32)",
      "--ring": "oklch(0.623 0.214 259.815)",
      "--chart-1": "oklch(0.646 0.222 41.116)",
      "--chart-2": "oklch(0.6 0.118 184.704)",
      "--chart-3": "oklch(0.398 0.07 227.392)",
      "--chart-4": "oklch(0.828 0.189 84.429)",
      "--chart-5": "oklch(0.769 0.188 70.08)",
      "--sidebar": "oklch(0.985 0 0)",
      "--sidebar-foreground": "oklch(0.141 0.005 285.823)",
      "--sidebar-primary": "oklch(0.623 0.214 259.815)",
      "--sidebar-primary-foreground": "oklch(0.97 0.014 254.604)",
      "--sidebar-accent": "oklch(0.967 0.001 286.375)",
      "--sidebar-accent-foreground": "oklch(0.21 0.006 285.885)",
      "--sidebar-border": "oklch(0.92 0.004 286.32)",
      "--sidebar-ring": "oklch(0.623 0.214 259.815)",
    },
    darkColors: {
      "--background": "oklch(0.141 0.005 285.823)",
      "--foreground": "oklch(0.985 0 0)",
      "--card": "oklch(0.21 0.006 285.885)",
      "--card-foreground": "oklch(0.985 0 0)",
      "--popover": "oklch(0.21 0.006 285.885)",
      "--popover-foreground": "oklch(0.985 0 0)",
      "--primary": "oklch(0.546 0.245 262.881)",
      "--primary-foreground": "oklch(0.985 0 0)",
      "--secondary": "oklch(0.274 0.006 286.033)",
      "--secondary-foreground": "oklch(0.985 0 0)",
      "--muted": "oklch(0.274 0.006 286.033)",
      "--muted-foreground": "oklch(0.705 0.015 286.067)",
      "--accent": "oklch(0.274 0.006 286.033)",
      "--accent-foreground": "oklch(0.985 0 0)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(1 0 0 / 10%)",
      "--input": "oklch(1 0 0 / 15%)",
      "--ring": "oklch(0.488 0.243 264.376)",
      "--chart-1": "oklch(0.488 0.243 264.376)",
      "--chart-2": "oklch(0.696 0.17 162.48)",
      "--chart-3": "oklch(0.769 0.188 70.08)",
      "--chart-4": "oklch(0.627 0.265 303.9)",
      "--chart-5": "oklch(0.645 0.246 16.439)",
      "--sidebar": "oklch(0.21 0.006 285.885)",
      "--sidebar-foreground": "oklch(0.985 0 0)",
      "--sidebar-primary": "oklch(0.546 0.245 262.881)",
      "--sidebar-primary-foreground": "oklch(0.985 0 0)",
      "--sidebar-accent": "oklch(0.274 0.006 286.033)",
      "--sidebar-accent-foreground": "oklch(0.985 0 0)",
      "--sidebar-border": "oklch(1 0 0 / 10%)",
      "--sidebar-ring": "oklch(0.488 0.243 264.376)",
    }
  },
  {
    name: "Bold Tech",
    value: "bold-tech",
    description: "Bold technology theme with strong contrasts",
    fontFamily: "Space Mono, ui-monospace, SFMono-Regular, 'SF Mono', Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Space+Mono:wght@400;700&display=swap",
    borderRadius: "0.25rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.09 0 0)",
      "--card": "oklch(0.99 0 0)",
      "--card-foreground": "oklch(0.09 0 0)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.09 0 0)",
      "--primary": "oklch(0.09 0 0)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.95 0 0)",
      "--secondary-foreground": "oklch(0.09 0 0)",
      "--muted": "oklch(0.95 0 0)",
      "--muted-foreground": "oklch(0.45 0 0)",
      "--accent": "oklch(0.95 0 0)",
      "--accent-foreground": "oklch(0.09 0 0)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.89 0 0)",
      "--input": "oklch(0.89 0 0)",
      "--ring": "oklch(0.09 0 0)",
      "--chart-1": "oklch(0.5 0.3 0)",
      "--chart-2": "oklch(0.5 0.3 90)",
      "--chart-3": "oklch(0.5 0.3 180)",
      "--chart-4": "oklch(0.5 0.3 270)",
      "--chart-5": "oklch(0.5 0.3 45)",
      "--sidebar": "oklch(0.98 0 0)",
      "--sidebar-foreground": "oklch(0.09 0 0)",
      "--sidebar-primary": "oklch(0.09 0 0)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.95 0 0)",
      "--sidebar-accent-foreground": "oklch(0.09 0 0)",
      "--sidebar-border": "oklch(0.89 0 0)",
      "--sidebar-ring": "oklch(0.09 0 0)"
    },
    darkColors: {
      "--background": "oklch(0.09 0 0)",
      "--foreground": "oklch(1 0 0)",
      "--card": "oklch(0.11 0 0)",
      "--card-foreground": "oklch(1 0 0)",
      "--popover": "oklch(0.09 0 0)",
      "--popover-foreground": "oklch(1 0 0)",
      "--primary": "oklch(1 0 0)",
      "--primary-foreground": "oklch(0.09 0 0)",
      "--secondary": "oklch(0.18 0 0)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.18 0 0)",
      "--muted-foreground": "oklch(0.65 0 0)",
      "--accent": "oklch(0.18 0 0)",
      "--accent-foreground": "oklch(1 0 0)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.27 0 0)",
      "--input": "oklch(0.27 0 0)",
      "--ring": "oklch(1 0 0)",
      "--chart-1": "oklch(0.7 0.3 0)",
      "--chart-2": "oklch(0.7 0.3 90)",
      "--chart-3": "oklch(0.7 0.3 180)",
      "--chart-4": "oklch(0.7 0.3 270)",
      "--chart-5": "oklch(0.7 0.3 45)",
      "--sidebar": "oklch(0.11 0 0)",
      "--sidebar-foreground": "oklch(1 0 0)",
      "--sidebar-primary": "oklch(1 0 0)",
      "--sidebar-primary-foreground": "oklch(0.09 0 0)",
      "--sidebar-accent": "oklch(0.18 0 0)",
      "--sidebar-accent-foreground": "oklch(1 0 0)",
      "--sidebar-border": "oklch(0.27 0 0)",
      "--sidebar-ring": "oklch(1 0 0)"
    }
  },
  {
    name: "Bubblegum",
    value: "bubblegum",
    description: "Playful bubblegum pink theme with Poppins font",
    fontFamily: "Poppins, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&display=swap",
    borderRadius: "1rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9399 0.0203 345.6985)",
      "--foreground": "oklch(0.4712 0 0)",
      "--card": "oklch(0.9498 0.0500 86.8891)",
      "--card-foreground": "oklch(0.4712 0 0)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.4712 0 0)",
      "--primary": "oklch(0.6209 0.1801 348.1385)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.8095 0.0694 198.1863)",
      "--secondary-foreground": "oklch(0.3211 0 0)",
      "--muted": "oklch(0.8800 0.0504 212.0952)",
      "--muted-foreground": "oklch(0.5795 0 0)",
      "--accent": "oklch(0.9195 0.0801 87.6670)",
      "--accent-foreground": "oklch(0.3211 0 0)",
      "--destructive": "oklch(0.7091 0.1697 21.9551)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.6209 0.1801 348.1385)",
      "--input": "oklch(0.9189 0 0)",
      "--ring": "oklch(0.7002 0.1597 350.7532)",
      "--chart-1": "oklch(0.7002 0.1597 350.7532)",
      "--chart-2": "oklch(0.8189 0.0799 212.0892)",
      "--chart-3": "oklch(0.9195 0.0801 87.6670)",
      "--chart-4": "oklch(0.7998 0.1110 348.1791)",
      "--chart-5": "oklch(0.6197 0.1899 353.9091)",
      "--sidebar": "oklch(0.9140 0.0424 343.0913)",
      "--sidebar-foreground": "oklch(0.3211 0 0)",
      "--sidebar-primary": "oklch(0.6559 0.2118 354.3084)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.8228 0.1095 346.0184)",
      "--sidebar-accent-foreground": "oklch(0.3211 0 0)",
      "--sidebar-border": "oklch(0.9464 0.0327 307.1745)",
      "--sidebar-ring": "oklch(0.6559 0.2118 354.3084)"
    },
    darkColors: {
      "--background": "oklch(0.2497 0.0305 234.1628)",
      "--foreground": "oklch(0.9306 0.0197 349.0785)",
      "--card": "oklch(0.2902 0.0299 233.5352)",
      "--card-foreground": "oklch(0.9306 0.0197 349.0785)",
      "--popover": "oklch(0.2902 0.0299 233.5352)",
      "--popover-foreground": "oklch(0.9306 0.0197 349.0785)",
      "--primary": "oklch(0.9195 0.0801 87.6670)",
      "--primary-foreground": "oklch(0.2497 0.0305 234.1628)",
      "--secondary": "oklch(0.7794 0.0803 4.1330)",
      "--secondary-foreground": "oklch(0.2497 0.0305 234.1628)",
      "--muted": "oklch(0.2713 0.0086 255.5780)",
      "--muted-foreground": "oklch(0.7794 0.0803 4.1330)",
      "--accent": "oklch(0.6699 0.0988 356.9762)",
      "--accent-foreground": "oklch(0.9306 0.0197 349.0785)",
      "--destructive": "oklch(0.6702 0.1806 350.3599)",
      "--destructive-foreground": "oklch(0.2497 0.0305 234.1628)",
      "--border": "oklch(0.3907 0.0399 242.2181)",
      "--input": "oklch(0.3093 0.0305 232.0027)",
      "--ring": "oklch(0.6998 0.0896 201.8672)",
      "--chart-1": "oklch(0.6998 0.0896 201.8672)",
      "--chart-2": "oklch(0.7794 0.0803 4.1330)",
      "--chart-3": "oklch(0.6699 0.0988 356.9762)",
      "--chart-4": "oklch(0.4408 0.0702 217.0848)",
      "--chart-5": "oklch(0.2713 0.0086 255.5780)",
      "--sidebar": "oklch(0.2303 0.0270 235.9743)",
      "--sidebar-foreground": "oklch(0.9670 0.0029 264.5419)",
      "--sidebar-primary": "oklch(0.6559 0.2118 354.3084)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.8228 0.1095 346.0184)",
      "--sidebar-accent-foreground": "oklch(0.2781 0.0296 256.8480)",
      "--sidebar-border": "oklch(0.3729 0.0306 259.7328)",
      "--sidebar-ring": "oklch(0.6559 0.2118 354.3084)"
    }
  },
  {
    name: "Caffeine",
    value: "caffeine",
    description: "Energetic coffee-inspired theme with Roboto font",
    fontFamily: "Roboto, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9821 0 0)",
      "--foreground": "oklch(0.2435 0 0)",
      "--card": "oklch(0.9911 0 0)",
      "--card-foreground": "oklch(0.2435 0 0)",
      "--popover": "oklch(0.9911 0 0)",
      "--popover-foreground": "oklch(0.2435 0 0)",
      "--primary": "oklch(0.4341 0.0392 41.9938)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.9200 0.0651 74.3695)",
      "--secondary-foreground": "oklch(0.3499 0.0685 40.8288)",
      "--muted": "oklch(0.9521 0 0)",
      "--muted-foreground": "oklch(0.5032 0 0)",
      "--accent": "oklch(0.9310 0 0)",
      "--accent-foreground": "oklch(0.2435 0 0)",
      "--destructive": "oklch(0.6271 0.1936 33.3390)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.8822 0 0)",
      "--input": "oklch(0.8822 0 0)",
      "--ring": "oklch(0.4341 0.0392 41.9938)",
      "--chart-1": "oklch(0.4341 0.0392 41.9938)",
      "--chart-2": "oklch(0.9200 0.0651 74.3695)",
      "--chart-3": "oklch(0.9310 0 0)",
      "--chart-4": "oklch(0.9367 0.0523 75.5009)",
      "--chart-5": "oklch(0.4338 0.0437 41.6746)",
      "--sidebar": "oklch(0.9881 0 0)",
      "--sidebar-foreground": "oklch(0.2645 0 0)",
      "--sidebar-primary": "oklch(0.3250 0 0)",
      "--sidebar-primary-foreground": "oklch(0.9881 0 0)",
      "--sidebar-accent": "oklch(0.9761 0 0)",
      "--sidebar-accent-foreground": "oklch(0.3250 0 0)",
      "--sidebar-border": "oklch(0.9401 0 0)",
      "--sidebar-ring": "oklch(0.7731 0 0)"
    },
    darkColors: {
      "--background": "oklch(0.1776 0 0)",
      "--foreground": "oklch(0.9491 0 0)",
      "--card": "oklch(0.2134 0 0)",
      "--card-foreground": "oklch(0.9491 0 0)",
      "--popover": "oklch(0.2134 0 0)",
      "--popover-foreground": "oklch(0.9491 0 0)",
      "--primary": "oklch(0.9247 0.0524 66.1732)",
      "--primary-foreground": "oklch(0.2029 0.0240 200.1962)",
      "--secondary": "oklch(0.3163 0.0190 63.6992)",
      "--secondary-foreground": "oklch(0.9247 0.0524 66.1732)",
      "--muted": "oklch(0.2520 0 0)",
      "--muted-foreground": "oklch(0.7699 0 0)",
      "--accent": "oklch(0.2850 0 0)",
      "--accent-foreground": "oklch(0.9491 0 0)",
      "--destructive": "oklch(0.6271 0.1936 33.3390)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.2351 0.0115 91.7467)",
      "--input": "oklch(0.4017 0 0)",
      "--ring": "oklch(0.9247 0.0524 66.1732)",
      "--chart-1": "oklch(0.9247 0.0524 66.1732)",
      "--chart-2": "oklch(0.3163 0.0190 63.6992)",
      "--chart-3": "oklch(0.2850 0 0)",
      "--chart-4": "oklch(0.3481 0.0219 67.0001)",
      "--chart-5": "oklch(0.9245 0.0533 67.0855)",
      "--sidebar": "oklch(0.2103 0.0059 285.8852)",
      "--sidebar-foreground": "oklch(0.9674 0.0013 286.3752)",
      "--sidebar-primary": "oklch(0.4882 0.2172 264.3763)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.2739 0.0055 286.0326)",
      "--sidebar-accent-foreground": "oklch(0.9674 0.0013 286.3752)",
      "--sidebar-border": "oklch(0.2739 0.0055 286.0326)",
      "--sidebar-ring": "oklch(0.8711 0.0055 286.2860)"
    }
  },
  {
    name: "Calypso",
    value: "calypso",
    description: "Calypso theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "1rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9715 0.0053 257.49)",
      "--foreground": "oklch(0.2783 0.0505 265.5)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2783 0.0505 265.5)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2783 0.0505 265.5)",
      "--primary": "oklch(0.4924 0.0665 220.5)",
      "--secondary": "oklch(0.5866 0.2102 29.04)",
      "--muted": "oklch(0.957 0.0106 258.56)",
      "--muted-foreground": "oklch(0.4437 0.0612 263.78)",
      "--accent": "oklch(0.957 0.0106 258.56)",
      "--accent-foreground": "oklch(0.2783 0.0505 265.5)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9196 0.0213 259.16)",
      "--input": "oklch(0.8614 0.0319 260.48)",
      "--ring": "oklch(0.4924 0.0665 220.5)",
      "--chart-1": "oklch(0.4924 0.0665 220.5)",
      "--chart-2": "oklch(0.5866 0.2102 29.04)",
      "--chart-3": "oklch(0.8655 0.0799 208.55)",
      "--chart-4": "oklch(0.8107 0.0998 22.73)",
      "--chart-5": "oklch(0.9579 0.0282 206.94)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.2783 0.0505 265.5)",
      "--sidebar-primary": "oklch(0.4924 0.0665 220.5)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9715 0.0053 257.49)",
      "--sidebar-accent-foreground": "oklch(0.2783 0.0505 265.5)",
      "--sidebar-border": "oklch(0.9196 0.0213 259.16)",
      "--sidebar-ring": "oklch(0.4924 0.0665 220.5)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2783 0.0505 265.5)"
    },
    darkColors: {
      "--background": "oklch(0.1535 0.0399 268.16)",
      "--foreground": "oklch(0.9196 0.0213 259.16)",
      "--card": "oklch(0.2077 0.0439 267.26)",
      "--card-foreground": "oklch(0.9196 0.0213 259.16)",
      "--popover": "oklch(0.2077 0.0439 267.26)",
      "--popover-foreground": "oklch(0.9196 0.0213 259.16)",
      "--primary": "oklch(0.4924 0.0665 220.5)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.5866 0.2102 29.04)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.2783 0.0505 265.5)",
      "--muted-foreground": "oklch(0.8614 0.0319 260.48)",
      "--accent": "oklch(0.2783 0.0505 265.5)",
      "--accent-foreground": "oklch(0.9196 0.0213 259.16)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2783 0.0505 265.5)",
      "--input": "oklch(0.3706 0.0559 263.8)",
      "--ring": "oklch(0.4924 0.0665 220.5)",
      "--chart-1": "oklch(0.4924 0.0665 220.5)",
      "--chart-2": "oklch(0.5866 0.2102 29.04)",
      "--chart-3": "oklch(0.8655 0.0799 208.55)",
      "--chart-4": "oklch(0.8107 0.0998 22.73)",
      "--chart-5": "oklch(0.9579 0.0282 206.94)",
      "--sidebar": "oklch(0.2077 0.0439 267.26)",
      "--sidebar-foreground": "oklch(0.9196 0.0213 259.16)",
      "--sidebar-primary": "oklch(0.4924 0.0665 220.5)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.2783 0.0505 265.5)",
      "--sidebar-accent-foreground": "oklch(0.9196 0.0213 259.16)",
      "--sidebar-border": "oklch(0.2783 0.0505 265.5)",
      "--sidebar-ring": "oklch(0.4924 0.0665 220.5)"
    }
  },
  {
    name: "Candyland",
    value: "candyland",
    description: "Sweet colorful candy theme with Comfortaa font",
    fontFamily: "Comfortaa, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Comfortaa:wght@400;500;600;700&display=swap",
    borderRadius: "1.25rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9809 0.0025 228.7836)",
      "--foreground": "oklch(0.3211 0 0)",
      "--card": "oklch(1.0000 0 0)",
      "--card-foreground": "oklch(0.3211 0 0)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.3211 0 0)",
      "--primary": "oklch(0.8677 0.0735 7.0855)",
      "--primary-foreground": "oklch(0 0 0)",
      "--secondary": "oklch(0.8148 0.0819 225.7537)",
      "--secondary-foreground": "oklch(0 0 0)",
      "--muted": "oklch(0.8828 0.0285 98.1033)",
      "--muted-foreground": "oklch(0.5382 0 0)",
      "--accent": "oklch(0.9680 0.2110 109.7692)",
      "--accent-foreground": "oklch(0 0 0)",
      "--destructive": "oklch(0.6368 0.2078 25.3313)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.8699 0 0)",
      "--input": "oklch(0.8699 0 0)",
      "--ring": "oklch(0.8677 0.0735 7.0855)",
      "--chart-1": "oklch(0.8677 0.0735 7.0855)",
      "--chart-2": "oklch(0.8148 0.0819 225.7537)",
      "--chart-3": "oklch(0.9680 0.2110 109.7692)",
      "--chart-4": "oklch(0.8027 0.1355 349.2347)",
      "--chart-5": "oklch(0.7395 0.2268 142.8504)",
      "--sidebar": "oklch(0.9809 0.0025 228.7836)",
      "--sidebar-foreground": "oklch(0.3211 0 0)",
      "--sidebar-primary": "oklch(0.8677 0.0735 7.0855)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.9680 0.2110 109.7692)",
      "--sidebar-accent-foreground": "oklch(0 0 0)",
      "--sidebar-border": "oklch(0.8699 0 0)",
      "--sidebar-ring": "oklch(0.8677 0.0735 7.0855)"
    },
    darkColors: {
      "--background": "oklch(0.2303 0.0125 264.2926)",
      "--foreground": "oklch(0.9219 0 0)",
      "--card": "oklch(0.3210 0.0078 223.6661)",
      "--card-foreground": "oklch(0.9219 0 0)",
      "--popover": "oklch(0.3210 0.0078 223.6661)",
      "--popover-foreground": "oklch(0.9219 0 0)",
      "--primary": "oklch(0.8027 0.1355 349.2347)",
      "--primary-foreground": "oklch(0 0 0)",
      "--secondary": "oklch(0.7395 0.2268 142.8504)",
      "--secondary-foreground": "oklch(0 0 0)",
      "--muted": "oklch(0.3867 0 0)",
      "--muted-foreground": "oklch(0.7155 0 0)",
      "--accent": "oklch(0.8148 0.0819 225.7537)",
      "--accent-foreground": "oklch(0 0 0)",
      "--destructive": "oklch(0.6368 0.2078 25.3313)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.3867 0 0)",
      "--input": "oklch(0.3867 0 0)",
      "--ring": "oklch(0.8027 0.1355 349.2347)",
      "--chart-1": "oklch(0.8027 0.1355 349.2347)",
      "--chart-2": "oklch(0.7395 0.2268 142.8504)",
      "--chart-3": "oklch(0.8148 0.0819 225.7537)",
      "--chart-4": "oklch(0.9680 0.2110 109.7692)",
      "--chart-5": "oklch(0.8652 0.1768 90.3816)",
      "--sidebar": "oklch(0.2303 0.0125 264.2926)",
      "--sidebar-foreground": "oklch(0.9219 0 0)",
      "--sidebar-primary": "oklch(0.8027 0.1355 349.2347)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.8148 0.0819 225.7537)",
      "--sidebar-accent-foreground": "oklch(0 0 0)",
      "--sidebar-border": "oklch(0.3867 0 0)",
      "--sidebar-ring": "oklch(0.8027 0.1355 349.2347)"
    }
  },
  {
    name: "Canvas",
    value: "canvas",
    description: "Canvas theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.274 0.008 286.03)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.274 0.008 286.03)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.274 0.008 286.03)",
      "--primary": "oklch(0.141 0.004 285.83)",
      "--secondary": "oklch(0.967 0.0027 286.38)",
      "--muted": "oklch(0.985 0.0013 286.84)",
      "--muted-foreground": "oklch(0.442 0.0147 285.79)",
      "--accent": "oklch(0.985 0.0013 286.84)",
      "--accent-foreground": "oklch(0.274 0.008 286.03)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.92 0.0053 286.32)",
      "--input": "oklch(0.871 0.008 286.29)",
      "--ring": "oklch(0.274 0.008 286.03)",
      "--chart-1": "oklch(0.141 0.004 285.83)",
      "--chart-2": "oklch(0.442 0.0147 285.79)",
      "--chart-3": "oklch(0.274 0.008 286.03)",
      "--chart-4": "oklch(0.705 0.012 286.07)",
      "--chart-5": "oklch(0.871 0.008 286.29)",
      "--sidebar": "oklch(0.967 0.0027 286.38)",
      "--sidebar-foreground": "oklch(0.274 0.008 286.03)",
      "--sidebar-primary": "oklch(0.141 0.004 285.83)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.92 0.0053 286.32)",
      "--sidebar-accent-foreground": "oklch(0.274 0.008 286.03)",
      "--sidebar-border": "oklch(0.92 0.0053 286.32)",
      "--sidebar-ring": "oklch(0.274 0.008 286.03)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.274 0.008 286.03)"
    },
    darkColors: {
      "--background": "oklch(0.141 0.004 285.83)",
      "--foreground": "oklch(0.92 0.0053 286.32)",
      "--card": "oklch(0.141 0.004 285.83)",
      "--card-foreground": "oklch(0.92 0.0053 286.32)",
      "--popover": "oklch(0.141 0.004 285.83)",
      "--popover-foreground": "oklch(0.92 0.0053 286.32)",
      "--primary": "oklch(0.985 0.0013 286.84)",
      "--primary-foreground": "oklch(0.21 0.0053 285.89)",
      "--secondary": "oklch(0.274 0.008 286.03)",
      "--secondary-foreground": "oklch(0.92 0.0053 286.32)",
      "--muted": "oklch(0.21 0.0053 285.89)",
      "--muted-foreground": "oklch(0.705 0.012 286.07)",
      "--accent": "oklch(0.21 0.0053 285.89)",
      "--accent-foreground": "oklch(0.92 0.0053 286.32)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.274 0.008 286.03)",
      "--input": "oklch(0.37 0.012 285.81)",
      "--ring": "oklch(0.92 0.0053 286.32)",
      "--chart-1": "oklch(0.985 0.0013 286.84)",
      "--chart-2": "oklch(0.705 0.012 286.07)",
      "--chart-3": "oklch(0.92 0.0053 286.32)",
      "--chart-4": "oklch(0.442 0.0147 285.79)",
      "--chart-5": "oklch(0.37 0.012 285.81)",
      "--sidebar": "oklch(0.21 0.0053 285.89)",
      "--sidebar-foreground": "oklch(0.92 0.0053 286.32)",
      "--sidebar-primary": "oklch(0.985 0.0013 286.84)",
      "--sidebar-primary-foreground": "oklch(0.21 0.0053 285.89)",
      "--sidebar-accent": "oklch(0.274 0.008 286.03)",
      "--sidebar-accent-foreground": "oklch(0.92 0.0053 286.32)",
      "--sidebar-border": "oklch(0.274 0.008 286.03)",
      "--sidebar-ring": "oklch(0.92 0.0053 286.32)"
    }
  },
  {
    name: "Catppuccin",
    value: "catppuccin",
    description: "Pastel purple theme inspired by Catppuccin with Fira Code font",
    fontFamily: "Fira Code, ui-monospace, monospace",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Fira+Code:wght@400;500;600&display=swap",
    borderRadius: "0.75rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9578 0.0058 264.5321)",
      "--foreground": "oklch(0.4355 0.0430 279.3250)",
      "--card": "oklch(1.0000 0 0)",
      "--card-foreground": "oklch(0.4355 0.0430 279.3250)",
      "--popover": "oklch(0.8575 0.0145 268.4756)",
      "--popover-foreground": "oklch(0.4355 0.0430 279.3250)",
      "--primary": "oklch(0.5547 0.2503 297.0156)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.8575 0.0145 268.4756)",
      "--secondary-foreground": "oklch(0.4355 0.0430 279.3250)",
      "--muted": "oklch(0.9060 0.0117 264.5071)",
      "--muted-foreground": "oklch(0.5471 0.0343 279.0837)",
      "--accent": "oklch(0.6820 0.1448 235.3822)",
      "--accent-foreground": "oklch(1.0000 0 0)",
      "--destructive": "oklch(0.5505 0.2155 19.8095)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.8083 0.0174 271.1982)",
      "--input": "oklch(0.8575 0.0145 268.4756)",
      "--ring": "oklch(0.5547 0.2503 297.0156)",
      "--chart-1": "oklch(0.5547 0.2503 297.0156)",
      "--chart-2": "oklch(0.6820 0.1448 235.3822)",
      "--chart-3": "oklch(0.6250 0.1772 140.4448)",
      "--chart-4": "oklch(0.6920 0.2041 42.4293)",
      "--chart-5": "oklch(0.7141 0.1045 33.0967)",
      "--sidebar": "oklch(0.9335 0.0087 264.5206)",
      "--sidebar-foreground": "oklch(0.4355 0.0430 279.3250)",
      "--sidebar-primary": "oklch(0.5547 0.2503 297.0156)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.6820 0.1448 235.3822)",
      "--sidebar-accent-foreground": "oklch(1.0000 0 0)",
      "--sidebar-border": "oklch(0.8083 0.0174 271.1982)",
      "--sidebar-ring": "oklch(0.5547 0.2503 297.0156)"
    },
    darkColors: {
      "--background": "oklch(0.2155 0.0254 284.0647)",
      "--foreground": "oklch(0.8787 0.0426 272.2767)",
      "--card": "oklch(0.2429 0.0304 283.9110)",
      "--card-foreground": "oklch(0.8787 0.0426 272.2767)",
      "--popover": "oklch(0.4037 0.0320 280.1520)",
      "--popover-foreground": "oklch(0.8787 0.0426 272.2767)",
      "--primary": "oklch(0.7871 0.1187 304.7693)",
      "--primary-foreground": "oklch(0.2429 0.0304 283.9110)",
      "--secondary": "oklch(0.4765 0.0340 278.6430)",
      "--secondary-foreground": "oklch(0.8787 0.0426 272.2767)",
      "--muted": "oklch(0.2973 0.0294 276.2144)",
      "--muted-foreground": "oklch(0.7510 0.0396 273.9320)",
      "--accent": "oklch(0.8467 0.0833 210.2545)",
      "--accent-foreground": "oklch(0.2429 0.0304 283.9110)",
      "--destructive": "oklch(0.7556 0.1297 2.7642)",
      "--destructive-foreground": "oklch(0.2429 0.0304 283.9110)",
      "--border": "oklch(0.3240 0.0319 281.9784)",
      "--input": "oklch(0.3240 0.0319 281.9784)",
      "--ring": "oklch(0.7871 0.1187 304.7693)",
      "--chart-1": "oklch(0.7871 0.1187 304.7693)",
      "--chart-2": "oklch(0.8467 0.0833 210.2545)",
      "--chart-3": "oklch(0.8577 0.1092 142.7153)",
      "--chart-4": "oklch(0.8237 0.1015 52.6294)",
      "--chart-5": "oklch(0.9226 0.0238 30.4919)",
      "--sidebar": "oklch(0.1828 0.0204 284.2039)",
      "--sidebar-foreground": "oklch(0.8787 0.0426 272.2767)",
      "--sidebar-primary": "oklch(0.7871 0.1187 304.7693)",
      "--sidebar-primary-foreground": "oklch(0.2429 0.0304 283.9110)",
      "--sidebar-accent": "oklch(0.8467 0.0833 210.2545)",
      "--sidebar-accent-foreground": "oklch(0.2429 0.0304 283.9110)",
      "--sidebar-border": "oklch(0.3240 0.0319 281.9784)",
      "--sidebar-ring": "oklch(0.7871 0.1187 304.7693)"
    }
  },
  {
    name: "Citrus",
    value: "citrus",
    description: "Citrus theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9851 0 0)",
      "--foreground": "oklch(0.269 0 0)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.269 0 0)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.269 0 0)",
      "--primary": "oklch(0.8719 0.1829 125.59)",
      "--secondary": "oklch(0.5591 0.0631 185.87)",
      "--muted": "oklch(0.97 0 0)",
      "--muted-foreground": "oklch(0.439 0 0)",
      "--accent": "oklch(0.97 0 0)",
      "--accent-foreground": "oklch(0.269 0 0)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.922 0 0)",
      "--input": "oklch(0.87 0 0)",
      "--ring": "oklch(0.8719 0.1829 125.59)",
      "--chart-1": "oklch(0.8719 0.1829 125.59)",
      "--chart-2": "oklch(0.5591 0.0631 185.87)",
      "--chart-3": "oklch(0.8719 0.1829 125.59)",
      "--chart-4": "oklch(0.8565 0.0734 182)",
      "--chart-5": "oklch(0.9676 0.0628 121.5)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.269 0 0)",
      "--sidebar-primary": "oklch(0.8719 0.1829 125.59)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.9851 0 0)",
      "--sidebar-accent-foreground": "oklch(0.269 0 0)",
      "--sidebar-border": "oklch(0.922 0 0)",
      "--sidebar-ring": "oklch(0.8719 0.1829 125.59)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.269 0 0)"
    },
    darkColors: {
      "--background": "oklch(0.145 0 0)",
      "--foreground": "oklch(0.922 0 0)",
      "--card": "oklch(0.205 0 0)",
      "--card-foreground": "oklch(0.922 0 0)",
      "--popover": "oklch(0.205 0 0)",
      "--popover-foreground": "oklch(0.922 0 0)",
      "--primary": "oklch(0.8719 0.1829 125.59)",
      "--primary-foreground": "oklch(0 0 0)",
      "--secondary": "oklch(0.5591 0.0631 185.87)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.269 0 0)",
      "--muted-foreground": "oklch(0.87 0 0)",
      "--accent": "oklch(0.269 0 0)",
      "--accent-foreground": "oklch(0.922 0 0)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.269 0 0)",
      "--input": "oklch(0.371 0 0)",
      "--ring": "oklch(0.8719 0.1829 125.59)",
      "--chart-1": "oklch(0.8719 0.1829 125.59)",
      "--chart-2": "oklch(0.5591 0.0631 185.87)",
      "--chart-3": "oklch(0.8719 0.1829 125.59)",
      "--chart-4": "oklch(0.8565 0.0734 182)",
      "--chart-5": "oklch(0.9676 0.0628 121.5)",
      "--sidebar": "oklch(0.205 0 0)",
      "--sidebar-foreground": "oklch(0.922 0 0)",
      "--sidebar-primary": "oklch(0.8719 0.1829 125.59)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.269 0 0)",
      "--sidebar-accent-foreground": "oklch(0.922 0 0)",
      "--sidebar-border": "oklch(0.269 0 0)",
      "--sidebar-ring": "oklch(0.8719 0.1829 125.59)"
    }
  },
  {
    name: "Claude",
    value: "claude",
    description: "Warm and friendly theme inspired by Claude AI with Inter font",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap",
    borderRadius: "0.625rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9818 0.0054 95.0986)",
      "--foreground": "oklch(0.3438 0.0269 95.7226)",
      "--card": "oklch(0.9818 0.0054 95.0986)",
      "--card-foreground": "oklch(0.1908 0.0020 106.5859)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.2671 0.0196 98.9390)",
      "--primary": "oklch(0.6171 0.1375 39.0427)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.9245 0.0138 92.9892)",
      "--secondary-foreground": "oklch(0.4334 0.0177 98.6048)",
      "--muted": "oklch(0.9341 0.0153 90.2390)",
      "--muted-foreground": "oklch(0.6059 0.0075 97.4233)",
      "--accent": "oklch(0.9245 0.0138 92.9892)",
      "--accent-foreground": "oklch(0.2671 0.0196 98.9390)",
      "--destructive": "oklch(0.1908 0.0020 106.5859)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.8847 0.0069 97.3627)",
      "--input": "oklch(0.7621 0.0156 98.3528)",
      "--ring": "oklch(0.6171 0.1375 39.0427)",
      "--chart-1": "oklch(0.5583 0.1276 42.9956)",
      "--chart-2": "oklch(0.6898 0.1581 290.4107)",
      "--chart-3": "oklch(0.8816 0.0276 93.1280)",
      "--chart-4": "oklch(0.8822 0.0403 298.1792)",
      "--chart-5": "oklch(0.5608 0.1348 42.0584)",
      "--sidebar": "oklch(0.9663 0.0080 98.8792)",
      "--sidebar-foreground": "oklch(0.3590 0.0051 106.6524)",
      "--sidebar-primary": "oklch(0.6171 0.1375 39.0427)",
      "--sidebar-primary-foreground": "oklch(0.9881 0 0)",
      "--sidebar-accent": "oklch(0.9245 0.0138 92.9892)",
      "--sidebar-accent-foreground": "oklch(0.3250 0 0)",
      "--sidebar-border": "oklch(0.9401 0 0)",
      "--sidebar-ring": "oklch(0.7731 0 0)"
    },
    darkColors: {
      "--background": "oklch(0.2679 0.0036 106.6427)",
      "--foreground": "oklch(0.8074 0.0142 93.0137)",
      "--card": "oklch(0.2679 0.0036 106.6427)",
      "--card-foreground": "oklch(0.9818 0.0054 95.0986)",
      "--popover": "oklch(0.3085 0.0035 106.6039)",
      "--popover-foreground": "oklch(0.9211 0.0040 106.4781)",
      "--primary": "oklch(0.6724 0.1308 38.7559)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.9818 0.0054 95.0986)",
      "--secondary-foreground": "oklch(0.3085 0.0035 106.6039)",
      "--muted": "oklch(0.2213 0.0038 106.7070)",
      "--muted-foreground": "oklch(0.7713 0.0169 99.0657)",
      "--accent": "oklch(0.2130 0.0078 95.4245)",
      "--accent-foreground": "oklch(0.9663 0.0080 98.8792)",
      "--destructive": "oklch(0.6368 0.2078 25.3313)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.3618 0.0101 106.8928)",
      "--input": "oklch(0.4336 0.0113 100.2195)",
      "--ring": "oklch(0.6724 0.1308 38.7559)",
      "--chart-1": "oklch(0.5583 0.1276 42.9956)",
      "--chart-2": "oklch(0.6898 0.1581 290.4107)",
      "--chart-3": "oklch(0.2130 0.0078 95.4245)",
      "--chart-4": "oklch(0.3074 0.0516 289.3230)",
      "--chart-5": "oklch(0.5608 0.1348 42.0584)",
      "--sidebar": "oklch(0.2357 0.0024 67.7077)",
      "--sidebar-foreground": "oklch(0.8074 0.0142 93.0137)",
      "--sidebar-primary": "oklch(0.3250 0 0)",
      "--sidebar-primary-foreground": "oklch(0.9881 0 0)",
      "--sidebar-accent": "oklch(0.1680 0.0020 106.6177)",
      "--sidebar-accent-foreground": "oklch(0.8074 0.0142 93.0137)",
      "--sidebar-border": "oklch(0.3618 0.0101 106.8928)",
      "--sidebar-ring": "oklch(0.6724 0.1308 38.7559)"
    }
  },
  {
    name: "Claymorphism",
    value: "claymorphism",
    description: "Soft clay-like theme with rounded edges and Source Sans Pro font",
    fontFamily: "Source Sans Pro, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Source+Sans+Pro:wght@400;600;700&display=swap",
    borderRadius: "1.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9232 0.0026 48.7171)",
      "--foreground": "oklch(0.2795 0.0368 260.0310)",
      "--card": "oklch(0.9699 0.0013 106.4238)",
      "--card-foreground": "oklch(0.2795 0.0368 260.0310)",
      "--popover": "oklch(0.9699 0.0013 106.4238)",
      "--popover-foreground": "oklch(0.2795 0.0368 260.0310)",
      "--primary": "oklch(0.5854 0.2041 277.1173)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.8687 0.0043 56.3660)",
      "--secondary-foreground": "oklch(0.4461 0.0263 256.8018)",
      "--muted": "oklch(0.9232 0.0026 48.7171)",
      "--muted-foreground": "oklch(0.5510 0.0234 264.3637)",
      "--accent": "oklch(0.9376 0.0260 321.9388)",
      "--accent-foreground": "oklch(0.3729 0.0306 259.7328)",
      "--destructive": "oklch(0.6368 0.2078 25.3313)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.8687 0.0043 56.3660)",
      "--input": "oklch(0.8687 0.0043 56.3660)",
      "--ring": "oklch(0.5854 0.2041 277.1173)",
      "--chart-1": "oklch(0.5854 0.2041 277.1173)",
      "--chart-2": "oklch(0.5106 0.2301 276.9656)",
      "--chart-3": "oklch(0.4568 0.2146 277.0229)",
      "--chart-4": "oklch(0.3984 0.1773 277.3662)",
      "--chart-5": "oklch(0.3588 0.1354 278.6973)",
      "--sidebar": "oklch(0.8687 0.0043 56.3660)",
      "--sidebar-foreground": "oklch(0.2795 0.0368 260.0310)",
      "--sidebar-primary": "oklch(0.5854 0.2041 277.1173)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.9376 0.0260 321.9388)",
      "--sidebar-accent-foreground": "oklch(0.3729 0.0306 259.7328)",
      "--sidebar-border": "oklch(0.8687 0.0043 56.3660)",
      "--sidebar-ring": "oklch(0.5854 0.2041 277.1173)"
    },
    darkColors: {
      "--background": "oklch(0.2244 0.0074 67.4370)",
      "--foreground": "oklch(0.9288 0.0126 255.5078)",
      "--card": "oklch(0.2801 0.0080 59.3379)",
      "--card-foreground": "oklch(0.9288 0.0126 255.5078)",
      "--popover": "oklch(0.2801 0.0080 59.3379)",
      "--popover-foreground": "oklch(0.9288 0.0126 255.5078)",
      "--primary": "oklch(0.6801 0.1583 276.9349)",
      "--primary-foreground": "oklch(0.2244 0.0074 67.4370)",
      "--secondary": "oklch(0.3359 0.0077 59.4197)",
      "--secondary-foreground": "oklch(0.8717 0.0093 258.3382)",
      "--muted": "oklch(0.2801 0.0080 59.3379)",
      "--muted-foreground": "oklch(0.7137 0.0192 261.3246)",
      "--accent": "oklch(0.3896 0.0074 59.4734)",
      "--accent-foreground": "oklch(0.8717 0.0093 258.3382)",
      "--destructive": "oklch(0.6368 0.2078 25.3313)",
      "--destructive-foreground": "oklch(0.2244 0.0074 67.4370)",
      "--border": "oklch(0.3359 0.0077 59.4197)",
      "--input": "oklch(0.3359 0.0077 59.4197)",
      "--ring": "oklch(0.6801 0.1583 276.9349)",
      "--chart-1": "oklch(0.6801 0.1583 276.9349)",
      "--chart-2": "oklch(0.5854 0.2041 277.1173)",
      "--chart-3": "oklch(0.5106 0.2301 276.9656)",
      "--chart-4": "oklch(0.4568 0.2146 277.0229)",
      "--chart-5": "oklch(0.3984 0.1773 277.3662)",
      "--sidebar": "oklch(0.3359 0.0077 59.4197)",
      "--sidebar-foreground": "oklch(0.9288 0.0126 255.5078)",
      "--sidebar-primary": "oklch(0.6801 0.1583 276.9349)",
      "--sidebar-primary-foreground": "oklch(0.2244 0.0074 67.4370)",
      "--sidebar-accent": "oklch(0.3896 0.0074 59.4734)",
      "--sidebar-accent-foreground": "oklch(0.8717 0.0093 258.3382)",
      "--sidebar-border": "oklch(0.3359 0.0077 59.4197)",
      "--sidebar-ring": "oklch(0.6801 0.1583 276.9349)"
    }
  },
  {
    name: "Clean Slate",
    value: "clean-slate",
    description: "Minimalist clean design with subtle grays",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.99 0 0)",
      "--foreground": "oklch(0.15 0 0)",
      "--card": "oklch(0.99 0 0)",
      "--card-foreground": "oklch(0.15 0 0)",
      "--popover": "oklch(0.99 0 0)",
      "--popover-foreground": "oklch(0.15 0 0)",
      "--primary": "oklch(0.35 0 0)",
      "--primary-foreground": "oklch(0.99 0 0)",
      "--secondary": "oklch(0.95 0 0)",
      "--secondary-foreground": "oklch(0.35 0 0)",
      "--muted": "oklch(0.95 0 0)",
      "--muted-foreground": "oklch(0.55 0 0)",
      "--accent": "oklch(0.95 0 0)",
      "--accent-foreground": "oklch(0.35 0 0)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.91 0 0)",
      "--input": "oklch(0.91 0 0)",
      "--ring": "oklch(0.35 0 0)",
      "--chart-1": "oklch(0.6 0.05 200)",
      "--chart-2": "oklch(0.5 0.05 220)",
      "--chart-3": "oklch(0.4 0.05 240)",
      "--chart-4": "oklch(0.7 0.05 180)",
      "--chart-5": "oklch(0.8 0.05 160)",
      "--sidebar": "oklch(0.98 0 0)",
      "--sidebar-foreground": "oklch(0.15 0 0)",
      "--sidebar-primary": "oklch(0.35 0 0)",
      "--sidebar-primary-foreground": "oklch(0.99 0 0)",
      "--sidebar-accent": "oklch(0.95 0 0)",
      "--sidebar-accent-foreground": "oklch(0.35 0 0)",
      "--sidebar-border": "oklch(0.91 0 0)",
      "--sidebar-ring": "oklch(0.35 0 0)"
    },
    darkColors: {
      "--background": "oklch(0.08 0 0)",
      "--foreground": "oklch(0.95 0 0)",
      "--card": "oklch(0.12 0 0)",
      "--card-foreground": "oklch(0.95 0 0)",
      "--popover": "oklch(0.08 0 0)",
      "--popover-foreground": "oklch(0.95 0 0)",
      "--primary": "oklch(0.75 0 0)",
      "--primary-foreground": "oklch(0.08 0 0)",
      "--secondary": "oklch(0.18 0 0)",
      "--secondary-foreground": "oklch(0.95 0 0)",
      "--muted": "oklch(0.18 0 0)",
      "--muted-foreground": "oklch(0.65 0 0)",
      "--accent": "oklch(0.18 0 0)",
      "--accent-foreground": "oklch(0.95 0 0)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.25 0 0)",
      "--input": "oklch(0.25 0 0)",
      "--ring": "oklch(0.75 0 0)",
      "--chart-1": "oklch(0.7 0.05 200)",
      "--chart-2": "oklch(0.6 0.05 220)",
      "--chart-3": "oklch(0.5 0.05 240)",
      "--chart-4": "oklch(0.8 0.05 180)",
      "--chart-5": "oklch(0.9 0.05 160)",
      "--sidebar": "oklch(0.12 0 0)",
      "--sidebar-foreground": "oklch(0.95 0 0)",
      "--sidebar-primary": "oklch(0.75 0 0)",
      "--sidebar-primary-foreground": "oklch(0.08 0 0)",
      "--sidebar-accent": "oklch(0.18 0 0)",
      "--sidebar-accent-foreground": "oklch(0.95 0 0)",
      "--sidebar-border": "oklch(0.25 0 0)",
      "--sidebar-ring": "oklch(0.75 0 0)"
    }
  },
  {
    name: "Cosmic Night",
    value: "cosmic-night",
    description: "Deep space colors with cosmic purples",
    fontFamily: "Orbitron, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Orbitron:wght@400;500;600;700&display=swap",
    borderRadius: "0.75rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.98 0.01 280)",
      "--foreground": "oklch(0.1 0.05 280)",
      "--card": "oklch(0.98 0.01 280)",
      "--card-foreground": "oklch(0.1 0.05 280)",
      "--popover": "oklch(0.98 0.01 280)",
      "--popover-foreground": "oklch(0.1 0.05 280)",
      "--primary": "oklch(0.45 0.15 280)",
      "--primary-foreground": "oklch(0.98 0.01 280)",
      "--secondary": "oklch(0.93 0.02 280)",
      "--secondary-foreground": "oklch(0.45 0.15 280)",
      "--muted": "oklch(0.93 0.02 280)",
      "--muted-foreground": "oklch(0.55 0.1 280)",
      "--accent": "oklch(0.93 0.02 280)",
      "--accent-foreground": "oklch(0.45 0.15 280)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.88 0.03 280)",
      "--input": "oklch(0.88 0.03 280)",
      "--ring": "oklch(0.45 0.15 280)",
      "--chart-1": "oklch(0.6 0.2 280)",
      "--chart-2": "oklch(0.5 0.25 300)",
      "--chart-3": "oklch(0.4 0.2 260)",
      "--chart-4": "oklch(0.7 0.15 320)",
      "--chart-5": "oklch(0.8 0.1 240)",
      "--sidebar": "oklch(0.96 0.02 280)",
      "--sidebar-foreground": "oklch(0.1 0.05 280)",
      "--sidebar-primary": "oklch(0.45 0.15 280)",
      "--sidebar-primary-foreground": "oklch(0.98 0.01 280)",
      "--sidebar-accent": "oklch(0.93 0.02 280)",
      "--sidebar-accent-foreground": "oklch(0.45 0.15 280)",
      "--sidebar-border": "oklch(0.88 0.03 280)",
      "--sidebar-ring": "oklch(0.45 0.15 280)"
    },
    darkColors: {
      "--background": "oklch(0.05 0.03 280)",
      "--foreground": "oklch(0.9 0.05 280)",
      "--card": "oklch(0.08 0.05 280)",
      "--card-foreground": "oklch(0.9 0.05 280)",
      "--popover": "oklch(0.05 0.03 280)",
      "--popover-foreground": "oklch(0.9 0.05 280)",
      "--primary": "oklch(0.7 0.2 280)",
      "--primary-foreground": "oklch(0.05 0.03 280)",
      "--secondary": "oklch(0.15 0.05 280)",
      "--secondary-foreground": "oklch(0.9 0.05 280)",
      "--muted": "oklch(0.15 0.05 280)",
      "--muted-foreground": "oklch(0.65 0.1 280)",
      "--accent": "oklch(0.15 0.05 280)",
      "--accent-foreground": "oklch(0.9 0.05 280)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.25 0.05 280)",
      "--input": "oklch(0.25 0.05 280)",
      "--ring": "oklch(0.7 0.2 280)",
      "--chart-1": "oklch(0.7 0.25 280)",
      "--chart-2": "oklch(0.6 0.3 300)",
      "--chart-3": "oklch(0.5 0.25 260)",
      "--chart-4": "oklch(0.8 0.2 320)",
      "--chart-5": "oklch(0.9 0.15 240)",
      "--sidebar": "oklch(0.08 0.05 280)",
      "--sidebar-foreground": "oklch(0.9 0.05 280)",
      "--sidebar-primary": "oklch(0.7 0.2 280)",
      "--sidebar-primary-foreground": "oklch(0.05 0.03 280)",
      "--sidebar-accent": "oklch(0.15 0.05 280)",
      "--sidebar-accent-foreground": "oklch(0.9 0.05 280)",
      "--sidebar-border": "oklch(0.25 0.05 280)",
      "--sidebar-ring": "oklch(0.7 0.2 280)"
    }
  },
  {
    name: "Cyberpunk",
    value: "cyberpunk",
    description: "Futuristic neon cyberpunk theme with Orbitron font",
    fontFamily: "Orbitron, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Orbitron:wght@400;500;600;700;900&display=swap",
    borderRadius: "0.375rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9816 0.0017 247.8390)",
      "--foreground": "oklch(0.1649 0.0352 281.8285)",
      "--card": "oklch(1.0000 0 0)",
      "--card-foreground": "oklch(0.1649 0.0352 281.8285)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.1649 0.0352 281.8285)",
      "--primary": "oklch(0.6726 0.2904 341.4084)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.9595 0.0200 286.0164)",
      "--secondary-foreground": "oklch(0.1649 0.0352 281.8285)",
      "--muted": "oklch(0.9595 0.0200 286.0164)",
      "--muted-foreground": "oklch(0.1649 0.0352 281.8285)",
      "--accent": "oklch(0.8903 0.1739 171.2690)",
      "--accent-foreground": "oklch(0.1649 0.0352 281.8285)",
      "--destructive": "oklch(0.6535 0.2348 34.0370)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.9205 0.0086 225.0878)",
      "--input": "oklch(0.9205 0.0086 225.0878)",
      "--ring": "oklch(0.6726 0.2904 341.4084)",
      "--chart-1": "oklch(0.6726 0.2904 341.4084)",
      "--chart-2": "oklch(0.5488 0.2944 299.0954)",
      "--chart-3": "oklch(0.8442 0.1457 209.2851)",
      "--chart-4": "oklch(0.8903 0.1739 171.2690)",
      "--chart-5": "oklch(0.9168 0.1915 101.4070)",
      "--sidebar": "oklch(0.9595 0.0200 286.0164)",
      "--sidebar-foreground": "oklch(0.1649 0.0352 281.8285)",
      "--sidebar-primary": "oklch(0.6726 0.2904 341.4084)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.8903 0.1739 171.2690)",
      "--sidebar-accent-foreground": "oklch(0.1649 0.0352 281.8285)",
      "--sidebar-border": "oklch(0.9205 0.0086 225.0878)",
      "--sidebar-ring": "oklch(0.6726 0.2904 341.4084)"
    },
    darkColors: {
      "--background": "oklch(0.1649 0.0352 281.8285)",
      "--foreground": "oklch(0.9513 0.0074 260.7315)",
      "--card": "oklch(0.2542 0.0611 281.1423)",
      "--card-foreground": "oklch(0.9513 0.0074 260.7315)",
      "--popover": "oklch(0.2542 0.0611 281.1423)",
      "--popover-foreground": "oklch(0.9513 0.0074 260.7315)",
      "--primary": "oklch(0.6726 0.2904 341.4084)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.2542 0.0611 281.1423)",
      "--secondary-foreground": "oklch(0.9513 0.0074 260.7315)",
      "--muted": "oklch(0.2542 0.0611 281.1423)",
      "--muted-foreground": "oklch(0.6245 0.0500 278.1046)",
      "--accent": "oklch(0.8903 0.1739 171.2690)",
      "--accent-foreground": "oklch(0.1649 0.0352 281.8285)",
      "--destructive": "oklch(0.6535 0.2348 34.0370)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.3279 0.0832 280.7890)",
      "--input": "oklch(0.3279 0.0832 280.7890)",
      "--ring": "oklch(0.6726 0.2904 341.4084)",
      "--chart-1": "oklch(0.6726 0.2904 341.4084)",
      "--chart-2": "oklch(0.5488 0.2944 299.0954)",
      "--chart-3": "oklch(0.8442 0.1457 209.2851)",
      "--chart-4": "oklch(0.8903 0.1739 171.2690)",
      "--chart-5": "oklch(0.9168 0.1915 101.4070)",
      "--sidebar": "oklch(0.1649 0.0352 281.8285)",
      "--sidebar-foreground": "oklch(0.9513 0.0074 260.7315)",
      "--sidebar-primary": "oklch(0.6726 0.2904 341.4084)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.8903 0.1739 171.2690)",
      "--sidebar-accent-foreground": "oklch(0.1649 0.0352 281.8285)",
      "--sidebar-border": "oklch(0.3279 0.0832 280.7890)",
      "--sidebar-ring": "oklch(0.6726 0.2904 341.4084)"
    }
  },
  {
    name: "Darkmatter",
    value: "darkmatter",
    description: "Dark sci-fi theme with warm amber accents",
    fontFamily: "ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif",
    fontWeight: "400",
    borderRadius: "0.625rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1.0000 0 0)",
      "--foreground": "oklch(0.2101 0.0318 264.6645)",
      "--card": "oklch(1.0000 0 0)",
      "--card-foreground": "oklch(0.2101 0.0318 264.6645)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.2101 0.0318 264.6645)",
      "--primary": "oklch(0.6716 0.1368 48.5130)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.5360 0.0398 196.0280)",
      "--secondary-foreground": "oklch(1.0000 0 0)",
      "--muted": "oklch(0.9670 0.0029 264.5419)",
      "--muted-foreground": "oklch(0.5510 0.0234 264.3637)",
      "--accent": "oklch(0.9491 0 0)",
      "--accent-foreground": "oklch(0.2101 0.0318 264.6645)",
      "--destructive": "oklch(0.6368 0.2078 25.3313)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.9276 0.0058 264.5313)",
      "--input": "oklch(0.9276 0.0058 264.5313)",
      "--ring": "oklch(0.6716 0.1368 48.5130)",
      "--chart-1": "oklch(0.5940 0.0443 196.0233)",
      "--chart-2": "oklch(0.7214 0.1337 49.9802)",
      "--chart-3": "oklch(0.8721 0.0864 68.5474)",
      "--chart-4": "oklch(0.6268 0 0)",
      "--chart-5": "oklch(0.6830 0 0)",
      "--sidebar": "oklch(0.9670 0.0029 264.5419)",
      "--sidebar-foreground": "oklch(0.2101 0.0318 264.6645)",
      "--sidebar-primary": "oklch(0.6716 0.1368 48.5130)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(1.0000 0 0)",
      "--sidebar-accent-foreground": "oklch(0.2101 0.0318 264.6645)",
      "--sidebar-border": "oklch(0.9276 0.0058 264.5313)",
      "--sidebar-ring": "oklch(0.6716 0.1368 48.5130)"
    },
    darkColors: {
      "--background": "oklch(0.1797 0.0043 308.1928)",
      "--foreground": "oklch(0.8109 0 0)",
      "--card": "oklch(0.1822 0 0)",
      "--card-foreground": "oklch(0.8109 0 0)",
      "--popover": "oklch(0.1797 0.0043 308.1928)",
      "--popover-foreground": "oklch(0.8109 0 0)",
      "--primary": "oklch(0.7214 0.1337 49.9802)",
      "--primary-foreground": "oklch(0.1797 0.0043 308.1928)",
      "--secondary": "oklch(0.5940 0.0443 196.0233)",
      "--secondary-foreground": "oklch(0.1797 0.0043 308.1928)",
      "--muted": "oklch(0.2520 0 0)",
      "--muted-foreground": "oklch(0.6268 0 0)",
      "--accent": "oklch(0.3211 0 0)",
      "--accent-foreground": "oklch(0.8109 0 0)",
      "--destructive": "oklch(0.5940 0.0443 196.0233)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.2520 0 0)",
      "--input": "oklch(0.2520 0 0)",
      "--ring": "oklch(0.7214 0.1337 49.9802)",
      "--chart-1": "oklch(0.5940 0.0443 196.0233)",
      "--chart-2": "oklch(0.7214 0.1337 49.9802)",
      "--chart-3": "oklch(0.8721 0.0864 68.5474)",
      "--chart-4": "oklch(0.6268 0 0)",
      "--chart-5": "oklch(0.6830 0 0)",
      "--sidebar": "oklch(0.1822 0 0)",
      "--sidebar-foreground": "oklch(0.8109 0 0)",
      "--sidebar-primary": "oklch(0.7214 0.1337 49.9802)",
      "--sidebar-primary-foreground": "oklch(0.1797 0.0043 308.1928)",
      "--sidebar-accent": "oklch(0.3211 0 0)",
      "--sidebar-accent-foreground": "oklch(0.8109 0 0)",
      "--sidebar-border": "oklch(0.2520 0 0)",
      "--sidebar-ring": "oklch(0.7214 0.1337 49.9802)"
    }
  },
  {
    name: "Default",
    value: "default",
    description: "Clean default theme with system fonts",
    fontFamily: "ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif",
    fontWeight: "400",
    borderRadius: "0.625rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.1450 0 0)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.1450 0 0)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.1450 0 0)",
      "--primary": "oklch(0.2050 0 0)",
      "--primary-foreground": "oklch(0.9850 0 0)",
      "--secondary": "oklch(0.9700 0 0)",
      "--secondary-foreground": "oklch(0.2050 0 0)",
      "--muted": "oklch(0.9700 0 0)",
      "--muted-foreground": "oklch(0.5560 0 0)",
      "--accent": "oklch(0.9700 0 0)",
      "--accent-foreground": "oklch(0.2050 0 0)",
      "--destructive": "oklch(0.5770 0.2450 27.3250)",
      "--destructive-foreground": "oklch(1 0 0)",
      "--border": "oklch(0.9220 0 0)",
      "--input": "oklch(0.9220 0 0)",
      "--ring": "oklch(0.7080 0 0)",
      "--chart-1": "oklch(0.8100 0.1000 252)",
      "--chart-2": "oklch(0.6200 0.1900 260)",
      "--chart-3": "oklch(0.5500 0.2200 263)",
      "--chart-4": "oklch(0.4900 0.2200 264)",
      "--chart-5": "oklch(0.4200 0.1800 266)",
      "--sidebar": "oklch(0.9850 0 0)",
      "--sidebar-foreground": "oklch(0.1450 0 0)",
      "--sidebar-primary": "oklch(0.2050 0 0)",
      "--sidebar-primary-foreground": "oklch(0.9850 0 0)",
      "--sidebar-accent": "oklch(0.9700 0 0)",
      "--sidebar-accent-foreground": "oklch(0.2050 0 0)",
      "--sidebar-border": "oklch(0.9220 0 0)",
      "--sidebar-ring": "oklch(0.7080 0 0)",
    },
    darkColors: {
      "--background": "oklch(0.1450 0 0)",
      "--foreground": "oklch(0.9850 0 0)",
      "--card": "oklch(0.2050 0 0)",
      "--card-foreground": "oklch(0.9850 0 0)",
      "--popover": "oklch(0.2690 0 0)",
      "--popover-foreground": "oklch(0.9850 0 0)",
      "--primary": "oklch(0.9220 0 0)",
      "--primary-foreground": "oklch(0.2050 0 0)",
      "--secondary": "oklch(0.2690 0 0)",
      "--secondary-foreground": "oklch(0.9850 0 0)",
      "--muted": "oklch(0.2690 0 0)",
      "--muted-foreground": "oklch(0.7080 0 0)",
      "--accent": "oklch(0.3710 0 0)",
      "--accent-foreground": "oklch(0.9850 0 0)",
      "--destructive": "oklch(0.7040 0.1910 22.2160)",
      "--destructive-foreground": "oklch(0.9850 0 0)",
      "--border": "oklch(0.2750 0 0)",
      "--input": "oklch(0.3250 0 0)",
      "--ring": "oklch(0.5560 0 0)",
      "--chart-1": "oklch(0.8100 0.1000 252)",
      "--chart-2": "oklch(0.6200 0.1900 260)",
      "--chart-3": "oklch(0.5500 0.2200 263)",
      "--chart-4": "oklch(0.4900 0.2200 264)",
      "--chart-5": "oklch(0.4200 0.1800 266)",
      "--sidebar": "oklch(0.2050 0 0)",
      "--sidebar-foreground": "oklch(0.9850 0 0)",
      "--sidebar-primary": "oklch(0.4880 0.2430 264.3760)",
      "--sidebar-primary-foreground": "oklch(0.9850 0 0)",
      "--sidebar-accent": "oklch(0.2690 0 0)",
      "--sidebar-accent-foreground": "oklch(0.9850 0 0)",
      "--sidebar-border": "oklch(0.2750 0 0)",
      "--sidebar-ring": "oklch(0.4390 0 0)",
    }
  },
  // LOCAL CUSTOMIZATION NOTE (plebdevs.com):
  // This theme is intentionally maintained in this fork and may conflict during
  // upstream pleb.school syncs. Keep this block unless intentionally removing
  // PlebDevs branding.
  {
    name: "PlebDevs",
    value: "plebdevs",
    description: "PlebDevs brand theme based on default with a green primary accent",
    fontFamily: "ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif",
    fontWeight: "400",
    borderRadius: "0.625rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.1450 0 0)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.1450 0 0)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.1450 0 0)",
      "--primary": "#34c759",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.9650 0.005 160)",
      "--secondary-foreground": "oklch(0.2050 0 0)",
      "--muted": "oklch(0.9650 0.005 160)",
      "--muted-foreground": "oklch(0.5200 0 0)",
      "--accent": "oklch(0.9550 0.015 160)",
      "--accent-foreground": "oklch(0.2050 0 0)",
      "--destructive": "oklch(0.5770 0.2450 27.3250)",
      "--destructive-foreground": "oklch(1 0 0)",
      "--border": "oklch(0.9220 0.004 160)",
      "--input": "oklch(0.9220 0.004 160)",
      "--ring": "#34c759",
      "--chart-1": "#34c759",
      "--chart-2": "#28b54a",
      "--chart-3": "#1f963d",
      "--chart-4": "#62d980",
      "--chart-5": "#8de6a4",
      "--sidebar": "oklch(0.9800 0.005 160)",
      "--sidebar-foreground": "oklch(0.1450 0 0)",
      "--sidebar-primary": "#34c759",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9550 0.015 160)",
      "--sidebar-accent-foreground": "oklch(0.2050 0 0)",
      "--sidebar-border": "oklch(0.9220 0.004 160)",
      "--sidebar-ring": "#34c759",
    },
    darkColors: {
      "--background": "oklch(0.1450 0 0)",
      "--foreground": "oklch(0.9850 0 0)",
      "--card": "oklch(0.2050 0 0)",
      "--card-foreground": "oklch(0.9850 0 0)",
      "--popover": "oklch(0.2400 0.005 160)",
      "--popover-foreground": "oklch(0.9850 0 0)",
      "--primary": "#34c759",
      "--primary-foreground": "oklch(0.1450 0 0)",
      "--secondary": "oklch(0.2690 0.008 160)",
      "--secondary-foreground": "oklch(0.9850 0 0)",
      "--muted": "oklch(0.2690 0.008 160)",
      "--muted-foreground": "oklch(0.7080 0 0)",
      "--accent": "oklch(0.3200 0.025 160)",
      "--accent-foreground": "oklch(0.9850 0 0)",
      "--destructive": "oklch(0.7040 0.1910 22.2160)",
      "--destructive-foreground": "oklch(0.9850 0 0)",
      "--border": "oklch(0.2900 0.006 160)",
      "--input": "oklch(0.3250 0.008 160)",
      "--ring": "#34c759",
      "--chart-1": "#34c759",
      "--chart-2": "#28b54a",
      "--chart-3": "#1f963d",
      "--chart-4": "#62d980",
      "--chart-5": "#8de6a4",
      "--sidebar": "oklch(0.2050 0 0)",
      "--sidebar-foreground": "oklch(0.9850 0 0)",
      "--sidebar-primary": "#34c759",
      "--sidebar-primary-foreground": "oklch(0.1450 0 0)",
      "--sidebar-accent": "oklch(0.3200 0.025 160)",
      "--sidebar-accent-foreground": "oklch(0.9850 0 0)",
      "--sidebar-border": "oklch(0.2900 0.006 160)",
      "--sidebar-ring": "#34c759",
    }
  },
  {
    name: "Doom64",
    value: "doom64",
    description: "Dark retro gaming theme inspired by Doom with JetBrains Mono font",
    fontFamily: "JetBrains Mono, ui-monospace, monospace",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;600;700&display=swap",
    borderRadius: "0.25rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.8452 0 0)",
      "--foreground": "oklch(0.2393 0 0)",
      "--card": "oklch(0.7572 0 0)",
      "--card-foreground": "oklch(0.2393 0 0)",
      "--popover": "oklch(0.7572 0 0)",
      "--popover-foreground": "oklch(0.2393 0 0)",
      "--primary": "oklch(0.5016 0.1887 27.4816)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.4955 0.0896 126.1858)",
      "--secondary-foreground": "oklch(1.0000 0 0)",
      "--muted": "oklch(0.7826 0 0)",
      "--muted-foreground": "oklch(0.4091 0 0)",
      "--accent": "oklch(0.5880 0.0993 245.7394)",
      "--accent-foreground": "oklch(1.0000 0 0)",
      "--destructive": "oklch(0.7076 0.1975 46.4558)",
      "--destructive-foreground": "oklch(0 0 0)",
      "--border": "oklch(0.4313 0 0)",
      "--input": "oklch(0.4313 0 0)",
      "--ring": "oklch(0.5016 0.1887 27.4816)",
      "--chart-1": "oklch(0.5016 0.1887 27.4816)",
      "--chart-2": "oklch(0.4955 0.0896 126.1858)",
      "--chart-3": "oklch(0.5880 0.0993 245.7394)",
      "--chart-4": "oklch(0.7076 0.1975 46.4558)",
      "--chart-5": "oklch(0.5656 0.0431 40.4319)",
      "--sidebar": "oklch(0.7572 0 0)",
      "--sidebar-foreground": "oklch(0.2393 0 0)",
      "--sidebar-primary": "oklch(0.5016 0.1887 27.4816)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.5880 0.0993 245.7394)",
      "--sidebar-accent-foreground": "oklch(1.0000 0 0)",
      "--sidebar-border": "oklch(0.4313 0 0)",
      "--sidebar-ring": "oklch(0.5016 0.1887 27.4816)"
    },
    darkColors: {
      "--background": "oklch(0.2178 0 0)",
      "--foreground": "oklch(0.9067 0 0)",
      "--card": "oklch(0.2850 0 0)",
      "--card-foreground": "oklch(0.9067 0 0)",
      "--popover": "oklch(0.2850 0 0)",
      "--popover-foreground": "oklch(0.9067 0 0)",
      "--primary": "oklch(0.6083 0.2090 27.0276)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.6423 0.1467 133.0145)",
      "--secondary-foreground": "oklch(0 0 0)",
      "--muted": "oklch(0.2645 0 0)",
      "--muted-foreground": "oklch(0.7058 0 0)",
      "--accent": "oklch(0.7482 0.1235 244.7492)",
      "--accent-foreground": "oklch(0 0 0)",
      "--destructive": "oklch(0.7839 0.1719 68.0943)",
      "--destructive-foreground": "oklch(0 0 0)",
      "--border": "oklch(0.4091 0 0)",
      "--input": "oklch(0.4091 0 0)",
      "--ring": "oklch(0.6083 0.2090 27.0276)",
      "--chart-1": "oklch(0.6083 0.2090 27.0276)",
      "--chart-2": "oklch(0.6423 0.1467 133.0145)",
      "--chart-3": "oklch(0.7482 0.1235 244.7492)",
      "--chart-4": "oklch(0.7839 0.1719 68.0943)",
      "--chart-5": "oklch(0.6471 0.0334 40.7963)",
      "--sidebar": "oklch(0.1913 0 0)",
      "--sidebar-foreground": "oklch(0.9067 0 0)",
      "--sidebar-primary": "oklch(0.6083 0.2090 27.0276)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.7482 0.1235 244.7492)",
      "--sidebar-accent-foreground": "oklch(0 0 0)",
      "--sidebar-border": "oklch(0.4091 0 0)",
      "--sidebar-ring": "oklch(0.6083 0.2090 27.0276)"
    }
  },
  {
    name: "Elegant Luxury",
    value: "elegant-luxury",
    description: "Sophisticated luxury design with gold accents",
    fontFamily: "Playfair Display, ui-serif, Georgia, Cambria, 'Times New Roman', Times, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;500;600;700&display=swap",
    borderRadius: "0.75rem",
    style: "new-york",
    lightColors: {
      "--background": "oklch(0.99 0.01 50)",
      "--foreground": "oklch(0.1 0.02 50)",
      "--card": "oklch(0.99 0.01 50)",
      "--card-foreground": "oklch(0.1 0.02 50)",
      "--popover": "oklch(0.99 0.01 50)",
      "--popover-foreground": "oklch(0.1 0.02 50)",
      "--primary": "oklch(0.35 0.1 50)",
      "--primary-foreground": "oklch(0.99 0.01 50)",
      "--secondary": "oklch(0.94 0.02 50)",
      "--secondary-foreground": "oklch(0.35 0.1 50)",
      "--muted": "oklch(0.94 0.02 50)",
      "--muted-foreground": "oklch(0.55 0.05 50)",
      "--accent": "oklch(0.9 0.05 50)",
      "--accent-foreground": "oklch(0.35 0.1 50)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.89 0.03 50)",
      "--input": "oklch(0.89 0.03 50)",
      "--ring": "oklch(0.35 0.1 50)",
      "--chart-1": "oklch(0.6 0.15 50)",
      "--chart-2": "oklch(0.5 0.2 70)",
      "--chart-3": "oklch(0.4 0.15 30)",
      "--chart-4": "oklch(0.7 0.1 90)",
      "--chart-5": "oklch(0.8 0.05 10)",
      "--sidebar": "oklch(0.97 0.02 50)",
      "--sidebar-foreground": "oklch(0.1 0.02 50)",
      "--sidebar-primary": "oklch(0.35 0.1 50)",
      "--sidebar-primary-foreground": "oklch(0.99 0.01 50)",
      "--sidebar-accent": "oklch(0.94 0.02 50)",
      "--sidebar-accent-foreground": "oklch(0.35 0.1 50)",
      "--sidebar-border": "oklch(0.89 0.03 50)",
      "--sidebar-ring": "oklch(0.35 0.1 50)"
    },
    darkColors: {
      "--background": "oklch(0.06 0.02 50)",
      "--foreground": "oklch(0.94 0.03 50)",
      "--card": "oklch(0.09 0.03 50)",
      "--card-foreground": "oklch(0.94 0.03 50)",
      "--popover": "oklch(0.06 0.02 50)",
      "--popover-foreground": "oklch(0.94 0.03 50)",
      "--primary": "oklch(0.75 0.15 50)",
      "--primary-foreground": "oklch(0.06 0.02 50)",
      "--secondary": "oklch(0.16 0.03 50)",
      "--secondary-foreground": "oklch(0.94 0.03 50)",
      "--muted": "oklch(0.16 0.03 50)",
      "--muted-foreground": "oklch(0.65 0.08 50)",
      "--accent": "oklch(0.2 0.05 50)",
      "--accent-foreground": "oklch(0.94 0.03 50)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.24 0.03 50)",
      "--input": "oklch(0.24 0.03 50)",
      "--ring": "oklch(0.75 0.15 50)",
      "--chart-1": "oklch(0.7 0.2 50)",
      "--chart-2": "oklch(0.6 0.25 70)",
      "--chart-3": "oklch(0.5 0.2 30)",
      "--chart-4": "oklch(0.8 0.15 90)",
      "--chart-5": "oklch(0.9 0.1 10)",
      "--sidebar": "oklch(0.09 0.03 50)",
      "--sidebar-foreground": "oklch(0.94 0.03 50)",
      "--sidebar-primary": "oklch(0.75 0.15 50)",
      "--sidebar-primary-foreground": "oklch(0.06 0.02 50)",
      "--sidebar-accent": "oklch(0.16 0.03 50)",
      "--sidebar-accent-foreground": "oklch(0.94 0.03 50)",
      "--sidebar-border": "oklch(0.24 0.03 50)",
      "--sidebar-ring": "oklch(0.75 0.15 50)"
    }
  },
  {
    name: "Emerald",
    value: "emerald",
    description: "Emerald theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.75rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9849 0.0024 160.68)",
      "--foreground": "oklch(0.2742 0.0208 165.96)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2742 0.0208 165.96)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2742 0.0208 165.96)",
      "--primary": "oklch(0.765 0.177 163.22)",
      "--secondary": "oklch(0.551 0.0899 200.52)",
      "--muted": "oklch(0.9692 0.005 161.7)",
      "--muted-foreground": "oklch(0.4453 0.0279 166.55)",
      "--accent": "oklch(0.9692 0.005 161.7)",
      "--accent-foreground": "oklch(0.2742 0.0208 165.96)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9263 0.0099 162.17)",
      "--input": "oklch(0.8504 0.0147 163.22)",
      "--ring": "oklch(0.765 0.177 163.22)",
      "--chart-1": "oklch(0.765 0.177 163.22)",
      "--chart-2": "oklch(0.551 0.0899 200.52)",
      "--chart-3": "oklch(0.845 0.143 164.98)",
      "--chart-4": "oklch(0.8576 0.1202 192.38)",
      "--chart-5": "oklch(0.95 0.052 163.05)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.2742 0.0208 165.96)",
      "--sidebar-primary": "oklch(0.765 0.177 163.22)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.9849 0.0024 160.68)",
      "--sidebar-accent-foreground": "oklch(0.2742 0.0208 165.96)",
      "--sidebar-border": "oklch(0.9263 0.0099 162.17)",
      "--sidebar-ring": "oklch(0.765 0.177 163.22)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2742 0.0208 165.96)"
    },
    darkColors: {
      "--background": "oklch(0.1374 0.0152 166.46)",
      "--foreground": "oklch(0.9263 0.0099 162.17)",
      "--card": "oklch(0.2119 0.0173 166.78)",
      "--card-foreground": "oklch(0.9263 0.0099 162.17)",
      "--popover": "oklch(0.2119 0.0173 166.78)",
      "--popover-foreground": "oklch(0.9263 0.0099 162.17)",
      "--primary": "oklch(0.765 0.177 163.22)",
      "--primary-foreground": "oklch(0 0 0)",
      "--secondary": "oklch(0.551 0.0899 200.52)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.2742 0.0208 165.96)",
      "--muted-foreground": "oklch(0.8504 0.0147 163.22)",
      "--accent": "oklch(0.2742 0.0208 165.96)",
      "--accent-foreground": "oklch(0.9263 0.0099 162.17)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2742 0.0208 165.96)",
      "--input": "oklch(0.3731 0.0248 166.06)",
      "--ring": "oklch(0.765 0.177 163.22)",
      "--chart-1": "oklch(0.765 0.177 163.22)",
      "--chart-2": "oklch(0.551 0.0899 200.52)",
      "--chart-3": "oklch(0.845 0.143 164.98)",
      "--chart-4": "oklch(0.8576 0.1202 192.38)",
      "--chart-5": "oklch(0.95 0.052 163.05)",
      "--sidebar": "oklch(0.2119 0.0173 166.78)",
      "--sidebar-foreground": "oklch(0.9263 0.0099 162.17)",
      "--sidebar-primary": "oklch(0.765 0.177 163.22)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.2742 0.0208 165.96)",
      "--sidebar-accent-foreground": "oklch(0.9263 0.0099 162.17)",
      "--sidebar-border": "oklch(0.2742 0.0208 165.96)",
      "--sidebar-ring": "oklch(0.765 0.177 163.22)"
    }
  },
  {
    name: "Forest",
    value: "forest",
    description: "Forest theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9843 0.0035 287.12)",
      "--foreground": "oklch(0.2738 0.021 286.32)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2738 0.021 286.32)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2738 0.021 286.32)",
      "--primary": "oklch(0.5276 0.1072 182.22)",
      "--secondary": "oklch(0.5236 0.0505 250.18)",
      "--muted": "oklch(0.9665 0.007 286.66)",
      "--muted-foreground": "oklch(0.4418 0.0385 286.07)",
      "--accent": "oklch(0.9665 0.007 286.66)",
      "--accent-foreground": "oklch(0.2738 0.021 286.32)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9197 0.014 286.6)",
      "--input": "oklch(0.8708 0.021 286.57)",
      "--ring": "oklch(0.5276 0.1072 182.22)",
      "--chart-1": "oklch(0.5276 0.1072 182.22)",
      "--chart-2": "oklch(0.5236 0.0505 250.18)",
      "--chart-3": "oklch(0.8522 0.1484 177.84)",
      "--chart-4": "oklch(0.8208 0.0278 242.12)",
      "--chart-5": "oklch(0.9511 0.0547 177.24)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.2738 0.021 286.32)",
      "--sidebar-primary": "oklch(0.5276 0.1072 182.22)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9843 0.0035 287.12)",
      "--sidebar-accent-foreground": "oklch(0.2738 0.021 286.32)",
      "--sidebar-border": "oklch(0.9197 0.014 286.6)",
      "--sidebar-ring": "oklch(0.5276 0.1072 182.22)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2738 0.021 286.32)"
    },
    darkColors: {
      "--background": "oklch(0.1408 0.0105 286.11)",
      "--foreground": "oklch(0.9197 0.014 286.6)",
      "--card": "oklch(0.2098 0.014 286.17)",
      "--card-foreground": "oklch(0.9197 0.014 286.6)",
      "--popover": "oklch(0.2098 0.014 286.17)",
      "--popover-foreground": "oklch(0.9197 0.014 286.6)",
      "--primary": "oklch(0.5276 0.1072 182.22)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.5236 0.0505 250.18)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.2738 0.021 286.32)",
      "--muted-foreground": "oklch(0.8708 0.021 286.57)",
      "--accent": "oklch(0.2738 0.021 286.32)",
      "--accent-foreground": "oklch(0.9197 0.014 286.6)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2738 0.021 286.32)",
      "--input": "oklch(0.3698 0.0315 286.09)",
      "--ring": "oklch(0.5276 0.1072 182.22)",
      "--chart-1": "oklch(0.5276 0.1072 182.22)",
      "--chart-2": "oklch(0.5236 0.0505 250.18)",
      "--chart-3": "oklch(0.8522 0.1484 177.84)",
      "--chart-4": "oklch(0.8208 0.0278 242.12)",
      "--chart-5": "oklch(0.9511 0.0547 177.24)",
      "--sidebar": "oklch(0.2098 0.014 286.17)",
      "--sidebar-foreground": "oklch(0.9197 0.014 286.6)",
      "--sidebar-primary": "oklch(0.5276 0.1072 182.22)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.2738 0.021 286.32)",
      "--sidebar-accent-foreground": "oklch(0.9197 0.014 286.6)",
      "--sidebar-border": "oklch(0.2738 0.021 286.32)",
      "--sidebar-ring": "oklch(0.5276 0.1072 182.22)"
    }
  },
  {
    name: "Graphite",
    value: "graphite",
    description: "Professional monochrome graphite theme with IBM Plex Sans font",
    fontFamily: "IBM Plex Sans, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@400;500;600;700&display=swap",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9551 0 0)",
      "--foreground": "oklch(0.3211 0 0)",
      "--card": "oklch(0.9702 0 0)",
      "--card-foreground": "oklch(0.3211 0 0)",
      "--popover": "oklch(0.9702 0 0)",
      "--popover-foreground": "oklch(0.3211 0 0)",
      "--primary": "oklch(0.4891 0 0)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.9067 0 0)",
      "--secondary-foreground": "oklch(0.3211 0 0)",
      "--muted": "oklch(0.8853 0 0)",
      "--muted-foreground": "oklch(0.5103 0 0)",
      "--accent": "oklch(0.8078 0 0)",
      "--accent-foreground": "oklch(0.3211 0 0)",
      "--destructive": "oklch(0.5594 0.1900 25.8625)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.8576 0 0)",
      "--input": "oklch(0.9067 0 0)",
      "--ring": "oklch(0.4891 0 0)",
      "--chart-1": "oklch(0.4891 0 0)",
      "--chart-2": "oklch(0.4863 0.0361 196.0278)",
      "--chart-3": "oklch(0.6534 0 0)",
      "--chart-4": "oklch(0.7316 0 0)",
      "--chart-5": "oklch(0.8078 0 0)",
      "--sidebar": "oklch(0.9370 0 0)",
      "--sidebar-foreground": "oklch(0.3211 0 0)",
      "--sidebar-primary": "oklch(0.4891 0 0)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.8078 0 0)",
      "--sidebar-accent-foreground": "oklch(0.3211 0 0)",
      "--sidebar-border": "oklch(0.8576 0 0)",
      "--sidebar-ring": "oklch(0.4891 0 0)"
    },
    darkColors: {
      "--background": "oklch(0.2178 0 0)",
      "--foreground": "oklch(0.8853 0 0)",
      "--card": "oklch(0.2435 0 0)",
      "--card-foreground": "oklch(0.8853 0 0)",
      "--popover": "oklch(0.2435 0 0)",
      "--popover-foreground": "oklch(0.8853 0 0)",
      "--primary": "oklch(0.7058 0 0)",
      "--primary-foreground": "oklch(0.2178 0 0)",
      "--secondary": "oklch(0.3092 0 0)",
      "--secondary-foreground": "oklch(0.8853 0 0)",
      "--muted": "oklch(0.2850 0 0)",
      "--muted-foreground": "oklch(0.5999 0 0)",
      "--accent": "oklch(0.3715 0 0)",
      "--accent-foreground": "oklch(0.8853 0 0)",
      "--destructive": "oklch(0.6591 0.1530 22.1703)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.3290 0 0)",
      "--input": "oklch(0.3092 0 0)",
      "--ring": "oklch(0.7058 0 0)",
      "--chart-1": "oklch(0.7058 0 0)",
      "--chart-2": "oklch(0.6714 0.0339 206.3482)",
      "--chart-3": "oklch(0.5452 0 0)",
      "--chart-4": "oklch(0.4604 0 0)",
      "--chart-5": "oklch(0.3715 0 0)",
      "--sidebar": "oklch(0.2393 0 0)",
      "--sidebar-foreground": "oklch(0.8853 0 0)",
      "--sidebar-primary": "oklch(0.7058 0 0)",
      "--sidebar-primary-foreground": "oklch(0.2178 0 0)",
      "--sidebar-accent": "oklch(0.3715 0 0)",
      "--sidebar-accent-foreground": "oklch(0.8853 0 0)",
      "--sidebar-border": "oklch(0.3290 0 0)",
      "--sidebar-ring": "oklch(0.7058 0 0)"
    }
  },
  {
    name: "Green",
    value: "green",
    description: "Natural green theme with Source Sans Pro",
    fontFamily: "Source Sans Pro, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Source+Sans+Pro:wght@400;600;700&display=swap",
    borderRadius: "0.65rem",
    style: "new-york",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.141 0.005 285.823)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.141 0.005 285.823)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.141 0.005 285.823)",
      "--primary": "oklch(0.723 0.219 149.579)",
      "--primary-foreground": "oklch(0.982 0.018 155.826)",
      "--secondary": "oklch(0.967 0.001 286.375)",
      "--secondary-foreground": "oklch(0.21 0.006 285.885)",
      "--muted": "oklch(0.967 0.001 286.375)",
      "--muted-foreground": "oklch(0.552 0.016 285.938)",
      "--accent": "oklch(0.967 0.001 286.375)",
      "--accent-foreground": "oklch(0.21 0.006 285.885)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.92 0.004 286.32)",
      "--input": "oklch(0.92 0.004 286.32)",
      "--ring": "oklch(0.723 0.219 149.579)",
      "--chart-1": "oklch(0.646 0.222 41.116)",
      "--chart-2": "oklch(0.6 0.118 184.704)",
      "--chart-3": "oklch(0.398 0.07 227.392)",
      "--chart-4": "oklch(0.828 0.189 84.429)",
      "--chart-5": "oklch(0.769 0.188 70.08)",
      "--sidebar": "oklch(0.985 0 0)",
      "--sidebar-foreground": "oklch(0.141 0.005 285.823)",
      "--sidebar-primary": "oklch(0.723 0.219 149.579)",
      "--sidebar-primary-foreground": "oklch(0.982 0.018 155.826)",
      "--sidebar-accent": "oklch(0.967 0.001 286.375)",
      "--sidebar-accent-foreground": "oklch(0.21 0.006 285.885)",
      "--sidebar-border": "oklch(0.92 0.004 286.32)",
      "--sidebar-ring": "oklch(0.723 0.219 149.579)",
    },
    darkColors: {
      "--background": "oklch(0.141 0.005 285.823)",
      "--foreground": "oklch(0.985 0 0)",
      "--card": "oklch(0.21 0.006 285.885)",
      "--card-foreground": "oklch(0.985 0 0)",
      "--popover": "oklch(0.21 0.006 285.885)",
      "--popover-foreground": "oklch(0.985 0 0)",
      "--primary": "oklch(0.615 0.154 125.5)",
      "--primary-foreground": "oklch(0.988 0.026 108.4)",
      "--secondary": "oklch(0.274 0.006 286.033)",
      "--secondary-foreground": "oklch(0.985 0 0)",
      "--muted": "oklch(0.274 0.006 286.033)",
      "--muted-foreground": "oklch(0.705 0.015 286.067)",
      "--accent": "oklch(0.274 0.006 286.033)",
      "--accent-foreground": "oklch(0.985 0 0)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(1 0 0 / 10%)",
      "--input": "oklch(1 0 0 / 15%)",
      "--ring": "oklch(0.527 0.154 150.069)",
      "--chart-1": "oklch(0.488 0.243 264.376)",
      "--chart-2": "oklch(0.696 0.17 162.48)",
      "--chart-3": "oklch(0.769 0.188 70.08)",
      "--chart-4": "oklch(0.627 0.265 303.9)",
      "--chart-5": "oklch(0.645 0.246 16.439)",
      "--sidebar": "oklch(0.21 0.006 285.885)",
      "--sidebar-foreground": "oklch(0.985 0 0)",
      "--sidebar-primary": "oklch(0.615 0.154 125.5)",
      "--sidebar-primary-foreground": "oklch(0.988 0.026 108.4)",
      "--sidebar-accent": "oklch(0.274 0.006 286.033)",
      "--sidebar-accent-foreground": "oklch(0.985 0 0)",
      "--sidebar-border": "oklch(1 0 0 / 10%)",
      "--sidebar-ring": "oklch(0.527 0.154 150.069)",
    }
  },
  {
    name: "Miami",
    value: "miami",
    description: "Miami theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.2638 0.0399 288.01)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2638 0.0399 288.01)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2638 0.0399 288.01)",
      "--primary": "oklch(0.5787 0.1955 343.4)",
      "--secondary": "oklch(0.5051 0.0901 224.61)",
      "--muted": "oklch(0.9869 0.0042 280.57)",
      "--muted-foreground": "oklch(0.4463 0.0483 286.39)",
      "--accent": "oklch(0.9869 0.0042 280.57)",
      "--accent-foreground": "oklch(0.2638 0.0399 288.01)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9309 0.0168 282.31)",
      "--input": "oklch(0.8708 0.0252 283.4)",
      "--ring": "oklch(0.5787 0.1955 343.4)",
      "--chart-1": "oklch(0.5787 0.1955 343.4)",
      "--chart-2": "oklch(0.5051 0.0901 224.61)",
      "--chart-3": "oklch(0.8269 0.0952 337.34)",
      "--chart-4": "oklch(0.8628 0.1057 208.83)",
      "--chart-5": "oklch(0.9501 0.0231 334)",
      "--sidebar": "oklch(0.9703 0.0084 281.74)",
      "--sidebar-foreground": "oklch(0.2638 0.0399 288.01)",
      "--sidebar-primary": "oklch(0.5787 0.1955 343.4)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9309 0.0168 282.31)",
      "--sidebar-accent-foreground": "oklch(0.2638 0.0399 288.01)",
      "--sidebar-border": "oklch(0.9309 0.0168 282.31)",
      "--sidebar-ring": "oklch(0.5787 0.1955 343.4)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2638 0.0399 288.01)"
    },
    darkColors: {
      "--background": "oklch(0.1292 0.0315 290.68)",
      "--foreground": "oklch(0.9309 0.0168 282.31)",
      "--card": "oklch(0.1292 0.0315 290.68)",
      "--card-foreground": "oklch(0.9309 0.0168 282.31)",
      "--popover": "oklch(0.1292 0.0315 290.68)",
      "--popover-foreground": "oklch(0.9309 0.0168 282.31)",
      "--primary": "oklch(0.5787 0.1955 343.4)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.5051 0.0901 224.61)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.2083 0.0347 289.9)",
      "--muted-foreground": "oklch(0.7052 0.0378 286.14)",
      "--accent": "oklch(0.2083 0.0347 289.9)",
      "--accent-foreground": "oklch(0.9309 0.0168 282.31)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2638 0.0399 288.01)",
      "--input": "oklch(0.3722 0.0441 286.47)",
      "--ring": "oklch(0.5787 0.1955 343.4)",
      "--chart-1": "oklch(0.5787 0.1955 343.4)",
      "--chart-2": "oklch(0.5051 0.0901 224.61)",
      "--chart-3": "oklch(0.8269 0.0952 337.34)",
      "--chart-4": "oklch(0.8628 0.1057 208.83)",
      "--chart-5": "oklch(0.9501 0.0231 334)",
      "--sidebar": "oklch(0.2083 0.0347 289.9)",
      "--sidebar-foreground": "oklch(0.9309 0.0168 282.31)",
      "--sidebar-primary": "oklch(0.5787 0.1955 343.4)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.2638 0.0399 288.01)",
      "--sidebar-accent-foreground": "oklch(0.9309 0.0168 282.31)",
      "--sidebar-border": "oklch(0.2638 0.0399 288.01)",
      "--sidebar-ring": "oklch(0.5787 0.1955 343.4)"
    }
  },
  {
    name: "Midnight Bloom",
    value: "midnight-bloom",
    description: "Dark elegance with blooming purple accents",
    fontFamily: "Lora, ui-serif, Georgia, Cambria, 'Times New Roman', Times, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Lora:wght@400;500;600;700&display=swap",
    borderRadius: "0.6rem",
    style: "new-york",
    lightColors: {
      "--background": "oklch(0.98 0.02 300)",
      "--foreground": "oklch(0.12 0.05 300)",
      "--card": "oklch(0.98 0.02 300)",
      "--card-foreground": "oklch(0.12 0.05 300)",
      "--popover": "oklch(0.98 0.02 300)",
      "--popover-foreground": "oklch(0.12 0.05 300)",
      "--primary": "oklch(0.4 0.15 300)",
      "--primary-foreground": "oklch(0.98 0.02 300)",
      "--secondary": "oklch(0.93 0.03 300)",
      "--secondary-foreground": "oklch(0.4 0.15 300)",
      "--muted": "oklch(0.93 0.03 300)",
      "--muted-foreground": "oklch(0.55 0.1 300)",
      "--accent": "oklch(0.88 0.05 300)",
      "--accent-foreground": "oklch(0.4 0.15 300)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.87 0.04 300)",
      "--input": "oklch(0.87 0.04 300)",
      "--ring": "oklch(0.4 0.15 300)",
      "--chart-1": "oklch(0.6 0.2 300)",
      "--chart-2": "oklch(0.5 0.25 320)",
      "--chart-3": "oklch(0.4 0.2 280)",
      "--chart-4": "oklch(0.7 0.15 340)",
      "--chart-5": "oklch(0.8 0.1 260)",
      "--sidebar": "oklch(0.96 0.03 300)",
      "--sidebar-foreground": "oklch(0.12 0.05 300)",
      "--sidebar-primary": "oklch(0.4 0.15 300)",
      "--sidebar-primary-foreground": "oklch(0.98 0.02 300)",
      "--sidebar-accent": "oklch(0.93 0.03 300)",
      "--sidebar-accent-foreground": "oklch(0.4 0.15 300)",
      "--sidebar-border": "oklch(0.87 0.04 300)",
      "--sidebar-ring": "oklch(0.4 0.15 300)"
    },
    darkColors: {
      "--background": "oklch(0.04 0.03 300)",
      "--foreground": "oklch(0.9 0.05 300)",
      "--card": "oklch(0.07 0.05 300)",
      "--card-foreground": "oklch(0.9 0.05 300)",
      "--popover": "oklch(0.04 0.03 300)",
      "--popover-foreground": "oklch(0.9 0.05 300)",
      "--primary": "oklch(0.72 0.2 300)",
      "--primary-foreground": "oklch(0.04 0.03 300)",
      "--secondary": "oklch(0.14 0.05 300)",
      "--secondary-foreground": "oklch(0.9 0.05 300)",
      "--muted": "oklch(0.14 0.05 300)",
      "--muted-foreground": "oklch(0.65 0.1 300)",
      "--accent": "oklch(0.18 0.08 300)",
      "--accent-foreground": "oklch(0.9 0.05 300)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.22 0.05 300)",
      "--input": "oklch(0.22 0.05 300)",
      "--ring": "oklch(0.72 0.2 300)",
      "--chart-1": "oklch(0.7 0.25 300)",
      "--chart-2": "oklch(0.6 0.3 320)",
      "--chart-3": "oklch(0.5 0.25 280)",
      "--chart-4": "oklch(0.8 0.2 340)",
      "--chart-5": "oklch(0.9 0.15 260)",
      "--sidebar": "oklch(0.07 0.05 300)",
      "--sidebar-foreground": "oklch(0.9 0.05 300)",
      "--sidebar-primary": "oklch(0.72 0.2 300)",
      "--sidebar-primary-foreground": "oklch(0.04 0.03 300)",
      "--sidebar-accent": "oklch(0.14 0.05 300)",
      "--sidebar-accent-foreground": "oklch(0.9 0.05 300)",
      "--sidebar-border": "oklch(0.22 0.05 300)",
      "--sidebar-ring": "oklch(0.72 0.2 300)"
    }
  },
  {
    name: "Mocha Mousse",
    value: "mocha-mousse",
    description: "Warm coffee-inspired browns and creams",
    fontFamily: "Merriweather, ui-serif, Georgia, Cambria, 'Times New Roman', Times, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Merriweather:wght@400;500;600;700&display=swap",
    borderRadius: "0.75rem",
    style: "new-york",
    lightColors: {
      "--background": "oklch(0.98 0.01 60)",
      "--foreground": "oklch(0.15 0.03 60)",
      "--card": "oklch(0.98 0.01 60)",
      "--card-foreground": "oklch(0.15 0.03 60)",
      "--popover": "oklch(0.98 0.01 60)",
      "--popover-foreground": "oklch(0.15 0.03 60)",
      "--primary": "oklch(0.35 0.08 60)",
      "--primary-foreground": "oklch(0.98 0.01 60)",
      "--secondary": "oklch(0.94 0.02 60)",
      "--secondary-foreground": "oklch(0.35 0.08 60)",
      "--muted": "oklch(0.94 0.02 60)",
      "--muted-foreground": "oklch(0.55 0.05 60)",
      "--accent": "oklch(0.9 0.04 60)",
      "--accent-foreground": "oklch(0.35 0.08 60)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.88 0.03 60)",
      "--input": "oklch(0.88 0.03 60)",
      "--ring": "oklch(0.35 0.08 60)",
      "--chart-1": "oklch(0.6 0.12 60)",
      "--chart-2": "oklch(0.5 0.15 80)",
      "--chart-3": "oklch(0.4 0.12 40)",
      "--chart-4": "oklch(0.7 0.08 100)",
      "--chart-5": "oklch(0.8 0.05 20)",
      "--sidebar": "oklch(0.96 0.02 60)",
      "--sidebar-foreground": "oklch(0.15 0.03 60)",
      "--sidebar-primary": "oklch(0.35 0.08 60)",
      "--sidebar-primary-foreground": "oklch(0.98 0.01 60)",
      "--sidebar-accent": "oklch(0.94 0.02 60)",
      "--sidebar-accent-foreground": "oklch(0.35 0.08 60)",
      "--sidebar-border": "oklch(0.88 0.03 60)",
      "--sidebar-ring": "oklch(0.35 0.08 60)"
    },
    darkColors: {
      "--background": "oklch(0.08 0.02 60)",
      "--foreground": "oklch(0.92 0.03 60)",
      "--card": "oklch(0.11 0.03 60)",
      "--card-foreground": "oklch(0.92 0.03 60)",
      "--popover": "oklch(0.08 0.02 60)",
      "--popover-foreground": "oklch(0.92 0.03 60)",
      "--primary": "oklch(0.73 0.12 60)",
      "--primary-foreground": "oklch(0.08 0.02 60)",
      "--secondary": "oklch(0.17 0.03 60)",
      "--secondary-foreground": "oklch(0.92 0.03 60)",
      "--muted": "oklch(0.17 0.03 60)",
      "--muted-foreground": "oklch(0.63 0.08 60)",
      "--accent": "oklch(0.21 0.05 60)",
      "--accent-foreground": "oklch(0.92 0.03 60)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.25 0.03 60)",
      "--input": "oklch(0.25 0.03 60)",
      "--ring": "oklch(0.73 0.12 60)",
      "--chart-1": "oklch(0.7 0.15 60)",
      "--chart-2": "oklch(0.6 0.18 80)",
      "--chart-3": "oklch(0.5 0.15 40)",
      "--chart-4": "oklch(0.8 0.12 100)",
      "--chart-5": "oklch(0.9 0.08 20)",
      "--sidebar": "oklch(0.11 0.03 60)",
      "--sidebar-foreground": "oklch(0.92 0.03 60)",
      "--sidebar-primary": "oklch(0.73 0.12 60)",
      "--sidebar-primary-foreground": "oklch(0.08 0.02 60)",
      "--sidebar-accent": "oklch(0.17 0.03 60)",
      "--sidebar-accent-foreground": "oklch(0.92 0.03 60)",
      "--sidebar-border": "oklch(0.25 0.03 60)",
      "--sidebar-ring": "oklch(0.73 0.12 60)"
    }
  },
  {
    name: "Modern Minimal",
    value: "modern-minimal",
    description: "Ultra-clean minimal design with subtle accents",
    fontFamily: "Poppins, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&display=swap",
    borderRadius: "0.375rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.12 0 0)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.12 0 0)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.12 0 0)",
      "--primary": "oklch(0.2 0 0)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.97 0 0)",
      "--secondary-foreground": "oklch(0.2 0 0)",
      "--muted": "oklch(0.97 0 0)",
      "--muted-foreground": "oklch(0.5 0 0)",
      "--accent": "oklch(0.97 0 0)",
      "--accent-foreground": "oklch(0.2 0 0)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.93 0 0)",
      "--input": "oklch(0.93 0 0)",
      "--ring": "oklch(0.2 0 0)",
      "--chart-1": "oklch(0.6 0.02 200)",
      "--chart-2": "oklch(0.5 0.02 220)",
      "--chart-3": "oklch(0.4 0.02 240)",
      "--chart-4": "oklch(0.7 0.02 180)",
      "--chart-5": "oklch(0.8 0.02 160)",
      "--sidebar": "oklch(0.99 0 0)",
      "--sidebar-foreground": "oklch(0.12 0 0)",
      "--sidebar-primary": "oklch(0.2 0 0)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.97 0 0)",
      "--sidebar-accent-foreground": "oklch(0.2 0 0)",
      "--sidebar-border": "oklch(0.93 0 0)",
      "--sidebar-ring": "oklch(0.2 0 0)"
    },
    darkColors: {
      "--background": "oklch(0.04 0 0)",
      "--foreground": "oklch(0.96 0 0)",
      "--card": "oklch(0.06 0 0)",
      "--card-foreground": "oklch(0.96 0 0)",
      "--popover": "oklch(0.04 0 0)",
      "--popover-foreground": "oklch(0.96 0 0)",
      "--primary": "oklch(0.85 0 0)",
      "--primary-foreground": "oklch(0.04 0 0)",
      "--secondary": "oklch(0.12 0 0)",
      "--secondary-foreground": "oklch(0.96 0 0)",
      "--muted": "oklch(0.12 0 0)",
      "--muted-foreground": "oklch(0.6 0 0)",
      "--accent": "oklch(0.12 0 0)",
      "--accent-foreground": "oklch(0.96 0 0)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.18 0 0)",
      "--input": "oklch(0.18 0 0)",
      "--ring": "oklch(0.85 0 0)",
      "--chart-1": "oklch(0.7 0.02 200)",
      "--chart-2": "oklch(0.6 0.02 220)",
      "--chart-3": "oklch(0.5 0.02 240)",
      "--chart-4": "oklch(0.8 0.02 180)",
      "--chart-5": "oklch(0.9 0.02 160)",
      "--sidebar": "oklch(0.06 0 0)",
      "--sidebar-foreground": "oklch(0.96 0 0)",
      "--sidebar-primary": "oklch(0.85 0 0)",
      "--sidebar-primary-foreground": "oklch(0.04 0 0)",
      "--sidebar-accent": "oklch(0.12 0 0)",
      "--sidebar-accent-foreground": "oklch(0.96 0 0)",
      "--sidebar-border": "oklch(0.18 0 0)",
      "--sidebar-ring": "oklch(0.85 0 0)"
    }
  },
  {
    name: "Mono",
    value: "mono",
    description: "Pure monochrome theme with system fonts",
    fontFamily: "ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace",
    fontWeight: "400",
    borderRadius: "0rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1.0000 0 0)",
      "--foreground": "oklch(0.1448 0 0)",
      "--card": "oklch(1.0000 0 0)",
      "--card-foreground": "oklch(0.1448 0 0)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.1448 0 0)",
      "--primary": "oklch(0.5555 0 0)",
      "--primary-foreground": "oklch(0.9851 0 0)",
      "--secondary": "oklch(0.9702 0 0)",
      "--secondary-foreground": "oklch(0.2046 0 0)",
      "--muted": "oklch(0.9702 0 0)",
      "--muted-foreground": "oklch(0.5486 0 0)",
      "--accent": "oklch(0.9702 0 0)",
      "--accent-foreground": "oklch(0.2046 0 0)",
      "--destructive": "oklch(0.5830 0.2387 28.4765)",
      "--destructive-foreground": "oklch(0.9702 0 0)",
      "--border": "oklch(0.9219 0 0)",
      "--input": "oklch(0.9219 0 0)",
      "--ring": "oklch(0.7090 0 0)",
      "--chart-1": "oklch(0.5555 0 0)",
      "--chart-2": "oklch(0.5555 0 0)",
      "--chart-3": "oklch(0.5555 0 0)",
      "--chart-4": "oklch(0.5555 0 0)",
      "--chart-5": "oklch(0.5555 0 0)",
      "--sidebar": "oklch(0.9851 0 0)",
      "--sidebar-foreground": "oklch(0.1448 0 0)",
      "--sidebar-primary": "oklch(0.2046 0 0)",
      "--sidebar-primary-foreground": "oklch(0.9851 0 0)",
      "--sidebar-accent": "oklch(0.9702 0 0)",
      "--sidebar-accent-foreground": "oklch(0.2046 0 0)",
      "--sidebar-border": "oklch(0.9219 0 0)",
      "--sidebar-ring": "oklch(0.7090 0 0)"
    },
    darkColors: {
      "--background": "oklch(0.1448 0 0)",
      "--foreground": "oklch(0.9851 0 0)",
      "--card": "oklch(0.2134 0 0)",
      "--card-foreground": "oklch(0.9851 0 0)",
      "--popover": "oklch(0.2686 0 0)",
      "--popover-foreground": "oklch(0.9851 0 0)",
      "--primary": "oklch(0.5555 0 0)",
      "--primary-foreground": "oklch(0.9851 0 0)",
      "--secondary": "oklch(0.2686 0 0)",
      "--secondary-foreground": "oklch(0.9851 0 0)",
      "--muted": "oklch(0.2686 0 0)",
      "--muted-foreground": "oklch(0.7090 0 0)",
      "--accent": "oklch(0.3715 0 0)",
      "--accent-foreground": "oklch(0.9851 0 0)",
      "--destructive": "oklch(0.7022 0.1892 22.2279)",
      "--destructive-foreground": "oklch(0.2686 0 0)",
      "--border": "oklch(0.3407 0 0)",
      "--input": "oklch(0.4386 0 0)",
      "--ring": "oklch(0.5555 0 0)",
      "--chart-1": "oklch(0.5555 0 0)",
      "--chart-2": "oklch(0.5555 0 0)",
      "--chart-3": "oklch(0.5555 0 0)",
      "--chart-4": "oklch(0.5555 0 0)",
      "--chart-5": "oklch(0.5555 0 0)",
      "--sidebar": "oklch(0.2046 0 0)",
      "--sidebar-foreground": "oklch(0.9851 0 0)",
      "--sidebar-primary": "oklch(0.9851 0 0)",
      "--sidebar-primary-foreground": "oklch(0.2046 0 0)",
      "--sidebar-accent": "oklch(0.2686 0 0)",
      "--sidebar-accent-foreground": "oklch(0.9851 0 0)",
      "--sidebar-border": "oklch(0.3407 0 0)",
      "--sidebar-ring": "oklch(0.5555 0 0)"
    }
  },
  {
    name: "Nature",
    value: "nature",
    description: "Organic green nature theme with Nunito font",
    fontFamily: "Nunito, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Nunito:wght@400;500;600;700&display=swap",
    borderRadius: "1rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9711 0.0074 80.7211)",
      "--foreground": "oklch(0.3000 0.0358 30.2042)",
      "--card": "oklch(0.9711 0.0074 80.7211)",
      "--card-foreground": "oklch(0.3000 0.0358 30.2042)",
      "--popover": "oklch(0.9711 0.0074 80.7211)",
      "--popover-foreground": "oklch(0.3000 0.0358 30.2042)",
      "--primary": "oklch(0.5234 0.1347 144.1672)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.9571 0.0210 147.6360)",
      "--secondary-foreground": "oklch(0.4254 0.1159 144.3078)",
      "--muted": "oklch(0.9370 0.0142 74.4218)",
      "--muted-foreground": "oklch(0.4495 0.0486 39.2110)",
      "--accent": "oklch(0.8952 0.0504 146.0366)",
      "--accent-foreground": "oklch(0.4254 0.1159 144.3078)",
      "--destructive": "oklch(0.5386 0.1937 26.7249)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.8805 0.0208 74.6428)",
      "--input": "oklch(0.8805 0.0208 74.6428)",
      "--ring": "oklch(0.5234 0.1347 144.1672)",
      "--chart-1": "oklch(0.6731 0.1624 144.2083)",
      "--chart-2": "oklch(0.5752 0.1446 144.1813)",
      "--chart-3": "oklch(0.5234 0.1347 144.1672)",
      "--chart-4": "oklch(0.4254 0.1159 144.3078)",
      "--chart-5": "oklch(0.2157 0.0453 145.7256)",
      "--sidebar": "oklch(0.9370 0.0142 74.4218)",
      "--sidebar-foreground": "oklch(0.3000 0.0358 30.2042)",
      "--sidebar-primary": "oklch(0.5234 0.1347 144.1672)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.8952 0.0504 146.0366)",
      "--sidebar-accent-foreground": "oklch(0.4254 0.1159 144.3078)",
      "--sidebar-border": "oklch(0.8805 0.0208 74.6428)",
      "--sidebar-ring": "oklch(0.5234 0.1347 144.1672)"
    },
    darkColors: {
      "--background": "oklch(0.2683 0.0279 150.7681)",
      "--foreground": "oklch(0.9423 0.0097 72.6595)",
      "--card": "oklch(0.3327 0.0271 146.9867)",
      "--card-foreground": "oklch(0.9423 0.0097 72.6595)",
      "--popover": "oklch(0.3327 0.0271 146.9867)",
      "--popover-foreground": "oklch(0.9423 0.0097 72.6595)",
      "--primary": "oklch(0.6731 0.1624 144.2083)",
      "--primary-foreground": "oklch(0.2157 0.0453 145.7256)",
      "--secondary": "oklch(0.3942 0.0265 142.9926)",
      "--secondary-foreground": "oklch(0.8970 0.0166 142.5518)",
      "--muted": "oklch(0.3327 0.0271 146.9867)",
      "--muted-foreground": "oklch(0.8579 0.0174 76.0955)",
      "--accent": "oklch(0.5752 0.1446 144.1813)",
      "--accent-foreground": "oklch(0.9423 0.0097 72.6595)",
      "--destructive": "oklch(0.5386 0.1937 26.7249)",
      "--destructive-foreground": "oklch(0.9423 0.0097 72.6595)",
      "--border": "oklch(0.3942 0.0265 142.9926)",
      "--input": "oklch(0.3942 0.0265 142.9926)",
      "--ring": "oklch(0.6731 0.1624 144.2083)",
      "--chart-1": "oklch(0.7660 0.1179 145.2950)",
      "--chart-2": "oklch(0.7185 0.1417 144.8887)",
      "--chart-3": "oklch(0.6731 0.1624 144.2083)",
      "--chart-4": "oklch(0.6291 0.1543 144.2031)",
      "--chart-5": "oklch(0.5752 0.1446 144.1813)",
      "--sidebar": "oklch(0.2683 0.0279 150.7681)",
      "--sidebar-foreground": "oklch(0.9423 0.0097 72.6595)",
      "--sidebar-primary": "oklch(0.6731 0.1624 144.2083)",
      "--sidebar-primary-foreground": "oklch(0.2157 0.0453 145.7256)",
      "--sidebar-accent": "oklch(0.5752 0.1446 144.1813)",
      "--sidebar-accent-foreground": "oklch(0.9423 0.0097 72.6595)",
      "--sidebar-border": "oklch(0.3942 0.0265 142.9926)",
      "--sidebar-ring": "oklch(0.6731 0.1624 144.2083)"
    }
  },
  {
    name: "Neo Brutalism",
    value: "neo-brutalism",
    description: "Bold brutalist design with stark contrasts",
    fontFamily: "Space Grotesk, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&display=swap",
    borderRadius: "0rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0 0 0)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0 0 0)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0 0 0)",
      "--primary": "oklch(0 0 0)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.9 0 0)",
      "--secondary-foreground": "oklch(0 0 0)",
      "--muted": "oklch(0.9 0 0)",
      "--muted-foreground": "oklch(0.4 0 0)",
      "--accent": "oklch(0.9 0 0)",
      "--accent-foreground": "oklch(0 0 0)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0 0 0)",
      "--input": "oklch(0 0 0)",
      "--ring": "oklch(0 0 0)",
      "--chart-1": "oklch(0.8 0.3 0)",
      "--chart-2": "oklch(0.8 0.3 90)",
      "--chart-3": "oklch(0.8 0.3 180)",
      "--chart-4": "oklch(0.8 0.3 270)",
      "--chart-5": "oklch(0.8 0.3 45)",
      "--sidebar": "oklch(0.95 0 0)",
      "--sidebar-foreground": "oklch(0 0 0)",
      "--sidebar-primary": "oklch(0 0 0)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9 0 0)",
      "--sidebar-accent-foreground": "oklch(0 0 0)",
      "--sidebar-border": "oklch(0 0 0)",
      "--sidebar-ring": "oklch(0 0 0)"
    },
    darkColors: {
      "--background": "oklch(0 0 0)",
      "--foreground": "oklch(1 0 0)",
      "--card": "oklch(0 0 0)",
      "--card-foreground": "oklch(1 0 0)",
      "--popover": "oklch(0 0 0)",
      "--popover-foreground": "oklch(1 0 0)",
      "--primary": "oklch(1 0 0)",
      "--primary-foreground": "oklch(0 0 0)",
      "--secondary": "oklch(0.2 0 0)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.2 0 0)",
      "--muted-foreground": "oklch(0.7 0 0)",
      "--accent": "oklch(0.2 0 0)",
      "--accent-foreground": "oklch(1 0 0)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(1 0 0)",
      "--input": "oklch(1 0 0)",
      "--ring": "oklch(1 0 0)",
      "--chart-1": "oklch(0.6 0.3 0)",
      "--chart-2": "oklch(0.6 0.3 90)",
      "--chart-3": "oklch(0.6 0.3 180)",
      "--chart-4": "oklch(0.6 0.3 270)",
      "--chart-5": "oklch(0.6 0.3 45)",
      "--sidebar": "oklch(0.1 0 0)",
      "--sidebar-foreground": "oklch(1 0 0)",
      "--sidebar-primary": "oklch(1 0 0)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.2 0 0)",
      "--sidebar-accent-foreground": "oklch(1 0 0)",
      "--sidebar-border": "oklch(1 0 0)",
      "--sidebar-ring": "oklch(1 0 0)"
    }
  },
  {
    name: "Neutral",
    value: "neutral",
    description: "Clean neutral theme with Inter font",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap",
    borderRadius: "0.625rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.145 0 0)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.145 0 0)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.145 0 0)",
      "--primary": "oklch(0.205 0 0)",
      "--primary-foreground": "oklch(0.985 0 0)",
      "--secondary": "oklch(0.97 0 0)",
      "--secondary-foreground": "oklch(0.205 0 0)",
      "--muted": "oklch(0.97 0 0)",
      "--muted-foreground": "oklch(0.556 0 0)",
      "--accent": "oklch(0.97 0 0)",
      "--accent-foreground": "oklch(0.205 0 0)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--destructive-foreground": "oklch(0.985 0 0)",
      "--border": "oklch(0.922 0 0)",
      "--input": "oklch(0.922 0 0)",
      "--ring": "oklch(0.708 0 0)",
      "--chart-1": "oklch(0.646 0.222 41.116)",
      "--chart-2": "oklch(0.6 0.118 184.704)",
      "--chart-3": "oklch(0.398 0.07 227.392)",
      "--chart-4": "oklch(0.828 0.189 84.429)",
      "--chart-5": "oklch(0.769 0.188 70.08)",
      "--sidebar": "oklch(0.985 0 0)",
      "--sidebar-foreground": "oklch(0.145 0 0)",
      "--sidebar-primary": "oklch(0.205 0 0)",
      "--sidebar-primary-foreground": "oklch(0.985 0 0)",
      "--sidebar-accent": "oklch(0.97 0 0)",
      "--sidebar-accent-foreground": "oklch(0.205 0 0)",
      "--sidebar-border": "oklch(0.922 0 0)",
      "--sidebar-ring": "oklch(0.708 0 0)",
    },
    darkColors: {
      "--background": "oklch(0.145 0 0)",
      "--foreground": "oklch(0.985 0 0)",
      "--card": "oklch(0.205 0 0)",
      "--card-foreground": "oklch(0.985 0 0)",
      "--popover": "oklch(0.269 0 0)",
      "--popover-foreground": "oklch(0.985 0 0)",
      "--primary": "oklch(0.922 0 0)",
      "--primary-foreground": "oklch(0.205 0 0)",
      "--secondary": "oklch(0.269 0 0)",
      "--secondary-foreground": "oklch(0.985 0 0)",
      "--muted": "oklch(0.269 0 0)",
      "--muted-foreground": "oklch(0.708 0 0)",
      "--accent": "oklch(0.371 0 0)",
      "--accent-foreground": "oklch(0.985 0 0)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--destructive-foreground": "oklch(0.985 0 0)",
      "--border": "oklch(1 0 0 / 10%)",
      "--input": "oklch(1 0 0 / 15%)",
      "--ring": "oklch(0.556 0 0)",
      "--chart-1": "oklch(0.488 0.243 264.376)",
      "--chart-2": "oklch(0.696 0.17 162.48)",
      "--chart-3": "oklch(0.769 0.188 70.08)",
      "--chart-4": "oklch(0.627 0.265 303.9)",
      "--chart-5": "oklch(0.645 0.246 16.439)",
      "--sidebar": "oklch(0.205 0 0)",
      "--sidebar-foreground": "oklch(0.985 0 0)",
      "--sidebar-primary": "oklch(0.488 0.243 264.376)",
      "--sidebar-primary-foreground": "oklch(0.985 0 0)",
      "--sidebar-accent": "oklch(0.269 0 0)",
      "--sidebar-accent-foreground": "oklch(0.985 0 0)",
      "--sidebar-border": "oklch(1 0 0 / 10%)",
      "--sidebar-ring": "oklch(0.439 0 0)",
    }
  },
  {
    name: "Northern Lights",
    value: "northern-lights",
    description: "Aurora-inspired blues and greens",
    fontFamily: "Nunito, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Nunito:wght@400;500;600;700&display=swap",
    borderRadius: "0.8rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.98 0.01 200)",
      "--foreground": "oklch(0.1 0.05 200)",
      "--card": "oklch(0.98 0.01 200)",
      "--card-foreground": "oklch(0.1 0.05 200)",
      "--popover": "oklch(0.98 0.01 200)",
      "--popover-foreground": "oklch(0.1 0.05 200)",
      "--primary": "oklch(0.45 0.15 200)",
      "--primary-foreground": "oklch(0.98 0.01 200)",
      "--secondary": "oklch(0.93 0.02 200)",
      "--secondary-foreground": "oklch(0.45 0.15 200)",
      "--muted": "oklch(0.93 0.02 200)",
      "--muted-foreground": "oklch(0.55 0.1 200)",
      "--accent": "oklch(0.88 0.05 160)",
      "--accent-foreground": "oklch(0.45 0.15 200)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.87 0.03 200)",
      "--input": "oklch(0.87 0.03 200)",
      "--ring": "oklch(0.45 0.15 200)",
      "--chart-1": "oklch(0.6 0.2 200)",
      "--chart-2": "oklch(0.5 0.25 160)",
      "--chart-3": "oklch(0.4 0.2 180)",
      "--chart-4": "oklch(0.7 0.15 140)",
      "--chart-5": "oklch(0.8 0.1 220)",
      "--sidebar": "oklch(0.96 0.02 200)",
      "--sidebar-foreground": "oklch(0.1 0.05 200)",
      "--sidebar-primary": "oklch(0.45 0.15 200)",
      "--sidebar-primary-foreground": "oklch(0.98 0.01 200)",
      "--sidebar-accent": "oklch(0.93 0.02 200)",
      "--sidebar-accent-foreground": "oklch(0.45 0.15 200)",
      "--sidebar-border": "oklch(0.87 0.03 200)",
      "--sidebar-ring": "oklch(0.45 0.15 200)"
    },
    darkColors: {
      "--background": "oklch(0.05 0.03 200)",
      "--foreground": "oklch(0.9 0.05 200)",
      "--card": "oklch(0.08 0.05 200)",
      "--card-foreground": "oklch(0.9 0.05 200)",
      "--popover": "oklch(0.05 0.03 200)",
      "--popover-foreground": "oklch(0.9 0.05 200)",
      "--primary": "oklch(0.7 0.2 200)",
      "--primary-foreground": "oklch(0.05 0.03 200)",
      "--secondary": "oklch(0.15 0.05 200)",
      "--secondary-foreground": "oklch(0.9 0.05 200)",
      "--muted": "oklch(0.15 0.05 200)",
      "--muted-foreground": "oklch(0.65 0.1 200)",
      "--accent": "oklch(0.18 0.08 160)",
      "--accent-foreground": "oklch(0.9 0.05 200)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.25 0.05 200)",
      "--input": "oklch(0.25 0.05 200)",
      "--ring": "oklch(0.7 0.2 200)",
      "--chart-1": "oklch(0.7 0.25 200)",
      "--chart-2": "oklch(0.6 0.3 160)",
      "--chart-3": "oklch(0.5 0.25 180)",
      "--chart-4": "oklch(0.8 0.2 140)",
      "--chart-5": "oklch(0.9 0.15 220)",
      "--sidebar": "oklch(0.08 0.05 200)",
      "--sidebar-foreground": "oklch(0.9 0.05 200)",
      "--sidebar-primary": "oklch(0.7 0.2 200)",
      "--sidebar-primary-foreground": "oklch(0.05 0.03 200)",
      "--sidebar-accent": "oklch(0.15 0.05 200)",
      "--sidebar-accent-foreground": "oklch(0.9 0.05 200)",
      "--sidebar-border": "oklch(0.25 0.05 200)",
      "--sidebar-ring": "oklch(0.7 0.2 200)"
    }
  },
  {
    name: "Notebook",
    value: "notebook",
    description: "Clean paper-like notebook theme with Georgia font",
    fontFamily: "Georgia, ui-serif, serif",
    fontWeight: "400",
    borderRadius: "0.375rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9821 0 0)",
      "--foreground": "oklch(0.3485 0 0)",
      "--card": "oklch(1.0000 0 0)",
      "--card-foreground": "oklch(0.3485 0 0)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.3485 0 0)",
      "--primary": "oklch(0.4891 0 0)",
      "--primary-foreground": "oklch(0.9551 0 0)",
      "--secondary": "oklch(0.9006 0 0)",
      "--secondary-foreground": "oklch(0.3485 0 0)",
      "--muted": "oklch(0.9158 0 0)",
      "--muted-foreground": "oklch(0.4313 0 0)",
      "--accent": "oklch(0.9354 0.0456 94.8549)",
      "--accent-foreground": "oklch(0.4015 0.0436 37.9587)",
      "--destructive": "oklch(0.6627 0.0978 20.0041)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.5538 0.0025 17.2320)",
      "--input": "oklch(1.0000 0 0)",
      "--ring": "oklch(0.7058 0 0)",
      "--chart-1": "oklch(0.3211 0 0)",
      "--chart-2": "oklch(0.4495 0 0)",
      "--chart-3": "oklch(0.5693 0 0)",
      "--chart-4": "oklch(0.6830 0 0)",
      "--chart-5": "oklch(0.7921 0 0)",
      "--sidebar": "oklch(0.9551 0 0)",
      "--sidebar-foreground": "oklch(0.3485 0 0)",
      "--sidebar-primary": "oklch(0.4891 0 0)",
      "--sidebar-primary-foreground": "oklch(0.9551 0 0)",
      "--sidebar-accent": "oklch(0.9354 0.0456 94.8549)",
      "--sidebar-accent-foreground": "oklch(0.4015 0.0436 37.9587)",
      "--sidebar-border": "oklch(0.8078 0 0)",
      "--sidebar-ring": "oklch(0.7058 0 0)"
    },
    darkColors: {
      "--background": "oklch(0.2891 0 0)",
      "--foreground": "oklch(0.8945 0 0)",
      "--card": "oklch(0.3211 0 0)",
      "--card-foreground": "oklch(0.8945 0 0)",
      "--popover": "oklch(0.3211 0 0)",
      "--popover-foreground": "oklch(0.8945 0 0)",
      "--primary": "oklch(0.7572 0 0)",
      "--primary-foreground": "oklch(0.2891 0 0)",
      "--secondary": "oklch(0.4676 0 0)",
      "--secondary-foreground": "oklch(0.8078 0 0)",
      "--muted": "oklch(0.3904 0 0)",
      "--muted-foreground": "oklch(0.7058 0 0)",
      "--accent": "oklch(0.9067 0 0)",
      "--accent-foreground": "oklch(0.3211 0 0)",
      "--destructive": "oklch(0.7915 0.0491 18.2410)",
      "--destructive-foreground": "oklch(0.2891 0 0)",
      "--border": "oklch(0.4276 0 0)",
      "--input": "oklch(0.3211 0 0)",
      "--ring": "oklch(0.8078 0 0)",
      "--chart-1": "oklch(0.9521 0 0)",
      "--chart-2": "oklch(0.8576 0 0)",
      "--chart-3": "oklch(0.7572 0 0)",
      "--chart-4": "oklch(0.6534 0 0)",
      "--chart-5": "oklch(0.5452 0 0)",
      "--sidebar": "oklch(0.2478 0 0)",
      "--sidebar-foreground": "oklch(0.8945 0 0)",
      "--sidebar-primary": "oklch(0.7572 0 0)",
      "--sidebar-primary-foreground": "oklch(0.2478 0 0)",
      "--sidebar-accent": "oklch(0.9067 0 0)",
      "--sidebar-accent-foreground": "oklch(0.3211 0 0)",
      "--sidebar-border": "oklch(0.4276 0 0)",
      "--sidebar-ring": "oklch(0.8078 0 0)"
    }
  },
  {
    name: "Ocean Breeze",
    value: "ocean-breeze",
    description: "Refreshing ocean blues and aqua tones",
    fontFamily: "Open Sans, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;500;600;700&display=swap",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.99 0.01 190)",
      "--foreground": "oklch(0.1 0.05 190)",
      "--card": "oklch(0.99 0.01 190)",
      "--card-foreground": "oklch(0.1 0.05 190)",
      "--popover": "oklch(0.99 0.01 190)",
      "--popover-foreground": "oklch(0.1 0.05 190)",
      "--primary": "oklch(0.5 0.18 190)",
      "--primary-foreground": "oklch(0.99 0.01 190)",
      "--secondary": "oklch(0.94 0.02 190)",
      "--secondary-foreground": "oklch(0.5 0.18 190)",
      "--muted": "oklch(0.94 0.02 190)",
      "--muted-foreground": "oklch(0.55 0.12 190)",
      "--accent": "oklch(0.9 0.05 170)",
      "--accent-foreground": "oklch(0.5 0.18 190)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.88 0.03 190)",
      "--input": "oklch(0.88 0.03 190)",
      "--ring": "oklch(0.5 0.18 190)",
      "--chart-1": "oklch(0.6 0.2 190)",
      "--chart-2": "oklch(0.5 0.25 170)",
      "--chart-3": "oklch(0.4 0.2 210)",
      "--chart-4": "oklch(0.7 0.15 150)",
      "--chart-5": "oklch(0.8 0.1 230)",
      "--sidebar": "oklch(0.97 0.02 190)",
      "--sidebar-foreground": "oklch(0.1 0.05 190)",
      "--sidebar-primary": "oklch(0.5 0.18 190)",
      "--sidebar-primary-foreground": "oklch(0.99 0.01 190)",
      "--sidebar-accent": "oklch(0.94 0.02 190)",
      "--sidebar-accent-foreground": "oklch(0.5 0.18 190)",
      "--sidebar-border": "oklch(0.88 0.03 190)",
      "--sidebar-ring": "oklch(0.5 0.18 190)"
    },
    darkColors: {
      "--background": "oklch(0.06 0.03 190)",
      "--foreground": "oklch(0.9 0.05 190)",
      "--card": "oklch(0.09 0.05 190)",
      "--card-foreground": "oklch(0.9 0.05 190)",
      "--popover": "oklch(0.06 0.03 190)",
      "--popover-foreground": "oklch(0.9 0.05 190)",
      "--primary": "oklch(0.68 0.22 190)",
      "--primary-foreground": "oklch(0.06 0.03 190)",
      "--secondary": "oklch(0.16 0.05 190)",
      "--secondary-foreground": "oklch(0.9 0.05 190)",
      "--muted": "oklch(0.16 0.05 190)",
      "--muted-foreground": "oklch(0.65 0.12 190)",
      "--accent": "oklch(0.19 0.08 170)",
      "--accent-foreground": "oklch(0.9 0.05 190)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.26 0.05 190)",
      "--input": "oklch(0.26 0.05 190)",
      "--ring": "oklch(0.68 0.22 190)",
      "--chart-1": "oklch(0.7 0.25 190)",
      "--chart-2": "oklch(0.6 0.3 170)",
      "--chart-3": "oklch(0.5 0.25 210)",
      "--chart-4": "oklch(0.8 0.2 150)",
      "--chart-5": "oklch(0.9 0.15 230)",
      "--sidebar": "oklch(0.09 0.05 190)",
      "--sidebar-foreground": "oklch(0.9 0.05 190)",
      "--sidebar-primary": "oklch(0.68 0.22 190)",
      "--sidebar-primary-foreground": "oklch(0.06 0.03 190)",
      "--sidebar-accent": "oklch(0.16 0.05 190)",
      "--sidebar-accent-foreground": "oklch(0.9 0.05 190)",
      "--sidebar-border": "oklch(0.26 0.05 190)",
      "--sidebar-ring": "oklch(0.68 0.22 190)"
    }
  },
  {
    name: "Orange",
    value: "orange",
    description: "Vibrant orange theme with Poppins",
    fontFamily: "Poppins, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&display=swap",
    borderRadius: "0.65rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.141 0.005 285.823)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.141 0.005 285.823)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.141 0.005 285.823)",
      "--primary": "oklch(0.705 0.213 47.604)",
      "--primary-foreground": "oklch(0.98 0.016 73.684)",
      "--secondary": "oklch(0.967 0.001 286.375)",
      "--secondary-foreground": "oklch(0.21 0.006 285.885)",
      "--muted": "oklch(0.967 0.001 286.375)",
      "--muted-foreground": "oklch(0.552 0.016 285.938)",
      "--accent": "oklch(0.967 0.001 286.375)",
      "--accent-foreground": "oklch(0.21 0.006 285.885)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.92 0.004 286.32)",
      "--input": "oklch(0.92 0.004 286.32)",
      "--ring": "oklch(0.705 0.213 47.604)",
      "--chart-1": "oklch(0.646 0.222 41.116)",
      "--chart-2": "oklch(0.6 0.118 184.704)",
      "--chart-3": "oklch(0.398 0.07 227.392)",
      "--chart-4": "oklch(0.828 0.189 84.429)",
      "--chart-5": "oklch(0.769 0.188 70.08)",
      "--sidebar": "oklch(0.985 0 0)",
      "--sidebar-foreground": "oklch(0.141 0.005 285.823)",
      "--sidebar-primary": "oklch(0.705 0.213 47.604)",
      "--sidebar-primary-foreground": "oklch(0.98 0.016 73.684)",
      "--sidebar-accent": "oklch(0.967 0.001 286.375)",
      "--sidebar-accent-foreground": "oklch(0.21 0.006 285.885)",
      "--sidebar-border": "oklch(0.92 0.004 286.32)",
      "--sidebar-ring": "oklch(0.705 0.213 47.604)",
    },
    darkColors: {
      "--background": "oklch(0.141 0.005 285.823)",
      "--foreground": "oklch(0.985 0 0)",
      "--card": "oklch(0.21 0.006 285.885)",
      "--card-foreground": "oklch(0.985 0 0)",
      "--popover": "oklch(0.21 0.006 285.885)",
      "--popover-foreground": "oklch(0.985 0 0)",
      "--primary": "oklch(0.646 0.222 41.116)",
      "--primary-foreground": "oklch(0.98 0.016 73.684)",
      "--secondary": "oklch(0.274 0.006 286.033)",
      "--secondary-foreground": "oklch(0.985 0 0)",
      "--muted": "oklch(0.274 0.006 286.033)",
      "--muted-foreground": "oklch(0.705 0.015 286.067)",
      "--accent": "oklch(0.274 0.006 286.033)",
      "--accent-foreground": "oklch(0.985 0 0)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(1 0 0 / 10%)",
      "--input": "oklch(1 0 0 / 15%)",
      "--ring": "oklch(0.646 0.222 41.116)",
      "--chart-1": "oklch(0.488 0.243 264.376)",
      "--chart-2": "oklch(0.696 0.17 162.48)",
      "--chart-3": "oklch(0.769 0.188 70.08)",
      "--chart-4": "oklch(0.627 0.265 303.9)",
      "--chart-5": "oklch(0.645 0.246 16.439)",
      "--sidebar": "oklch(0.21 0.006 285.885)",
      "--sidebar-foreground": "oklch(0.985 0 0)",
      "--sidebar-primary": "oklch(0.646 0.222 41.116)",
      "--sidebar-primary-foreground": "oklch(0.98 0.016 73.684)",
      "--sidebar-accent": "oklch(0.274 0.006 286.033)",
      "--sidebar-accent-foreground": "oklch(0.985 0 0)",
      "--sidebar-border": "oklch(1 0 0 / 10%)",
      "--sidebar-ring": "oklch(0.646 0.222 41.116)",
    }
  },
  {
    name: "Perpetuity",
    value: "perpetuity",
    description: "Timeless blue theme with Playfair Display font",
    fontFamily: "Playfair Display, ui-serif, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;500;600;700&display=swap",
    borderRadius: "0.75rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9491 0.0085 197.0126)",
      "--foreground": "oklch(0.3772 0.0619 212.6640)",
      "--card": "oklch(0.9724 0.0053 197.0692)",
      "--card-foreground": "oklch(0.3772 0.0619 212.6640)",
      "--popover": "oklch(0.9724 0.0053 197.0692)",
      "--popover-foreground": "oklch(0.3772 0.0619 212.6640)",
      "--primary": "oklch(0.5624 0.0947 203.2755)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.9244 0.0181 196.8450)",
      "--secondary-foreground": "oklch(0.3772 0.0619 212.6640)",
      "--muted": "oklch(0.9295 0.0107 196.9723)",
      "--muted-foreground": "oklch(0.5428 0.0594 201.5662)",
      "--accent": "oklch(0.9021 0.0297 201.8915)",
      "--accent-foreground": "oklch(0.3772 0.0619 212.6640)",
      "--destructive": "oklch(0.5732 0.1901 25.5409)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.8931 0.0205 204.4136)",
      "--input": "oklch(0.9244 0.0181 196.8450)",
      "--ring": "oklch(0.5624 0.0947 203.2755)",
      "--chart-1": "oklch(0.5624 0.0947 203.2755)",
      "--chart-2": "oklch(0.6389 0.1029 201.5918)",
      "--chart-3": "oklch(0.7124 0.1075 201.2486)",
      "--chart-4": "oklch(0.7701 0.0979 201.1816)",
      "--chart-5": "oklch(0.8336 0.0771 200.9702)",
      "--sidebar": "oklch(0.9280 0.0183 205.3151)",
      "--sidebar-foreground": "oklch(0.3772 0.0619 212.6640)",
      "--sidebar-primary": "oklch(0.5624 0.0947 203.2755)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.9021 0.0297 201.8915)",
      "--sidebar-accent-foreground": "oklch(0.3772 0.0619 212.6640)",
      "--sidebar-border": "oklch(0.8931 0.0205 204.4136)",
      "--sidebar-ring": "oklch(0.5624 0.0947 203.2755)"
    },
    darkColors: {
      "--background": "oklch(0.2068 0.0247 224.4533)",
      "--foreground": "oklch(0.8520 0.1269 195.0354)",
      "--card": "oklch(0.2293 0.0276 216.0674)",
      "--card-foreground": "oklch(0.8520 0.1269 195.0354)",
      "--popover": "oklch(0.2293 0.0276 216.0674)",
      "--popover-foreground": "oklch(0.8520 0.1269 195.0354)",
      "--primary": "oklch(0.8520 0.1269 195.0354)",
      "--primary-foreground": "oklch(0.2068 0.0247 224.4533)",
      "--secondary": "oklch(0.3775 0.0564 216.5010)",
      "--secondary-foreground": "oklch(0.8520 0.1269 195.0354)",
      "--muted": "oklch(0.2894 0.0412 218.8153)",
      "--muted-foreground": "oklch(0.6611 0.0975 195.0526)",
      "--accent": "oklch(0.3775 0.0564 216.5010)",
      "--accent-foreground": "oklch(0.8520 0.1269 195.0354)",
      "--destructive": "oklch(0.6168 0.2086 25.8088)",
      "--destructive-foreground": "oklch(0.9612 0 0)",
      "--border": "oklch(0.3775 0.0564 216.5010)",
      "--input": "oklch(0.3775 0.0564 216.5010)",
      "--ring": "oklch(0.8520 0.1269 195.0354)",
      "--chart-1": "oklch(0.8520 0.1269 195.0354)",
      "--chart-2": "oklch(0.6611 0.0975 195.0526)",
      "--chart-3": "oklch(0.5804 0.0849 195.0673)",
      "--chart-4": "oklch(0.4269 0.0630 202.6247)",
      "--chart-5": "oklch(0.3142 0.0455 204.1575)",
      "--sidebar": "oklch(0.2068 0.0247 224.4533)",
      "--sidebar-foreground": "oklch(0.8520 0.1269 195.0354)",
      "--sidebar-primary": "oklch(0.8520 0.1269 195.0354)",
      "--sidebar-primary-foreground": "oklch(0.2068 0.0247 224.4533)",
      "--sidebar-accent": "oklch(0.3775 0.0564 216.5010)",
      "--sidebar-accent-foreground": "oklch(0.8520 0.1269 195.0354)",
      "--sidebar-border": "oklch(0.3775 0.0564 216.5010)",
      "--sidebar-ring": "oklch(0.8520 0.1269 195.0354)"
    }
  },
  {
    name: "Quick Pink",
    value: "quick-pink",
    description: "Quick Pink theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9889 0.0063 337.02)",
      "--foreground": "oklch(0.2707 0.0378 336.77)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2707 0.0378 336.77)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2707 0.0378 336.77)",
      "--primary": "oklch(0.5314 0.1384 355.11)",
      "--secondary": "oklch(0.9243 0.0252 336.86)",
      "--muted": "oklch(0.9725 0.0126 336.87)",
      "--muted-foreground": "oklch(0.4405 0.0479 336.69)",
      "--accent": "oklch(0.9725 0.0126 336.87)",
      "--accent-foreground": "oklch(0.2707 0.0378 336.77)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9243 0.0252 336.86)",
      "--input": "oklch(0.8726 0.0378 336.85)",
      "--ring": "oklch(0.5314 0.1384 355.11)",
      "--chart-1": "oklch(0.5314 0.1384 355.11)",
      "--chart-2": "oklch(0.8988 0.0381 341.91)",
      "--chart-3": "oklch(0.719 0.1257 347.27)",
      "--chart-4": "oklch(0.8233 0.0748 344.64)",
      "--chart-5": "oklch(0.9477 0.0176 340.95)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.2707 0.0378 336.77)",
      "--sidebar-primary": "oklch(0.5314 0.1384 355.11)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9889 0.0063 337.02)",
      "--sidebar-accent-foreground": "oklch(0.2707 0.0378 336.77)",
      "--sidebar-border": "oklch(0.9243 0.0252 336.86)",
      "--sidebar-ring": "oklch(0.5314 0.1384 355.11)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2707 0.0378 336.77)"
    },
    darkColors: {
      "--background": "oklch(0.1438 0.0189 336.7)",
      "--foreground": "oklch(0.9243 0.0252 336.86)",
      "--card": "oklch(0.1929 0.0252 336.72)",
      "--card-foreground": "oklch(0.9243 0.0252 336.86)",
      "--popover": "oklch(0.1929 0.0252 336.72)",
      "--popover-foreground": "oklch(0.9243 0.0252 336.86)",
      "--primary": "oklch(0.5314 0.1384 355.11)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.371 0.0441 336.69)",
      "--secondary-foreground": "oklch(0.9889 0.0063 337.02)",
      "--muted": "oklch(0.2707 0.0378 336.77)",
      "--muted-foreground": "oklch(0.8726 0.0378 336.85)",
      "--accent": "oklch(0.2707 0.0378 336.77)",
      "--accent-foreground": "oklch(0.9243 0.0252 336.86)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2707 0.0378 336.77)",
      "--input": "oklch(0.371 0.0441 336.69)",
      "--ring": "oklch(0.5314 0.1384 355.11)",
      "--chart-1": "oklch(0.5314 0.1384 355.11)",
      "--chart-2": "oklch(0.8988 0.0381 341.91)",
      "--chart-3": "oklch(0.719 0.1257 347.27)",
      "--chart-4": "oklch(0.8233 0.0748 344.64)",
      "--chart-5": "oklch(0.9477 0.0176 340.95)",
      "--sidebar": "oklch(0.1929 0.0252 336.72)",
      "--sidebar-foreground": "oklch(0.9243 0.0252 336.86)",
      "--sidebar-primary": "oklch(0.5314 0.1384 355.11)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.2707 0.0378 336.77)",
      "--sidebar-accent-foreground": "oklch(0.9243 0.0252 336.86)",
      "--sidebar-border": "oklch(0.2707 0.0378 336.77)",
      "--sidebar-ring": "oklch(0.5314 0.1384 355.11)"
    }
  },
  {
    name: "Razzmatazz",
    value: "razzmatazz",
    description: "Razzmatazz theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "1rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.2686 0.0032 34.85)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2686 0.0032 34.85)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2686 0.0032 34.85)",
      "--primary": "oklch(0.5827 0.2418 12.23)",
      "--secondary": "oklch(0.9567 0.0021 34.31)",
      "--muted": "oklch(0.9851 0.001 33.69)",
      "--muted-foreground": "oklch(0.4423 0.004 36.92)",
      "--accent": "oklch(0.9851 0.001 33.69)",
      "--accent-foreground": "oklch(0.2686 0.0032 34.85)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9227 0.0029 34.49)",
      "--input": "oklch(0.8696 0.0034 34.89)",
      "--ring": "oklch(0.5827 0.2418 12.23)",
      "--chart-1": "oklch(0.5827 0.2418 12.23)",
      "--chart-2": "oklch(0.8938 0.0563 3.77)",
      "--chart-3": "oklch(0.7135 0.1879 7.69)",
      "--chart-4": "oklch(0.8131 0.1129 5.67)",
      "--chart-5": "oklch(0.9429 0.0283 5.52)",
      "--sidebar": "oklch(0.9567 0.0021 34.31)",
      "--sidebar-foreground": "oklch(0.2686 0.0032 34.85)",
      "--sidebar-primary": "oklch(0.5827 0.2418 12.23)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9227 0.0029 34.49)",
      "--sidebar-accent-foreground": "oklch(0.2686 0.0032 34.85)",
      "--sidebar-border": "oklch(0.9227 0.0029 34.49)",
      "--sidebar-ring": "oklch(0.5827 0.2418 12.23)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2686 0.0032 34.85)"
    },
    darkColors: {
      "--background": "oklch(0.1463 0.0027 33.58)",
      "--foreground": "oklch(0.9227 0.0029 34.49)",
      "--card": "oklch(0.1463 0.0027 33.58)",
      "--card-foreground": "oklch(0.9227 0.0029 34.49)",
      "--popover": "oklch(0.1463 0.0027 33.58)",
      "--popover-foreground": "oklch(0.9227 0.0029 34.49)",
      "--primary": "oklch(0.5827 0.2418 12.23)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.2686 0.0032 34.85)",
      "--secondary-foreground": "oklch(0.9227 0.0029 34.49)",
      "--muted": "oklch(0.212 0.0029 34.69)",
      "--muted-foreground": "oklch(0.7088 0.0038 34.83)",
      "--accent": "oklch(0.212 0.0029 34.69)",
      "--accent-foreground": "oklch(0.9227 0.0029 34.49)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2686 0.0032 34.85)",
      "--input": "oklch(0.3731 0.0037 36.25)",
      "--ring": "oklch(0.5827 0.2418 12.23)",
      "--chart-1": "oklch(0.5827 0.2418 12.23)",
      "--chart-2": "oklch(0.8938 0.0563 3.77)",
      "--chart-3": "oklch(0.7135 0.1879 7.69)",
      "--chart-4": "oklch(0.8131 0.1129 5.67)",
      "--chart-5": "oklch(0.9429 0.0283 5.52)",
      "--sidebar": "oklch(0.212 0.0029 34.69)",
      "--sidebar-foreground": "oklch(0.9227 0.0029 34.49)",
      "--sidebar-primary": "oklch(0.5827 0.2418 12.23)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.2686 0.0032 34.85)",
      "--sidebar-accent-foreground": "oklch(0.9227 0.0029 34.49)",
      "--sidebar-border": "oklch(0.2686 0.0032 34.85)",
      "--sidebar-ring": "oklch(0.5827 0.2418 12.23)"
    }
  },
  {
    name: "Red",
    value: "red",
    description: "Bold red theme with modern typography",
    fontFamily: "Roboto, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;600;700&display=swap",
    borderRadius: "0.65rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.141 0.005 285.823)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.141 0.005 285.823)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.141 0.005 285.823)",
      "--primary": "oklch(0.637 0.237 25.331)",
      "--primary-foreground": "oklch(0.971 0.013 17.38)",
      "--secondary": "oklch(0.967 0.001 286.375)",
      "--secondary-foreground": "oklch(0.21 0.006 285.885)",
      "--muted": "oklch(0.967 0.001 286.375)",
      "--muted-foreground": "oklch(0.552 0.016 285.938)",
      "--accent": "oklch(0.967 0.001 286.375)",
      "--accent-foreground": "oklch(0.21 0.006 285.885)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.92 0.004 286.32)",
      "--input": "oklch(0.92 0.004 286.32)",
      "--ring": "oklch(0.637 0.237 25.331)",
      "--chart-1": "oklch(0.646 0.222 41.116)",
      "--chart-2": "oklch(0.6 0.118 184.704)",
      "--chart-3": "oklch(0.398 0.07 227.392)",
      "--chart-4": "oklch(0.828 0.189 84.429)",
      "--chart-5": "oklch(0.769 0.188 70.08)",
      "--sidebar": "oklch(0.985 0 0)",
      "--sidebar-foreground": "oklch(0.141 0.005 285.823)",
      "--sidebar-primary": "oklch(0.637 0.237 25.331)",
      "--sidebar-primary-foreground": "oklch(0.971 0.013 17.38)",
      "--sidebar-accent": "oklch(0.967 0.001 286.375)",
      "--sidebar-accent-foreground": "oklch(0.21 0.006 285.885)",
      "--sidebar-border": "oklch(0.92 0.004 286.32)",
      "--sidebar-ring": "oklch(0.637 0.237 25.331)",
    },
    darkColors: {
      "--background": "oklch(0.141 0.005 285.823)",
      "--foreground": "oklch(0.985 0 0)",
      "--card": "oklch(0.21 0.006 285.885)",
      "--card-foreground": "oklch(0.985 0 0)",
      "--popover": "oklch(0.21 0.006 285.885)",
      "--popover-foreground": "oklch(0.985 0 0)",
      "--primary": "oklch(0.637 0.237 25.331)",
      "--primary-foreground": "oklch(0.971 0.013 17.38)",
      "--secondary": "oklch(0.274 0.006 286.033)",
      "--secondary-foreground": "oklch(0.985 0 0)",
      "--muted": "oklch(0.274 0.006 286.033)",
      "--muted-foreground": "oklch(0.705 0.015 286.067)",
      "--accent": "oklch(0.274 0.006 286.033)",
      "--accent-foreground": "oklch(0.985 0 0)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(1 0 0 / 10%)",
      "--input": "oklch(1 0 0 / 15%)",
      "--ring": "oklch(0.637 0.237 25.331)",
      "--chart-1": "oklch(0.488 0.243 264.376)",
      "--chart-2": "oklch(0.696 0.17 162.48)",
      "--chart-3": "oklch(0.769 0.188 70.08)",
      "--chart-4": "oklch(0.627 0.265 303.9)",
      "--chart-5": "oklch(0.645 0.246 16.439)",
      "--sidebar": "oklch(0.21 0.006 285.885)",
      "--sidebar-foreground": "oklch(0.985 0 0)",
      "--sidebar-primary": "oklch(0.637 0.237 25.331)",
      "--sidebar-primary-foreground": "oklch(0.971 0.013 17.38)",
      "--sidebar-accent": "oklch(0.274 0.006 286.033)",
      "--sidebar-accent-foreground": "oklch(0.985 0 0)",
      "--sidebar-border": "oklch(1 0 0 / 10%)",
      "--sidebar-ring": "oklch(0.637 0.237 25.331)",
    }
  },
  {
    name: "Retro Arcade",
    value: "retro-arcade",
    description: "Vibrant 80s arcade-inspired neon colors",
    fontFamily: "Press Start 2P, ui-monospace, SFMono-Regular, 'SF Mono', Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Press+Start+2P&display=swap",
    borderRadius: "0.25rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.05 0 0)",
      "--foreground": "oklch(0.95 0.3 320)",
      "--card": "oklch(0.08 0.05 320)",
      "--card-foreground": "oklch(0.95 0.3 320)",
      "--popover": "oklch(0.05 0 0)",
      "--popover-foreground": "oklch(0.95 0.3 320)",
      "--primary": "oklch(0.8 0.35 320)",
      "--primary-foreground": "oklch(0.05 0 0)",
      "--secondary": "oklch(0.75 0.3 280)",
      "--secondary-foreground": "oklch(0.05 0 0)",
      "--muted": "oklch(0.12 0.05 320)",
      "--muted-foreground": "oklch(0.7 0.25 320)",
      "--accent": "oklch(0.8 0.35 60)",
      "--accent-foreground": "oklch(0.05 0 0)",
      "--destructive": "oklch(0.7 0.35 0)",
      "--destructive-foreground": "oklch(0.05 0 0)",
      "--border": "oklch(0.8 0.35 320)",
      "--input": "oklch(0.8 0.35 320)",
      "--ring": "oklch(0.8 0.35 320)",
      "--chart-1": "oklch(0.8 0.35 320)",
      "--chart-2": "oklch(0.8 0.35 280)",
      "--chart-3": "oklch(0.8 0.35 60)",
      "--chart-4": "oklch(0.8 0.35 180)",
      "--chart-5": "oklch(0.8 0.35 120)",
      "--sidebar": "oklch(0.08 0.05 320)",
      "--sidebar-foreground": "oklch(0.95 0.3 320)",
      "--sidebar-primary": "oklch(0.8 0.35 320)",
      "--sidebar-primary-foreground": "oklch(0.05 0 0)",
      "--sidebar-accent": "oklch(0.12 0.05 320)",
      "--sidebar-accent-foreground": "oklch(0.95 0.3 320)",
      "--sidebar-border": "oklch(0.8 0.35 320)",
      "--sidebar-ring": "oklch(0.8 0.35 320)"
    },
    darkColors: {
      "--background": "oklch(0.05 0 0)",
      "--foreground": "oklch(0.95 0.3 320)",
      "--card": "oklch(0.08 0.05 320)",
      "--card-foreground": "oklch(0.95 0.3 320)",
      "--popover": "oklch(0.05 0 0)",
      "--popover-foreground": "oklch(0.95 0.3 320)",
      "--primary": "oklch(0.8 0.35 320)",
      "--primary-foreground": "oklch(0.05 0 0)",
      "--secondary": "oklch(0.75 0.3 280)",
      "--secondary-foreground": "oklch(0.05 0 0)",
      "--muted": "oklch(0.12 0.05 320)",
      "--muted-foreground": "oklch(0.7 0.25 320)",
      "--accent": "oklch(0.8 0.35 60)",
      "--accent-foreground": "oklch(0.05 0 0)",
      "--destructive": "oklch(0.7 0.35 0)",
      "--destructive-foreground": "oklch(0.05 0 0)",
      "--border": "oklch(0.8 0.35 320)",
      "--input": "oklch(0.8 0.35 320)",
      "--ring": "oklch(0.8 0.35 320)",
      "--chart-1": "oklch(0.8 0.35 320)",
      "--chart-2": "oklch(0.8 0.35 280)",
      "--chart-3": "oklch(0.8 0.35 60)",
      "--chart-4": "oklch(0.8 0.35 180)",
      "--chart-5": "oklch(0.8 0.35 120)",
      "--sidebar": "oklch(0.08 0.05 320)",
      "--sidebar-foreground": "oklch(0.95 0.3 320)",
      "--sidebar-primary": "oklch(0.8 0.35 320)",
      "--sidebar-primary-foreground": "oklch(0.05 0 0)",
      "--sidebar-accent": "oklch(0.12 0.05 320)",
      "--sidebar-accent-foreground": "oklch(0.95 0.3 320)",
      "--sidebar-border": "oklch(0.8 0.35 320)",
      "--sidebar-ring": "oklch(0.8 0.35 320)"
    }
  },
  {
    name: "Rose",
    value: "rose",
    description: "Elegant rose theme with serif fonts",
    fontFamily: "Crimson Text, ui-serif, Georgia, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Crimson+Text:wght@400;600;700&display=swap",
    borderRadius: "0.65rem",
    style: "new-york",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.141 0.005 285.823)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.141 0.005 285.823)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.141 0.005 285.823)",
      "--primary": "oklch(0.645 0.246 16.439)",
      "--primary-foreground": "oklch(0.969 0.015 12.422)",
      "--secondary": "oklch(0.967 0.001 286.375)",
      "--secondary-foreground": "oklch(0.21 0.006 285.885)",
      "--muted": "oklch(0.967 0.001 286.375)",
      "--muted-foreground": "oklch(0.552 0.016 285.938)",
      "--accent": "oklch(0.967 0.001 286.375)",
      "--accent-foreground": "oklch(0.21 0.006 285.885)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.92 0.004 286.32)",
      "--input": "oklch(0.92 0.004 286.32)",
      "--ring": "oklch(0.645 0.246 16.439)",
      "--chart-1": "oklch(0.646 0.222 41.116)",
      "--chart-2": "oklch(0.6 0.118 184.704)",
      "--chart-3": "oklch(0.398 0.07 227.392)",
      "--chart-4": "oklch(0.828 0.189 84.429)",
      "--chart-5": "oklch(0.769 0.188 70.08)",
      "--sidebar": "oklch(0.985 0 0)",
      "--sidebar-foreground": "oklch(0.141 0.005 285.823)",
      "--sidebar-primary": "oklch(0.645 0.246 16.439)",
      "--sidebar-primary-foreground": "oklch(0.969 0.015 12.422)",
      "--sidebar-accent": "oklch(0.967 0.001 286.375)",
      "--sidebar-accent-foreground": "oklch(0.21 0.006 285.885)",
      "--sidebar-border": "oklch(0.92 0.004 286.32)",
      "--sidebar-ring": "oklch(0.645 0.246 16.439)",
    },
    darkColors: {
      "--background": "oklch(0.141 0.005 285.823)",
      "--foreground": "oklch(0.985 0 0)",
      "--card": "oklch(0.21 0.006 285.885)",
      "--card-foreground": "oklch(0.985 0 0)",
      "--popover": "oklch(0.21 0.006 285.885)",
      "--popover-foreground": "oklch(0.985 0 0)",
      "--primary": "oklch(0.645 0.246 16.439)",
      "--primary-foreground": "oklch(0.969 0.015 12.422)",
      "--secondary": "oklch(0.274 0.006 286.033)",
      "--secondary-foreground": "oklch(0.985 0 0)",
      "--muted": "oklch(0.274 0.006 286.033)",
      "--muted-foreground": "oklch(0.705 0.015 286.067)",
      "--accent": "oklch(0.274 0.006 286.033)",
      "--accent-foreground": "oklch(0.985 0 0)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(1 0 0 / 10%)",
      "--input": "oklch(1 0 0 / 15%)",
      "--ring": "oklch(0.645 0.246 16.439)",
      "--chart-1": "oklch(0.488 0.243 264.376)",
      "--chart-2": "oklch(0.696 0.17 162.48)",
      "--chart-3": "oklch(0.769 0.188 70.08)",
      "--chart-4": "oklch(0.627 0.265 303.9)",
      "--chart-5": "oklch(0.645 0.246 16.439)",
      "--sidebar": "oklch(0.21 0.006 285.885)",
      "--sidebar-foreground": "oklch(0.985 0 0)",
      "--sidebar-primary": "oklch(0.645 0.246 16.439)",
      "--sidebar-primary-foreground": "oklch(0.969 0.015 12.422)",
      "--sidebar-accent": "oklch(0.274 0.006 286.033)",
      "--sidebar-accent-foreground": "oklch(0.985 0 0)",
      "--sidebar-border": "oklch(1 0 0 / 10%)",
      "--sidebar-ring": "oklch(0.645 0.246 16.439)",
    }
  },
  {
    name: "Santa Fe",
    value: "santa-fe",
    description: "Santa Fe theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.75rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.2759 0.0032 202.42)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2759 0.0032 202.42)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2759 0.0032 202.42)",
      "--primary": "oklch(0.4646 0.0189 197.03)",
      "--secondary": "oklch(0.5742 0.1072 35.99)",
      "--muted": "oklch(0.9843 0.001 196)",
      "--muted-foreground": "oklch(0.4454 0.004 202.09)",
      "--accent": "oklch(0.9843 0.001 196)",
      "--accent-foreground": "oklch(0.2759 0.0032 202.42)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9273 0.0029 197.56)",
      "--input": "oklch(0.869 0.0034 198.74)",
      "--ring": "oklch(0.4646 0.0189 197.03)",
      "--chart-1": "oklch(0.4646 0.0189 197.03)",
      "--chart-2": "oklch(0.5742 0.1072 35.99)",
      "--chart-3": "oklch(0.8561 0.0302 188.27)",
      "--chart-4": "oklch(0.8228 0.0645 38.19)",
      "--chart-5": "oklch(0.9514 0.0117 187.62)",
      "--sidebar": "oklch(0.9685 0.0021 197.03)",
      "--sidebar-foreground": "oklch(0.2759 0.0032 202.42)",
      "--sidebar-primary": "oklch(0.4646 0.0189 197.03)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9273 0.0029 197.56)",
      "--sidebar-accent-foreground": "oklch(0.2759 0.0032 202.42)",
      "--sidebar-border": "oklch(0.9273 0.0029 197.56)",
      "--sidebar-ring": "oklch(0.4646 0.0189 197.03)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2759 0.0032 202.42)"
    },
    darkColors: {
      "--background": "oklch(0.134 0.0027 203.77)",
      "--foreground": "oklch(0.9273 0.0029 197.56)",
      "--card": "oklch(0.134 0.0027 203.77)",
      "--card-foreground": "oklch(0.9273 0.0029 197.56)",
      "--popover": "oklch(0.134 0.0027 203.77)",
      "--popover-foreground": "oklch(0.9273 0.0029 197.56)",
      "--primary": "oklch(0.4646 0.0189 197.03)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.5742 0.1072 35.99)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.2102 0.0029 203.59)",
      "--muted-foreground": "oklch(0.7054 0.0038 200.79)",
      "--accent": "oklch(0.2102 0.0029 203.59)",
      "--accent-foreground": "oklch(0.9273 0.0029 197.56)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2759 0.0032 202.42)",
      "--input": "oklch(0.3726 0.0037 201.79)",
      "--ring": "oklch(0.4646 0.0189 197.03)",
      "--chart-1": "oklch(0.4646 0.0189 197.03)",
      "--chart-2": "oklch(0.5742 0.1072 35.99)",
      "--chart-3": "oklch(0.8561 0.0302 188.27)",
      "--chart-4": "oklch(0.8228 0.0645 38.19)",
      "--chart-5": "oklch(0.9514 0.0117 187.62)",
      "--sidebar": "oklch(0.2102 0.0029 203.59)",
      "--sidebar-foreground": "oklch(0.9273 0.0029 197.56)",
      "--sidebar-primary": "oklch(0.4646 0.0189 197.03)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.2759 0.0032 202.42)",
      "--sidebar-accent-foreground": "oklch(0.9273 0.0029 197.56)",
      "--sidebar-border": "oklch(0.2759 0.0032 202.42)",
      "--sidebar-ring": "oklch(0.4646 0.0189 197.03)"
    }
  },
  {
    name: "Sky",
    value: "sky",
    description: "Sky theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "1rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9842 0.0032 228.71)",
      "--foreground": "oklch(0.2779 0.0189 236.17)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2779 0.0189 236.17)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2779 0.0189 236.17)",
      "--primary": "oklch(0.815 0.0757 230.32)",
      "--secondary": "oklch(0.9213 0.0126 230.32)",
      "--muted": "oklch(0.9682 0.0064 229.74)",
      "--muted-foreground": "oklch(0.4459 0.0239 235)",
      "--accent": "oklch(0.9682 0.0064 229.74)",
      "--accent-foreground": "oklch(0.2779 0.0189 236.17)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9213 0.0126 230.32)",
      "--input": "oklch(0.869 0.019 231.62)",
      "--ring": "oklch(0.815 0.0757 230.32)",
      "--chart-1": "oklch(0.815 0.0757 230.32)",
      "--chart-2": "oklch(0.9035 0.041 230.66)",
      "--chart-3": "oklch(0.7527 0.1062 232.98)",
      "--chart-4": "oklch(0.815 0.0757 230.32)",
      "--chart-5": "oklch(0.9521 0.0193 232.19)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.2779 0.0189 236.17)",
      "--sidebar-primary": "oklch(0.815 0.0757 230.32)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.9842 0.0032 228.71)",
      "--sidebar-accent-foreground": "oklch(0.2779 0.0189 236.17)",
      "--sidebar-border": "oklch(0.9213 0.0126 230.32)",
      "--sidebar-ring": "oklch(0.815 0.0757 230.32)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2779 0.0189 236.17)"
    },
    darkColors: {
      "--background": "oklch(0.1311 0.0164 238.31)",
      "--foreground": "oklch(0.9213 0.0126 230.32)",
      "--card": "oklch(0.209 0.0176 237.67)",
      "--card-foreground": "oklch(0.9213 0.0126 230.32)",
      "--popover": "oklch(0.209 0.0176 237.67)",
      "--popover-foreground": "oklch(0.9213 0.0126 230.32)",
      "--primary": "oklch(0.815 0.0757 230.32)",
      "--primary-foreground": "oklch(0 0 0)",
      "--secondary": "oklch(0.3723 0.0221 234.88)",
      "--secondary-foreground": "oklch(0.9842 0.0032 228.71)",
      "--muted": "oklch(0.2779 0.0189 236.17)",
      "--muted-foreground": "oklch(0.869 0.019 231.62)",
      "--accent": "oklch(0.2779 0.0189 236.17)",
      "--accent-foreground": "oklch(0.9213 0.0126 230.32)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2779 0.0189 236.17)",
      "--input": "oklch(0.3723 0.0221 234.88)",
      "--ring": "oklch(0.815 0.0757 230.32)",
      "--chart-1": "oklch(0.815 0.0757 230.32)",
      "--chart-2": "oklch(0.9035 0.041 230.66)",
      "--chart-3": "oklch(0.7527 0.1062 232.98)",
      "--chart-4": "oklch(0.815 0.0757 230.32)",
      "--chart-5": "oklch(0.9521 0.0193 232.19)",
      "--sidebar": "oklch(0.209 0.0176 237.67)",
      "--sidebar-foreground": "oklch(0.9213 0.0126 230.32)",
      "--sidebar-primary": "oklch(0.815 0.0757 230.32)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.2779 0.0189 236.17)",
      "--sidebar-accent-foreground": "oklch(0.9213 0.0126 230.32)",
      "--sidebar-border": "oklch(0.2779 0.0189 236.17)",
      "--sidebar-ring": "oklch(0.815 0.0757 230.32)"
    }
  },
  {
    name: "Soft Pop",
    value: "soft-pop",
    description: "Gentle pastels with playful pop colors",
    fontFamily: "Quicksand, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Quicksand:wght@400;500;600;700&display=swap",
    borderRadius: "1rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.99 0.01 330)",
      "--foreground": "oklch(0.15 0.05 330)",
      "--card": "oklch(0.99 0.01 330)",
      "--card-foreground": "oklch(0.15 0.05 330)",
      "--popover": "oklch(0.99 0.01 330)",
      "--popover-foreground": "oklch(0.15 0.05 330)",
      "--primary": "oklch(0.65 0.15 330)",
      "--primary-foreground": "oklch(0.99 0.01 330)",
      "--secondary": "oklch(0.94 0.03 330)",
      "--secondary-foreground": "oklch(0.65 0.15 330)",
      "--muted": "oklch(0.94 0.03 330)",
      "--muted-foreground": "oklch(0.55 0.1 330)",
      "--accent": "oklch(0.88 0.08 300)",
      "--accent-foreground": "oklch(0.65 0.15 330)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.87 0.04 330)",
      "--input": "oklch(0.87 0.04 330)",
      "--ring": "oklch(0.65 0.15 330)",
      "--chart-1": "oklch(0.7 0.2 330)",
      "--chart-2": "oklch(0.6 0.25 300)",
      "--chart-3": "oklch(0.5 0.2 0)",
      "--chart-4": "oklch(0.8 0.15 60)",
      "--chart-5": "oklch(0.9 0.1 120)",
      "--sidebar": "oklch(0.97 0.02 330)",
      "--sidebar-foreground": "oklch(0.15 0.05 330)",
      "--sidebar-primary": "oklch(0.65 0.15 330)",
      "--sidebar-primary-foreground": "oklch(0.99 0.01 330)",
      "--sidebar-accent": "oklch(0.94 0.03 330)",
      "--sidebar-accent-foreground": "oklch(0.65 0.15 330)",
      "--sidebar-border": "oklch(0.87 0.04 330)",
      "--sidebar-ring": "oklch(0.65 0.15 330)"
    },
    darkColors: {
      "--background": "oklch(0.08 0.03 330)",
      "--foreground": "oklch(0.92 0.05 330)",
      "--card": "oklch(0.11 0.05 330)",
      "--card-foreground": "oklch(0.92 0.05 330)",
      "--popover": "oklch(0.08 0.03 330)",
      "--popover-foreground": "oklch(0.92 0.05 330)",
      "--primary": "oklch(0.75 0.2 330)",
      "--primary-foreground": "oklch(0.08 0.03 330)",
      "--secondary": "oklch(0.18 0.05 330)",
      "--secondary-foreground": "oklch(0.92 0.05 330)",
      "--muted": "oklch(0.18 0.05 330)",
      "--muted-foreground": "oklch(0.65 0.12 330)",
      "--accent": "oklch(0.22 0.1 300)",
      "--accent-foreground": "oklch(0.92 0.05 330)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.28 0.05 330)",
      "--input": "oklch(0.28 0.05 330)",
      "--ring": "oklch(0.75 0.2 330)",
      "--chart-1": "oklch(0.8 0.25 330)",
      "--chart-2": "oklch(0.7 0.3 300)",
      "--chart-3": "oklch(0.6 0.25 0)",
      "--chart-4": "oklch(0.9 0.2 60)",
      "--chart-5": "oklch(1 0.15 120)",
      "--sidebar": "oklch(0.11 0.05 330)",
      "--sidebar-foreground": "oklch(0.92 0.05 330)",
      "--sidebar-primary": "oklch(0.75 0.2 330)",
      "--sidebar-primary-foreground": "oklch(0.08 0.03 330)",
      "--sidebar-accent": "oklch(0.18 0.05 330)",
      "--sidebar-accent-foreground": "oklch(0.92 0.05 330)",
      "--sidebar-border": "oklch(0.28 0.05 330)",
      "--sidebar-ring": "oklch(0.75 0.2 330)"
    }
  },
  {
    name: "Solar",
    value: "solar",
    description: "Solar theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "1rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9849 0.0077 64.83)",
      "--foreground": "oklch(0.2686 0.0441 67.04)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2686 0.0441 67.04)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2686 0.0441 67.04)",
      "--primary": "oklch(0.7374 0.1036 74.05)",
      "--secondary": "oklch(0.5648 0.1224 53.29)",
      "--muted": "oklch(0.9699 0.0147 65.83)",
      "--muted-foreground": "oklch(0.4441 0.0559 70.11)",
      "--accent": "oklch(0.9699 0.0147 65.83)",
      "--accent-foreground": "oklch(0.2686 0.0441 67.04)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9213 0.0294 66.14)",
      "--input": "oklch(0.869 0.0441 66.83)",
      "--ring": "oklch(0.7374 0.1036 74.05)",
      "--chart-1": "oklch(0.7374 0.1036 74.05)",
      "--chart-2": "oklch(0.5648 0.1224 53.29)",
      "--chart-3": "oklch(0.886 0.0954 93.22)",
      "--chart-4": "oklch(0.8552 0.0989 62.61)",
      "--chart-5": "oklch(0.966 0.0343 97.5)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.2686 0.0441 67.04)",
      "--sidebar-primary": "oklch(0.7374 0.1036 74.05)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.9849 0.0077 64.83)",
      "--sidebar-accent-foreground": "oklch(0.2686 0.0441 67.04)",
      "--sidebar-border": "oklch(0.9213 0.0294 66.14)",
      "--sidebar-ring": "oklch(0.7374 0.1036 74.05)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2686 0.0441 67.04)"
    },
    darkColors: {
      "--background": "oklch(0.146 0.0252 65.24)",
      "--foreground": "oklch(0.9213 0.0294 66.14)",
      "--card": "oklch(0.2156 0.0329 66.89)",
      "--card-foreground": "oklch(0.9213 0.0294 66.14)",
      "--popover": "oklch(0.2156 0.0329 66.89)",
      "--popover-foreground": "oklch(0.9213 0.0294 66.14)",
      "--primary": "oklch(0.7374 0.1036 74.05)",
      "--primary-foreground": "oklch(0 0 0)",
      "--secondary": "oklch(0.5648 0.1224 53.29)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.2686 0.0441 67.04)",
      "--muted-foreground": "oklch(0.869 0.0441 66.83)",
      "--accent": "oklch(0.2686 0.0441 67.04)",
      "--accent-foreground": "oklch(0.9213 0.0294 66.14)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2686 0.0441 67.04)",
      "--input": "oklch(0.3739 0.0514 69.09)",
      "--ring": "oklch(0.7374 0.1036 74.05)",
      "--chart-1": "oklch(0.7374 0.1036 74.05)",
      "--chart-2": "oklch(0.5648 0.1224 53.29)",
      "--chart-3": "oklch(0.886 0.0954 93.22)",
      "--chart-4": "oklch(0.8552 0.0989 62.61)",
      "--chart-5": "oklch(0.966 0.0343 97.5)",
      "--sidebar": "oklch(0.2156 0.0329 66.89)",
      "--sidebar-foreground": "oklch(0.9213 0.0294 66.14)",
      "--sidebar-primary": "oklch(0.7374 0.1036 74.05)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.2686 0.0441 67.04)",
      "--sidebar-accent-foreground": "oklch(0.9213 0.0294 66.14)",
      "--sidebar-border": "oklch(0.2686 0.0441 67.04)",
      "--sidebar-ring": "oklch(0.7374 0.1036 74.05)"
    }
  },
  {
    name: "Solar Dusk",
    value: "solar-dusk",
    description: "Warm sunset oranges and deep twilight blues",
    fontFamily: "Raleway, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Raleway:wght@400;500;600;700&display=swap",
    borderRadius: "0.6rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.98 0.02 40)",
      "--foreground": "oklch(0.12 0.05 40)",
      "--card": "oklch(0.98 0.02 40)",
      "--card-foreground": "oklch(0.12 0.05 40)",
      "--popover": "oklch(0.98 0.02 40)",
      "--popover-foreground": "oklch(0.12 0.05 40)",
      "--primary": "oklch(0.5 0.18 40)",
      "--primary-foreground": "oklch(0.98 0.02 40)",
      "--secondary": "oklch(0.93 0.03 40)",
      "--secondary-foreground": "oklch(0.5 0.18 40)",
      "--muted": "oklch(0.93 0.03 40)",
      "--muted-foreground": "oklch(0.55 0.12 40)",
      "--accent": "oklch(0.87 0.08 20)",
      "--accent-foreground": "oklch(0.5 0.18 40)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.86 0.04 40)",
      "--input": "oklch(0.86 0.04 40)",
      "--ring": "oklch(0.5 0.18 40)",
      "--chart-1": "oklch(0.6 0.2 40)",
      "--chart-2": "oklch(0.5 0.25 20)",
      "--chart-3": "oklch(0.4 0.2 60)",
      "--chart-4": "oklch(0.7 0.15 0)",
      "--chart-5": "oklch(0.8 0.1 80)",
      "--sidebar": "oklch(0.96 0.03 40)",
      "--sidebar-foreground": "oklch(0.12 0.05 40)",
      "--sidebar-primary": "oklch(0.5 0.18 40)",
      "--sidebar-primary-foreground": "oklch(0.98 0.02 40)",
      "--sidebar-accent": "oklch(0.93 0.03 40)",
      "--sidebar-accent-foreground": "oklch(0.5 0.18 40)",
      "--sidebar-border": "oklch(0.86 0.04 40)",
      "--sidebar-ring": "oklch(0.5 0.18 40)"
    },
    darkColors: {
      "--background": "oklch(0.06 0.03 240)",
      "--foreground": "oklch(0.9 0.05 40)",
      "--card": "oklch(0.09 0.05 240)",
      "--card-foreground": "oklch(0.9 0.05 40)",
      "--popover": "oklch(0.06 0.03 240)",
      "--popover-foreground": "oklch(0.9 0.05 40)",
      "--primary": "oklch(0.7 0.22 40)",
      "--primary-foreground": "oklch(0.06 0.03 240)",
      "--secondary": "oklch(0.16 0.05 240)",
      "--secondary-foreground": "oklch(0.9 0.05 40)",
      "--muted": "oklch(0.16 0.05 240)",
      "--muted-foreground": "oklch(0.65 0.12 40)",
      "--accent": "oklch(0.19 0.1 20)",
      "--accent-foreground": "oklch(0.9 0.05 40)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.26 0.05 240)",
      "--input": "oklch(0.26 0.05 240)",
      "--ring": "oklch(0.7 0.22 40)",
      "--chart-1": "oklch(0.7 0.25 40)",
      "--chart-2": "oklch(0.6 0.3 20)",
      "--chart-3": "oklch(0.5 0.25 60)",
      "--chart-4": "oklch(0.8 0.2 0)",
      "--chart-5": "oklch(0.9 0.15 80)",
      "--sidebar": "oklch(0.09 0.05 240)",
      "--sidebar-foreground": "oklch(0.9 0.05 40)",
      "--sidebar-primary": "oklch(0.7 0.22 40)",
      "--sidebar-primary-foreground": "oklch(0.06 0.03 240)",
      "--sidebar-accent": "oklch(0.16 0.05 240)",
      "--sidebar-accent-foreground": "oklch(0.9 0.05 40)",
      "--sidebar-border": "oklch(0.26 0.05 240)",
      "--sidebar-ring": "oklch(0.7 0.22 40)"
    }
  },
  {
    name: "Spooky",
    value: "spooky",
    description: "Spooky theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "1rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9863 0.0043 278.87)",
      "--foreground": "oklch(0.2753 0.0327 278.01)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2753 0.0327 278.01)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2753 0.0327 278.01)",
      "--primary": "oklch(0.5197 0.2712 293.31)",
      "--secondary": "oklch(0.7164 0.1906 47.14)",
      "--muted": "oklch(0.968 0.0087 279.34)",
      "--muted-foreground": "oklch(0.4291 0.0484 277.82)",
      "--accent": "oklch(0.968 0.0087 279.34)",
      "--accent-foreground": "oklch(0.2753 0.0327 278.01)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9232 0.0174 279.3)",
      "--input": "oklch(0.8719 0.026 278.63)",
      "--ring": "oklch(0.5197 0.2712 293.31)",
      "--chart-1": "oklch(0.5197 0.2712 293.31)",
      "--chart-2": "oklch(0.7164 0.1906 47.14)",
      "--chart-3": "oklch(0.8122 0.1073 293.99)",
      "--chart-4": "oklch(0.8362 0.114 51.61)",
      "--chart-5": "oklch(0.9444 0.0281 294.99)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.2753 0.0327 278.01)",
      "--sidebar-primary": "oklch(0.5197 0.2712 293.31)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9863 0.0043 278.87)",
      "--sidebar-accent-foreground": "oklch(0.2753 0.0327 278.01)",
      "--sidebar-border": "oklch(0.9232 0.0174 279.3)",
      "--sidebar-ring": "oklch(0.5197 0.2712 293.31)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2753 0.0327 278.01)"
    },
    darkColors: {
      "--background": "oklch(0.1379 0.0217 278.7)",
      "--foreground": "oklch(0.9232 0.0174 279.3)",
      "--card": "oklch(0.2101 0.0257 279.03)",
      "--card-foreground": "oklch(0.9232 0.0174 279.3)",
      "--popover": "oklch(0.2101 0.0257 279.03)",
      "--popover-foreground": "oklch(0.9232 0.0174 279.3)",
      "--primary": "oklch(0.5197 0.2712 293.31)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.7164 0.1906 47.14)",
      "--secondary-foreground": "oklch(0 0 0)",
      "--muted": "oklch(0.2753 0.0327 278.01)",
      "--muted-foreground": "oklch(0.8719 0.026 278.63)",
      "--accent": "oklch(0.2753 0.0327 278.01)",
      "--accent-foreground": "oklch(0.9232 0.0174 279.3)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2753 0.0327 278.01)",
      "--input": "oklch(0.371 0.0417 278.11)",
      "--ring": "oklch(0.5197 0.2712 293.31)",
      "--chart-1": "oklch(0.5197 0.2712 293.31)",
      "--chart-2": "oklch(0.7164 0.1906 47.14)",
      "--chart-3": "oklch(0.8122 0.1073 293.99)",
      "--chart-4": "oklch(0.8362 0.114 51.61)",
      "--chart-5": "oklch(0.9444 0.0281 294.99)",
      "--sidebar": "oklch(0.2101 0.0257 279.03)",
      "--sidebar-foreground": "oklch(0.9232 0.0174 279.3)",
      "--sidebar-primary": "oklch(0.5197 0.2712 293.31)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.2753 0.0327 278.01)",
      "--sidebar-accent-foreground": "oklch(0.9232 0.0174 279.3)",
      "--sidebar-border": "oklch(0.2753 0.0327 278.01)",
      "--sidebar-ring": "oklch(0.5197 0.2712 293.31)"
    }
  },
  {
    name: "Spring Bouquet",
    value: "spring-bouquet",
    description: "Spring Bouquet theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.984 0.0022 247.84)",
      "--foreground": "oklch(0.279 0.0299 260.05)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.279 0.0299 260.05)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.279 0.0299 260.05)",
      "--primary": "oklch(0.5315 0.0694 156.19)",
      "--secondary": "oklch(0.5748 0.0862 336.52)",
      "--muted": "oklch(0.9687 0.0051 247.88)",
      "--muted-foreground": "oklch(0.446 0.0313 257.3)",
      "--accent": "oklch(0.9687 0.0051 247.88)",
      "--accent-foreground": "oklch(0.279 0.0299 260.05)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.929 0.0095 255.53)",
      "--input": "oklch(0.869 0.016 252.91)",
      "--ring": "oklch(0.5315 0.0694 156.19)",
      "--chart-1": "oklch(0.5315 0.0694 156.19)",
      "--chart-2": "oklch(0.5748 0.0862 336.52)",
      "--chart-3": "oklch(0.8603 0.0731 158.6)",
      "--chart-4": "oklch(0.8292 0.0422 332)",
      "--chart-5": "oklch(0.9564 0.0234 159.23)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.279 0.0299 260.05)",
      "--sidebar-primary": "oklch(0.5315 0.0694 156.19)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.984 0.0022 247.84)",
      "--sidebar-accent-foreground": "oklch(0.279 0.0299 260.05)",
      "--sidebar-border": "oklch(0.929 0.0095 255.53)",
      "--sidebar-ring": "oklch(0.5315 0.0694 156.19)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.279 0.0299 260.05)"
    },
    darkColors: {
      "--background": "oklch(0.129 0.0306 264.71)",
      "--foreground": "oklch(0.929 0.0095 255.53)",
      "--card": "oklch(0.208 0.0306 265.77)",
      "--card-foreground": "oklch(0.929 0.0095 255.53)",
      "--popover": "oklch(0.208 0.0306 265.77)",
      "--popover-foreground": "oklch(0.929 0.0095 255.53)",
      "--primary": "oklch(0.5315 0.0694 156.19)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.5748 0.0862 336.52)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.279 0.0299 260.05)",
      "--muted-foreground": "oklch(0.869 0.016 252.91)",
      "--accent": "oklch(0.279 0.0299 260.05)",
      "--accent-foreground": "oklch(0.929 0.0095 255.53)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.279 0.0299 260.05)",
      "--input": "oklch(0.372 0.0321 257.31)",
      "--ring": "oklch(0.5315 0.0694 156.19)",
      "--chart-1": "oklch(0.5315 0.0694 156.19)",
      "--chart-2": "oklch(0.5748 0.0862 336.52)",
      "--chart-3": "oklch(0.8603 0.0731 158.6)",
      "--chart-4": "oklch(0.8292 0.0422 332)",
      "--chart-5": "oklch(0.9564 0.0234 159.23)",
      "--sidebar": "oklch(0.208 0.0306 265.77)",
      "--sidebar-foreground": "oklch(0.929 0.0095 255.53)",
      "--sidebar-primary": "oklch(0.5315 0.0694 156.19)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.279 0.0299 260.05)",
      "--sidebar-accent-foreground": "oklch(0.929 0.0095 255.53)",
      "--sidebar-border": "oklch(0.279 0.0299 260.05)",
      "--sidebar-ring": "oklch(0.5315 0.0694 156.19)"
    }
  },
  {
    name: "Starry Night",
    value: "starry-night",
    description: "Deep navy with shimmering star-like accents",
    fontFamily: "Crimson Text, ui-serif, Georgia, Cambria, 'Times New Roman', Times, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Crimson+Text:wght@400;500;600;700&display=swap",
    borderRadius: "0.5rem",
    style: "new-york",
    lightColors: {
      "--background": "oklch(0.97 0.01 240)",
      "--foreground": "oklch(0.08 0.05 240)",
      "--card": "oklch(0.97 0.01 240)",
      "--card-foreground": "oklch(0.08 0.05 240)",
      "--popover": "oklch(0.97 0.01 240)",
      "--popover-foreground": "oklch(0.08 0.05 240)",
      "--primary": "oklch(0.25 0.12 240)",
      "--primary-foreground": "oklch(0.97 0.01 240)",
      "--secondary": "oklch(0.92 0.02 240)",
      "--secondary-foreground": "oklch(0.25 0.12 240)",
      "--muted": "oklch(0.92 0.02 240)",
      "--muted-foreground": "oklch(0.5 0.08 240)",
      "--accent": "oklch(0.85 0.05 60)",
      "--accent-foreground": "oklch(0.25 0.12 240)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.84 0.03 240)",
      "--input": "oklch(0.84 0.03 240)",
      "--ring": "oklch(0.25 0.12 240)",
      "--chart-1": "oklch(0.5 0.15 240)",
      "--chart-2": "oklch(0.4 0.2 220)",
      "--chart-3": "oklch(0.3 0.15 260)",
      "--chart-4": "oklch(0.6 0.1 60)",
      "--chart-5": "oklch(0.7 0.05 80)",
      "--sidebar": "oklch(0.95 0.02 240)",
      "--sidebar-foreground": "oklch(0.08 0.05 240)",
      "--sidebar-primary": "oklch(0.25 0.12 240)",
      "--sidebar-primary-foreground": "oklch(0.97 0.01 240)",
      "--sidebar-accent": "oklch(0.92 0.02 240)",
      "--sidebar-accent-foreground": "oklch(0.25 0.12 240)",
      "--sidebar-border": "oklch(0.84 0.03 240)",
      "--sidebar-ring": "oklch(0.25 0.12 240)"
    },
    darkColors: {
      "--background": "oklch(0.03 0.02 240)",
      "--foreground": "oklch(0.95 0.03 60)",
      "--card": "oklch(0.05 0.03 240)",
      "--card-foreground": "oklch(0.95 0.03 60)",
      "--popover": "oklch(0.03 0.02 240)",
      "--popover-foreground": "oklch(0.95 0.03 60)",
      "--primary": "oklch(0.85 0.08 60)",
      "--primary-foreground": "oklch(0.03 0.02 240)",
      "--secondary": "oklch(0.12 0.03 240)",
      "--secondary-foreground": "oklch(0.95 0.03 60)",
      "--muted": "oklch(0.12 0.03 240)",
      "--muted-foreground": "oklch(0.7 0.05 60)",
      "--accent": "oklch(0.15 0.05 60)",
      "--accent-foreground": "oklch(0.95 0.03 60)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.2 0.03 240)",
      "--input": "oklch(0.2 0.03 240)",
      "--ring": "oklch(0.85 0.08 60)",
      "--chart-1": "oklch(0.6 0.2 240)",
      "--chart-2": "oklch(0.5 0.25 220)",
      "--chart-3": "oklch(0.4 0.2 260)",
      "--chart-4": "oklch(0.8 0.15 60)",
      "--chart-5": "oklch(0.9 0.1 80)",
      "--sidebar": "oklch(0.05 0.03 240)",
      "--sidebar-foreground": "oklch(0.95 0.03 60)",
      "--sidebar-primary": "oklch(0.85 0.08 60)",
      "--sidebar-primary-foreground": "oklch(0.03 0.02 240)",
      "--sidebar-accent": "oklch(0.12 0.03 240)",
      "--sidebar-accent-foreground": "oklch(0.95 0.03 60)",
      "--sidebar-border": "oklch(0.2 0.03 240)",
      "--sidebar-ring": "oklch(0.85 0.08 60)"
    }
  },
  {
    name: "Stone",
    value: "stone",
    description: "Warm stone theme with Crimson Text serif font",
    fontFamily: "Crimson Text, ui-serif, Georgia, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Crimson+Text:wght@400;600;700&display=swap",
    borderRadius: "0.5rem",
    style: "new-york",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.147 0.004 49.25)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.147 0.004 49.25)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.147 0.004 49.25)",
      "--primary": "oklch(0.216 0.006 56.043)",
      "--primary-foreground": "oklch(0.985 0.001 106.423)",
      "--secondary": "oklch(0.97 0.001 106.424)",
      "--secondary-foreground": "oklch(0.216 0.006 56.043)",
      "--muted": "oklch(0.97 0.001 106.424)",
      "--muted-foreground": "oklch(0.553 0.013 58.071)",
      "--accent": "oklch(0.97 0.001 106.424)",
      "--accent-foreground": "oklch(0.216 0.006 56.043)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--destructive-foreground": "oklch(0.985 0.001 106.423)",
      "--border": "oklch(0.923 0.003 48.717)",
      "--input": "oklch(0.923 0.003 48.717)",
      "--ring": "oklch(0.709 0.01 56.259)",
      "--chart-1": "oklch(0.646 0.222 41.116)",
      "--chart-2": "oklch(0.6 0.118 184.704)",
      "--chart-3": "oklch(0.398 0.07 227.392)",
      "--chart-4": "oklch(0.828 0.189 84.429)",
      "--chart-5": "oklch(0.769 0.188 70.08)",
      "--sidebar": "oklch(0.985 0.001 106.423)",
      "--sidebar-foreground": "oklch(0.147 0.004 49.25)",
      "--sidebar-primary": "oklch(0.216 0.006 56.043)",
      "--sidebar-primary-foreground": "oklch(0.985 0.001 106.423)",
      "--sidebar-accent": "oklch(0.97 0.001 106.424)",
      "--sidebar-accent-foreground": "oklch(0.216 0.006 56.043)",
      "--sidebar-border": "oklch(0.923 0.003 48.717)",
      "--sidebar-ring": "oklch(0.709 0.01 56.259)",
    },
    darkColors: {
      "--background": "oklch(0.147 0.004 49.25)",
      "--foreground": "oklch(0.985 0.001 106.423)",
      "--card": "oklch(0.216 0.006 56.043)",
      "--card-foreground": "oklch(0.985 0.001 106.423)",
      "--popover": "oklch(0.216 0.006 56.043)",
      "--popover-foreground": "oklch(0.985 0.001 106.423)",
      "--primary": "oklch(0.923 0.003 48.717)",
      "--primary-foreground": "oklch(0.216 0.006 56.043)",
      "--secondary": "oklch(0.268 0.007 34.298)",
      "--secondary-foreground": "oklch(0.985 0.001 106.423)",
      "--muted": "oklch(0.268 0.007 34.298)",
      "--muted-foreground": "oklch(0.709 0.01 56.259)",
      "--accent": "oklch(0.268 0.007 34.298)",
      "--accent-foreground": "oklch(0.985 0.001 106.423)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--destructive-foreground": "oklch(0.985 0.001 106.423)",
      "--border": "oklch(1 0.001 106.423 / 10%)",
      "--input": "oklch(1 0.001 106.423 / 15%)",
      "--ring": "oklch(0.553 0.013 58.071)",
      "--chart-1": "oklch(0.488 0.243 264.376)",
      "--chart-2": "oklch(0.696 0.17 162.48)",
      "--chart-3": "oklch(0.769 0.188 70.08)",
      "--chart-4": "oklch(0.627 0.265 303.9)",
      "--chart-5": "oklch(0.645 0.246 16.439)",
      "--sidebar": "oklch(0.216 0.006 56.043)",
      "--sidebar-foreground": "oklch(0.985 0.001 106.423)",
      "--sidebar-primary": "oklch(0.488 0.243 264.376)",
      "--sidebar-primary-foreground": "oklch(0.985 0.001 106.423)",
      "--sidebar-accent": "oklch(0.268 0.007 34.298)",
      "--sidebar-accent-foreground": "oklch(0.985 0.001 106.423)",
      "--sidebar-border": "oklch(1 0.001 106.423 / 10%)",
      "--sidebar-ring": "oklch(0.439 0.004 54.686)",
    }
  },
  {
    name: "Sunset Horizon",
    value: "sunset-horizon",
    description: "Warm sunset gradients with horizon blues",
    fontFamily: "Source Sans Pro, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Source+Sans+Pro:wght@400;500;600;700&display=swap",
    borderRadius: "0.625rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.98 0.02 30)",
      "--foreground": "oklch(0.1 0.05 30)",
      "--card": "oklch(0.98 0.02 30)",
      "--card-foreground": "oklch(0.1 0.05 30)",
      "--popover": "oklch(0.98 0.02 30)",
      "--popover-foreground": "oklch(0.1 0.05 30)",
      "--primary": "oklch(0.55 0.2 30)",
      "--primary-foreground": "oklch(0.98 0.02 30)",
      "--secondary": "oklch(0.93 0.03 30)",
      "--secondary-foreground": "oklch(0.55 0.2 30)",
      "--muted": "oklch(0.93 0.03 30)",
      "--muted-foreground": "oklch(0.55 0.15 30)",
      "--accent": "oklch(0.87 0.08 200)",
      "--accent-foreground": "oklch(0.55 0.2 30)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.85 0.04 30)",
      "--input": "oklch(0.85 0.04 30)",
      "--ring": "oklch(0.55 0.2 30)",
      "--chart-1": "oklch(0.6 0.25 30)",
      "--chart-2": "oklch(0.5 0.3 10)",
      "--chart-3": "oklch(0.4 0.25 50)",
      "--chart-4": "oklch(0.7 0.2 200)",
      "--chart-5": "oklch(0.8 0.15 220)",
      "--sidebar": "oklch(0.96 0.03 30)",
      "--sidebar-foreground": "oklch(0.1 0.05 30)",
      "--sidebar-primary": "oklch(0.55 0.2 30)",
      "--sidebar-primary-foreground": "oklch(0.98 0.02 30)",
      "--sidebar-accent": "oklch(0.93 0.03 30)",
      "--sidebar-accent-foreground": "oklch(0.55 0.2 30)",
      "--sidebar-border": "oklch(0.85 0.04 30)",
      "--sidebar-ring": "oklch(0.55 0.2 30)"
    },
    darkColors: {
      "--background": "oklch(0.05 0.03 200)",
      "--foreground": "oklch(0.92 0.05 30)",
      "--card": "oklch(0.08 0.05 200)",
      "--card-foreground": "oklch(0.92 0.05 30)",
      "--popover": "oklch(0.05 0.03 200)",
      "--popover-foreground": "oklch(0.92 0.05 30)",
      "--primary": "oklch(0.75 0.25 30)",
      "--primary-foreground": "oklch(0.05 0.03 200)",
      "--secondary": "oklch(0.15 0.05 200)",
      "--secondary-foreground": "oklch(0.92 0.05 30)",
      "--muted": "oklch(0.15 0.05 200)",
      "--muted-foreground": "oklch(0.65 0.15 30)",
      "--accent": "oklch(0.18 0.1 200)",
      "--accent-foreground": "oklch(0.92 0.05 30)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.25 0.05 200)",
      "--input": "oklch(0.25 0.05 200)",
      "--ring": "oklch(0.75 0.25 30)",
      "--chart-1": "oklch(0.7 0.3 30)",
      "--chart-2": "oklch(0.6 0.35 10)",
      "--chart-3": "oklch(0.5 0.3 50)",
      "--chart-4": "oklch(0.8 0.25 200)",
      "--chart-5": "oklch(0.9 0.2 220)",
      "--sidebar": "oklch(0.08 0.05 200)",
      "--sidebar-foreground": "oklch(0.92 0.05 30)",
      "--sidebar-primary": "oklch(0.75 0.25 30)",
      "--sidebar-primary-foreground": "oklch(0.05 0.03 200)",
      "--sidebar-accent": "oklch(0.15 0.05 200)",
      "--sidebar-accent-foreground": "oklch(0.92 0.05 30)",
      "--sidebar-border": "oklch(0.25 0.05 200)",
      "--sidebar-ring": "oklch(0.75 0.25 30)"
    }
  },
  {
    name: "Supabase",
    value: "supabase",
    description: "Fresh green theme inspired by Supabase with Inter font",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap",
    borderRadius: "0.5rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9911 0 0)",
      "--foreground": "oklch(0.2046 0 0)",
      "--card": "oklch(0.9911 0 0)",
      "--card-foreground": "oklch(0.2046 0 0)",
      "--popover": "oklch(0.9911 0 0)",
      "--popover-foreground": "oklch(0.4386 0 0)",
      "--primary": "oklch(0.8348 0.1302 160.9080)",
      "--primary-foreground": "oklch(0.2626 0.0147 166.4589)",
      "--secondary": "oklch(0.9940 0 0)",
      "--secondary-foreground": "oklch(0.2046 0 0)",
      "--muted": "oklch(0.9461 0 0)",
      "--muted-foreground": "oklch(0.2435 0 0)",
      "--accent": "oklch(0.9461 0 0)",
      "--accent-foreground": "oklch(0.2435 0 0)",
      "--destructive": "oklch(0.5523 0.1927 32.7272)",
      "--destructive-foreground": "oklch(0.9934 0.0032 17.2118)",
      "--border": "oklch(0.9037 0 0)",
      "--input": "oklch(0.9731 0 0)",
      "--ring": "oklch(0.8348 0.1302 160.9080)",
      "--chart-1": "oklch(0.8348 0.1302 160.9080)",
      "--chart-2": "oklch(0.6231 0.1880 259.8145)",
      "--chart-3": "oklch(0.6056 0.2189 292.7172)",
      "--chart-4": "oklch(0.7686 0.1647 70.0804)",
      "--chart-5": "oklch(0.6959 0.1491 162.4796)",
      "--sidebar": "oklch(0.9911 0 0)",
      "--sidebar-foreground": "oklch(0.5452 0 0)",
      "--sidebar-primary": "oklch(0.8348 0.1302 160.9080)",
      "--sidebar-primary-foreground": "oklch(0.2626 0.0147 166.4589)",
      "--sidebar-accent": "oklch(0.9461 0 0)",
      "--sidebar-accent-foreground": "oklch(0.2435 0 0)",
      "--sidebar-border": "oklch(0.9037 0 0)",
      "--sidebar-ring": "oklch(0.8348 0.1302 160.9080)"
    },
    darkColors: {
      "--background": "oklch(0.1822 0 0)",
      "--foreground": "oklch(0.9288 0.0126 255.5078)",
      "--card": "oklch(0.2046 0 0)",
      "--card-foreground": "oklch(0.9288 0.0126 255.5078)",
      "--popover": "oklch(0.2603 0 0)",
      "--popover-foreground": "oklch(0.7348 0 0)",
      "--primary": "oklch(0.4365 0.1044 156.7556)",
      "--primary-foreground": "oklch(0.9213 0.0135 167.1556)",
      "--secondary": "oklch(0.2603 0 0)",
      "--secondary-foreground": "oklch(0.9851 0 0)",
      "--muted": "oklch(0.2393 0 0)",
      "--muted-foreground": "oklch(0.7122 0 0)",
      "--accent": "oklch(0.3132 0 0)",
      "--accent-foreground": "oklch(0.9851 0 0)",
      "--destructive": "oklch(0.3123 0.0852 29.7877)",
      "--destructive-foreground": "oklch(0.9368 0.0045 34.3092)",
      "--border": "oklch(0.2809 0 0)",
      "--input": "oklch(0.2603 0 0)",
      "--ring": "oklch(0.8003 0.1821 151.7110)",
      "--chart-1": "oklch(0.8003 0.1821 151.7110)",
      "--chart-2": "oklch(0.7137 0.1434 254.6240)",
      "--chart-3": "oklch(0.7090 0.1592 293.5412)",
      "--chart-4": "oklch(0.8369 0.1644 84.4286)",
      "--chart-5": "oklch(0.7845 0.1325 181.9120)",
      "--sidebar": "oklch(0.1822 0 0)",
      "--sidebar-foreground": "oklch(0.6301 0 0)",
      "--sidebar-primary": "oklch(0.4365 0.1044 156.7556)",
      "--sidebar-primary-foreground": "oklch(0.9213 0.0135 167.1556)",
      "--sidebar-accent": "oklch(0.3132 0 0)",
      "--sidebar-accent-foreground": "oklch(0.9851 0 0)",
      "--sidebar-border": "oklch(0.2809 0 0)",
      "--sidebar-ring": "oklch(0.8003 0.1821 151.7110)"
    }
  },
  {
    name: "Tangerine",
    value: "tangerine",
    description: "Warm orange theme with cool blue-purple accents",
    fontFamily: "ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif",
    fontWeight: "400",
    borderRadius: "0.625rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9383 0.0042 236.4993)",
      "--foreground": "oklch(0.3211 0 0)",
      "--card": "oklch(1.0000 0 0)",
      "--card-foreground": "oklch(0.3211 0 0)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.3211 0 0)",
      "--primary": "oklch(0.6397 0.1720 36.4421)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.9670 0.0029 264.5419)",
      "--secondary-foreground": "oklch(0.4461 0.0263 256.8018)",
      "--muted": "oklch(0.9846 0.0017 247.8389)",
      "--muted-foreground": "oklch(0.5510 0.0234 264.3637)",
      "--accent": "oklch(0.9119 0.0222 243.8174)",
      "--accent-foreground": "oklch(0.3791 0.1378 265.5222)",
      "--destructive": "oklch(0.6368 0.2078 25.3313)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.9022 0.0052 247.8822)",
      "--input": "oklch(0.9700 0.0029 264.5420)",
      "--ring": "oklch(0.6397 0.1720 36.4421)",
      "--chart-1": "oklch(0.7156 0.0605 248.6845)",
      "--chart-2": "oklch(0.7875 0.0917 35.9616)",
      "--chart-3": "oklch(0.5778 0.0759 254.1573)",
      "--chart-4": "oklch(0.5016 0.0849 259.4902)",
      "--chart-5": "oklch(0.4241 0.0952 264.0306)",
      "--sidebar": "oklch(0.9030 0.0046 258.3257)",
      "--sidebar-foreground": "oklch(0.3211 0 0)",
      "--sidebar-primary": "oklch(0.6397 0.1720 36.4421)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.9119 0.0222 243.8174)",
      "--sidebar-accent-foreground": "oklch(0.3791 0.1378 265.5222)",
      "--sidebar-border": "oklch(0.9276 0.0058 264.5313)",
      "--sidebar-ring": "oklch(0.6397 0.1720 36.4421)"
    },
    darkColors: {
      "--background": "oklch(0.2598 0.0306 262.6666)",
      "--foreground": "oklch(0.9219 0 0)",
      "--card": "oklch(0.3106 0.0301 268.6365)",
      "--card-foreground": "oklch(0.9219 0 0)",
      "--popover": "oklch(0.2900 0.0249 268.3986)",
      "--popover-foreground": "oklch(0.9219 0 0)",
      "--primary": "oklch(0.6397 0.1720 36.4421)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.3095 0.0266 266.7132)",
      "--secondary-foreground": "oklch(0.9219 0 0)",
      "--muted": "oklch(0.3095 0.0266 266.7132)",
      "--muted-foreground": "oklch(0.7155 0 0)",
      "--accent": "oklch(0.3380 0.0589 267.5867)",
      "--accent-foreground": "oklch(0.8823 0.0571 254.1284)",
      "--destructive": "oklch(0.6368 0.2078 25.3313)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.3843 0.0301 269.7337)",
      "--input": "oklch(0.3843 0.0301 269.7337)",
      "--ring": "oklch(0.6397 0.1720 36.4421)",
      "--chart-1": "oklch(0.7156 0.0605 248.6845)",
      "--chart-2": "oklch(0.7693 0.0876 34.1875)",
      "--chart-3": "oklch(0.5778 0.0759 254.1573)",
      "--chart-4": "oklch(0.5016 0.0849 259.4902)",
      "--chart-5": "oklch(0.4241 0.0952 264.0306)",
      "--sidebar": "oklch(0.3100 0.0283 267.7408)",
      "--sidebar-foreground": "oklch(0.9219 0 0)",
      "--sidebar-primary": "oklch(0.6397 0.1720 36.4421)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.3380 0.0589 267.5867)",
      "--sidebar-accent-foreground": "oklch(0.8823 0.0571 254.1284)",
      "--sidebar-border": "oklch(0.3843 0.0301 269.7337)",
      "--sidebar-ring": "oklch(0.6397 0.1720 36.4421)"
    }
  },
  {
    name: "Twitter",
    value: "twitter",
    description: "Social media theme inspired by Twitter with Roboto font",
    fontFamily: "Roboto, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap",
    borderRadius: "0.75rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1.0000 0 0)",
      "--foreground": "oklch(0.1884 0.0128 248.5103)",
      "--card": "oklch(0.9784 0.0011 197.1387)",
      "--card-foreground": "oklch(0.1884 0.0128 248.5103)",
      "--popover": "oklch(1.0000 0 0)",
      "--popover-foreground": "oklch(0.1884 0.0128 248.5103)",
      "--primary": "oklch(0.6723 0.1606 244.9955)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.1884 0.0128 248.5103)",
      "--secondary-foreground": "oklch(1.0000 0 0)",
      "--muted": "oklch(0.9222 0.0013 286.3737)",
      "--muted-foreground": "oklch(0.1884 0.0128 248.5103)",
      "--accent": "oklch(0.9392 0.0166 250.8453)",
      "--accent-foreground": "oklch(0.6723 0.1606 244.9955)",
      "--destructive": "oklch(0.6188 0.2376 25.7658)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.9317 0.0118 231.6594)",
      "--input": "oklch(0.9809 0.0025 228.7836)",
      "--ring": "oklch(0.6818 0.1584 243.3540)",
      "--chart-1": "oklch(0.6723 0.1606 244.9955)",
      "--chart-2": "oklch(0.6907 0.1554 160.3454)",
      "--chart-3": "oklch(0.8214 0.1600 82.5337)",
      "--chart-4": "oklch(0.7064 0.1822 151.7125)",
      "--chart-5": "oklch(0.5919 0.2186 10.5826)",
      "--sidebar": "oklch(0.9784 0.0011 197.1387)",
      "--sidebar-foreground": "oklch(0.1884 0.0128 248.5103)",
      "--sidebar-primary": "oklch(0.6723 0.1606 244.9955)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.9392 0.0166 250.8453)",
      "--sidebar-accent-foreground": "oklch(0.6723 0.1606 244.9955)",
      "--sidebar-border": "oklch(0.9271 0.0101 238.5177)",
      "--sidebar-ring": "oklch(0.6818 0.1584 243.3540)"
    },
    darkColors: {
      "--background": "oklch(0 0 0)",
      "--foreground": "oklch(0.9328 0.0025 228.7857)",
      "--card": "oklch(0.2097 0.0080 274.5332)",
      "--card-foreground": "oklch(0.8853 0 0)",
      "--popover": "oklch(0 0 0)",
      "--popover-foreground": "oklch(0.9328 0.0025 228.7857)",
      "--primary": "oklch(0.6692 0.1607 245.0110)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.9622 0.0035 219.5331)",
      "--secondary-foreground": "oklch(0.1884 0.0128 248.5103)",
      "--muted": "oklch(0.2090 0 0)",
      "--muted-foreground": "oklch(0.5637 0.0078 247.9662)",
      "--accent": "oklch(0.1928 0.0331 242.5459)",
      "--accent-foreground": "oklch(0.6692 0.1607 245.0110)",
      "--destructive": "oklch(0.6188 0.2376 25.7658)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.2674 0.0047 248.0045)",
      "--input": "oklch(0.3020 0.0288 244.8244)",
      "--ring": "oklch(0.6818 0.1584 243.3540)",
      "--chart-1": "oklch(0.6723 0.1606 244.9955)",
      "--chart-2": "oklch(0.6907 0.1554 160.3454)",
      "--chart-3": "oklch(0.8214 0.1600 82.5337)",
      "--chart-4": "oklch(0.7064 0.1822 151.7125)",
      "--chart-5": "oklch(0.5919 0.2186 10.5826)",
      "--sidebar": "oklch(0.2097 0.0080 274.5332)",
      "--sidebar-foreground": "oklch(0.8853 0 0)",
      "--sidebar-primary": "oklch(0.6818 0.1584 243.3540)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.1928 0.0331 242.5459)",
      "--sidebar-accent-foreground": "oklch(0.6692 0.1607 245.0110)",
      "--sidebar-border": "oklch(0.2674 0.0047 248.0045)",
      "--sidebar-ring": "oklch(0.6818 0.1584 243.3540)"
    }
  },
  {
    name: "Typewriter",
    value: "typewriter",
    description: "Typewriter theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.985 0 22.65)",
      "--foreground": "oklch(0.2686 0 23.43)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2686 0 23.43)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2686 0 23.43)",
      "--primary": "oklch(0.1458 0 22.58)",
      "--secondary": "oklch(0.9224 0 23.19)",
      "--muted": "oklch(0.97 0 23.07)",
      "--muted-foreground": "oklch(0.4411 0 24.82)",
      "--accent": "oklch(0.97 0 23.07)",
      "--accent-foreground": "oklch(0.2686 0 23.43)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9224 0 23.19)",
      "--input": "oklch(0.8696 0 23.46)",
      "--ring": "oklch(0.2686 0 23.43)",
      "--chart-1": "oklch(0.1458 0 22.58)",
      "--chart-2": "oklch(0.4411 0 24.82)",
      "--chart-3": "oklch(0.2686 0 23.43)",
      "--chart-4": "oklch(0.7084 0 23.42)",
      "--chart-5": "oklch(0.8696 0 23.46)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.2686 0 23.43)",
      "--sidebar-primary": "oklch(0.1458 0 22.58)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.985 0 22.65)",
      "--sidebar-accent-foreground": "oklch(0.2686 0 23.43)",
      "--sidebar-border": "oklch(0.9224 0 23.19)",
      "--sidebar-ring": "oklch(0.2686 0 23.43)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2686 0 23.43)"
    },
    darkColors: {
      "--background": "oklch(0.1458 0 22.58)",
      "--foreground": "oklch(0.9224 0 23.19)",
      "--card": "oklch(0.2096 0 23.33)",
      "--card-foreground": "oklch(0.9224 0 23.19)",
      "--popover": "oklch(0.2096 0 23.33)",
      "--popover-foreground": "oklch(0.9224 0 23.19)",
      "--primary": "oklch(0.985 0 22.65)",
      "--primary-foreground": "oklch(0.2096 0 23.33)",
      "--secondary": "oklch(0.3722 0 24.37)",
      "--secondary-foreground": "oklch(0.985 0 22.65)",
      "--muted": "oklch(0.2686 0 23.43)",
      "--muted-foreground": "oklch(0.8696 0 23.46)",
      "--accent": "oklch(0.2686 0 23.43)",
      "--accent-foreground": "oklch(0.9224 0 23.19)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2686 0 23.43)",
      "--input": "oklch(0.3722 0 24.37)",
      "--ring": "oklch(0.9224 0 23.19)",
      "--chart-1": "oklch(0.985 0 22.65)",
      "--chart-2": "oklch(0.7084 0 23.42)",
      "--chart-3": "oklch(0.9224 0 23.19)",
      "--chart-4": "oklch(0.4411 0 24.82)",
      "--chart-5": "oklch(0.3722 0 24.37)",
      "--sidebar": "oklch(0.2096 0 23.33)",
      "--sidebar-foreground": "oklch(0.9224 0 23.19)",
      "--sidebar-primary": "oklch(0.985 0 22.65)",
      "--sidebar-primary-foreground": "oklch(0.2096 0 23.33)",
      "--sidebar-accent": "oklch(0.2686 0 23.43)",
      "--sidebar-accent-foreground": "oklch(0.9224 0 23.19)",
      "--sidebar-border": "oklch(0.2686 0 23.43)",
      "--sidebar-ring": "oklch(0.9224 0 23.19)"
    }
  },
  {
    name: "Underground",
    value: "underground",
    description: "Underground theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.3rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.2779 0.0162 226.51)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2779 0.0162 226.51)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2779 0.0162 226.51)",
      "--primary": "oklch(0.4478 0.1828 355.91)",
      "--secondary": "oklch(0.4469 0.1569 297.97)",
      "--muted": "oklch(0.9867 0.0017 219.35)",
      "--muted-foreground": "oklch(0.4463 0.0202 225.58)",
      "--accent": "oklch(0.9867 0.0017 219.35)",
      "--accent-foreground": "oklch(0.2779 0.0162 226.51)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9292 0.0071 220.94)",
      "--input": "oklch(0.8695 0.0105 222.21)",
      "--ring": "oklch(0.4478 0.1828 355.91)",
      "--chart-1": "oklch(0.4478 0.1828 355.91)",
      "--chart-2": "oklch(0.4469 0.1569 297.97)",
      "--chart-3": "oklch(0.8242 0.1178 344.61)",
      "--chart-4": "oklch(0.8178 0.0799 299.66)",
      "--chart-5": "oklch(0.9492 0.0277 340.92)",
      "--sidebar": "oklch(0.9701 0.0035 220.38)",
      "--sidebar-foreground": "oklch(0.2779 0.0162 226.51)",
      "--sidebar-primary": "oklch(0.4478 0.1828 355.91)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9292 0.0071 220.94)",
      "--sidebar-accent-foreground": "oklch(0.2779 0.0162 226.51)",
      "--sidebar-border": "oklch(0.9292 0.0071 220.94)",
      "--sidebar-ring": "oklch(0.4478 0.1828 355.91)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2779 0.0162 226.51)"
    },
    darkColors: {
      "--background": "oklch(0.1324 0.0125 228.42)",
      "--foreground": "oklch(0.9292 0.0071 220.94)",
      "--card": "oklch(0.1324 0.0125 228.42)",
      "--card-foreground": "oklch(0.9292 0.0071 220.94)",
      "--popover": "oklch(0.1324 0.0125 228.42)",
      "--popover-foreground": "oklch(0.9292 0.0071 220.94)",
      "--primary": "oklch(0.4478 0.1828 355.91)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.4469 0.1569 297.97)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.21 0.0139 227.92)",
      "--muted-foreground": "oklch(0.6386 0.0159 224.62)",
      "--accent": "oklch(0.21 0.0139 227.92)",
      "--accent-foreground": "oklch(0.9292 0.0071 220.94)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2779 0.0162 226.51)",
      "--input": "oklch(0.373 0.0183 225.41)",
      "--ring": "oklch(0.4478 0.1828 355.91)",
      "--chart-1": "oklch(0.4478 0.1828 355.91)",
      "--chart-2": "oklch(0.4469 0.1569 297.97)",
      "--chart-3": "oklch(0.8242 0.1178 344.61)",
      "--chart-4": "oklch(0.8178 0.0799 299.66)",
      "--chart-5": "oklch(0.9492 0.0277 340.92)",
      "--sidebar": "oklch(0.21 0.0139 227.92)",
      "--sidebar-foreground": "oklch(0.9292 0.0071 220.94)",
      "--sidebar-primary": "oklch(0.4478 0.1828 355.91)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.2779 0.0162 226.51)",
      "--sidebar-accent-foreground": "oklch(0.9292 0.0071 220.94)",
      "--sidebar-border": "oklch(0.2779 0.0162 226.51)",
      "--sidebar-ring": "oklch(0.4478 0.1828 355.91)"
    }
  },
  {
    name: "Vercel",
    value: "vercel",
    description: "Minimal black and white theme inspired by Vercel with Inter font",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap",
    borderRadius: "0.375rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9900 0 0)",
      "--foreground": "oklch(0 0 0)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0 0 0)",
      "--popover": "oklch(0.9900 0 0)",
      "--popover-foreground": "oklch(0 0 0)",
      "--primary": "oklch(0 0 0)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary": "oklch(0.9400 0 0)",
      "--secondary-foreground": "oklch(0 0 0)",
      "--muted": "oklch(0.9700 0 0)",
      "--muted-foreground": "oklch(0.4400 0 0)",
      "--accent": "oklch(0.9400 0 0)",
      "--accent-foreground": "oklch(0 0 0)",
      "--destructive": "oklch(0.6300 0.1900 23.0300)",
      "--destructive-foreground": "oklch(1 0 0)",
      "--border": "oklch(0.9200 0 0)",
      "--input": "oklch(0.9400 0 0)",
      "--ring": "oklch(0 0 0)",
      "--chart-1": "oklch(0.8100 0.1700 75.3500)",
      "--chart-2": "oklch(0.5500 0.2200 264.5300)",
      "--chart-3": "oklch(0.7200 0 0)",
      "--chart-4": "oklch(0.9200 0 0)",
      "--chart-5": "oklch(0.5600 0 0)",
      "--sidebar": "oklch(0.9900 0 0)",
      "--sidebar-foreground": "oklch(0 0 0)",
      "--sidebar-primary": "oklch(0 0 0)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9400 0 0)",
      "--sidebar-accent-foreground": "oklch(0 0 0)",
      "--sidebar-border": "oklch(0.9400 0 0)",
      "--sidebar-ring": "oklch(0 0 0)"
    },
    darkColors: {
      "--background": "oklch(0 0 0)",
      "--foreground": "oklch(1 0 0)",
      "--card": "oklch(0.1400 0 0)",
      "--card-foreground": "oklch(1 0 0)",
      "--popover": "oklch(0.1800 0 0)",
      "--popover-foreground": "oklch(1 0 0)",
      "--primary": "oklch(1 0 0)",
      "--primary-foreground": "oklch(0 0 0)",
      "--secondary": "oklch(0.2500 0 0)",
      "--secondary-foreground": "oklch(1 0 0)",
      "--muted": "oklch(0.2300 0 0)",
      "--muted-foreground": "oklch(0.7200 0 0)",
      "--accent": "oklch(0.3200 0 0)",
      "--accent-foreground": "oklch(1 0 0)",
      "--destructive": "oklch(0.6900 0.2000 23.9100)",
      "--destructive-foreground": "oklch(0 0 0)",
      "--border": "oklch(0.2600 0 0)",
      "--input": "oklch(0.3200 0 0)",
      "--ring": "oklch(0.7200 0 0)",
      "--chart-1": "oklch(0.8100 0.1700 75.3500)",
      "--chart-2": "oklch(0.5800 0.2100 260.8400)",
      "--chart-3": "oklch(0.5600 0 0)",
      "--chart-4": "oklch(0.4400 0 0)",
      "--chart-5": "oklch(0.9200 0 0)",
      "--sidebar": "oklch(0.1800 0 0)",
      "--sidebar-foreground": "oklch(1 0 0)",
      "--sidebar-primary": "oklch(1 0 0)",
      "--sidebar-primary-foreground": "oklch(0 0 0)",
      "--sidebar-accent": "oklch(0.3200 0 0)",
      "--sidebar-accent-foreground": "oklch(1 0 0)",
      "--sidebar-border": "oklch(0.2600 0 0)",
      "--sidebar-ring": "oklch(0.7200 0 0)"
    }
  },
  {
    name: "Vintage Paper",
    value: "vintage-paper",
    description: "Warm sepia-toned theme with old paper aesthetics",
    fontFamily: "ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif",
    fontWeight: "400",
    borderRadius: "0.625rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9582 0.0152 90.2357)",
      "--foreground": "oklch(0.3760 0.0225 64.3434)",
      "--card": "oklch(0.9914 0.0098 87.4695)",
      "--card-foreground": "oklch(0.3760 0.0225 64.3434)",
      "--popover": "oklch(0.9914 0.0098 87.4695)",
      "--popover-foreground": "oklch(0.3760 0.0225 64.3434)",
      "--primary": "oklch(0.6180 0.0778 65.5444)",
      "--primary-foreground": "oklch(1.0000 0 0)",
      "--secondary": "oklch(0.8846 0.0302 85.5655)",
      "--secondary-foreground": "oklch(0.4313 0.0300 64.9288)",
      "--muted": "oklch(0.9239 0.0190 83.0636)",
      "--muted-foreground": "oklch(0.5391 0.0387 71.1655)",
      "--accent": "oklch(0.8348 0.0426 88.8064)",
      "--accent-foreground": "oklch(0.3760 0.0225 64.3434)",
      "--destructive": "oklch(0.5471 0.1438 32.9149)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.8606 0.0321 84.5881)",
      "--input": "oklch(0.8606 0.0321 84.5881)",
      "--ring": "oklch(0.6180 0.0778 65.5444)",
      "--chart-1": "oklch(0.6180 0.0778 65.5444)",
      "--chart-2": "oklch(0.5604 0.0624 68.5805)",
      "--chart-3": "oklch(0.4851 0.0570 72.6827)",
      "--chart-4": "oklch(0.6777 0.0624 64.7755)",
      "--chart-5": "oklch(0.7264 0.0581 66.6967)",
      "--sidebar": "oklch(0.9239 0.0190 83.0636)",
      "--sidebar-foreground": "oklch(0.3760 0.0225 64.3434)",
      "--sidebar-primary": "oklch(0.6180 0.0778 65.5444)",
      "--sidebar-primary-foreground": "oklch(1.0000 0 0)",
      "--sidebar-accent": "oklch(0.8348 0.0426 88.8064)",
      "--sidebar-accent-foreground": "oklch(0.3760 0.0225 64.3434)",
      "--sidebar-border": "oklch(0.8606 0.0321 84.5881)",
      "--sidebar-ring": "oklch(0.6180 0.0778 65.5444)"
    },
    darkColors: {
      "--background": "oklch(0.2747 0.0139 57.6523)",
      "--foreground": "oklch(0.9239 0.0190 83.0636)",
      "--card": "oklch(0.3237 0.0155 59.0603)",
      "--card-foreground": "oklch(0.9239 0.0190 83.0636)",
      "--popover": "oklch(0.3237 0.0155 59.0603)",
      "--popover-foreground": "oklch(0.9239 0.0190 83.0636)",
      "--primary": "oklch(0.7264 0.0581 66.6967)",
      "--primary-foreground": "oklch(0.2747 0.0139 57.6523)",
      "--secondary": "oklch(0.3795 0.0181 57.1280)",
      "--secondary-foreground": "oklch(0.9239 0.0190 83.0636)",
      "--muted": "oklch(0.2939 0.0125 62.1298)",
      "--muted-foreground": "oklch(0.7982 0.0243 82.1078)",
      "--accent": "oklch(0.4186 0.0281 56.3404)",
      "--accent-foreground": "oklch(0.9239 0.0190 83.0636)",
      "--destructive": "oklch(0.5471 0.1438 32.9149)",
      "--destructive-foreground": "oklch(1.0000 0 0)",
      "--border": "oklch(0.3795 0.0181 57.1280)",
      "--input": "oklch(0.3795 0.0181 57.1280)",
      "--ring": "oklch(0.7264 0.0581 66.6967)",
      "--chart-1": "oklch(0.7264 0.0581 66.6967)",
      "--chart-2": "oklch(0.6777 0.0624 64.7755)",
      "--chart-3": "oklch(0.6180 0.0778 65.5444)",
      "--chart-4": "oklch(0.5604 0.0624 68.5805)",
      "--chart-5": "oklch(0.4851 0.0570 72.6827)",
      "--sidebar": "oklch(0.2747 0.0139 57.6523)",
      "--sidebar-foreground": "oklch(0.9239 0.0190 83.0636)",
      "--sidebar-primary": "oklch(0.7264 0.0581 66.6967)",
      "--sidebar-primary-foreground": "oklch(0.2747 0.0139 57.6523)",
      "--sidebar-accent": "oklch(0.4186 0.0281 56.3404)",
      "--sidebar-accent-foreground": "oklch(0.9239 0.0190 83.0636)",
      "--sidebar-border": "oklch(0.3795 0.0181 57.1280)",
      "--sidebar-ring": "oklch(0.7264 0.0581 66.6967)"
    }
  },
  {
    name: "Violet",
    value: "violet",
    description: "Purple violet theme with Playfair Display",
    fontFamily: "Playfair Display, ui-serif, Georgia, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;600;700&display=swap",
    borderRadius: "0.65rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.141 0.005 285.823)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.141 0.005 285.823)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.141 0.005 285.823)",
      "--primary": "oklch(0.606 0.25 292.717)",
      "--primary-foreground": "oklch(0.969 0.016 293.756)",
      "--secondary": "oklch(0.967 0.001 286.375)",
      "--secondary-foreground": "oklch(0.21 0.006 285.885)",
      "--muted": "oklch(0.967 0.001 286.375)",
      "--muted-foreground": "oklch(0.552 0.016 285.938)",
      "--accent": "oklch(0.967 0.001 286.375)",
      "--accent-foreground": "oklch(0.21 0.006 285.885)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.92 0.004 286.32)",
      "--input": "oklch(0.92 0.004 286.32)",
      "--ring": "oklch(0.606 0.25 292.717)",
      "--chart-1": "oklch(0.646 0.222 41.116)",
      "--chart-2": "oklch(0.6 0.118 184.704)",
      "--chart-3": "oklch(0.398 0.07 227.392)",
      "--chart-4": "oklch(0.828 0.189 84.429)",
      "--chart-5": "oklch(0.769 0.188 70.08)",
      "--sidebar": "oklch(0.985 0 0)",
      "--sidebar-foreground": "oklch(0.141 0.005 285.823)",
      "--sidebar-primary": "oklch(0.606 0.25 292.717)",
      "--sidebar-primary-foreground": "oklch(0.969 0.016 293.756)",
      "--sidebar-accent": "oklch(0.967 0.001 286.375)",
      "--sidebar-accent-foreground": "oklch(0.21 0.006 285.885)",
      "--sidebar-border": "oklch(0.92 0.004 286.32)",
      "--sidebar-ring": "oklch(0.606 0.25 292.717)",
    },
    darkColors: {
      "--background": "oklch(0.141 0.005 285.823)",
      "--foreground": "oklch(0.985 0 0)",
      "--card": "oklch(0.21 0.006 285.885)",
      "--card-foreground": "oklch(0.985 0 0)",
      "--popover": "oklch(0.21 0.006 285.885)",
      "--popover-foreground": "oklch(0.985 0 0)",
      "--primary": "oklch(0.541 0.281 293.009)",
      "--primary-foreground": "oklch(0.969 0.016 293.756)",
      "--secondary": "oklch(0.274 0.006 286.033)",
      "--secondary-foreground": "oklch(0.985 0 0)",
      "--muted": "oklch(0.274 0.006 286.033)",
      "--muted-foreground": "oklch(0.705 0.015 286.067)",
      "--accent": "oklch(0.274 0.006 286.033)",
      "--accent-foreground": "oklch(0.985 0 0)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(1 0 0 / 10%)",
      "--input": "oklch(1 0 0 / 15%)",
      "--ring": "oklch(0.541 0.281 293.009)",
      "--chart-1": "oklch(0.488 0.243 264.376)",
      "--chart-2": "oklch(0.696 0.17 162.48)",
      "--chart-3": "oklch(0.769 0.188 70.08)",
      "--chart-4": "oklch(0.627 0.265 303.9)",
      "--chart-5": "oklch(0.645 0.246 16.439)",
      "--sidebar": "oklch(0.21 0.006 285.885)",
      "--sidebar-foreground": "oklch(0.985 0 0)",
      "--sidebar-primary": "oklch(0.541 0.281 293.009)",
      "--sidebar-primary-foreground": "oklch(0.969 0.016 293.756)",
      "--sidebar-accent": "oklch(0.274 0.006 286.033)",
      "--sidebar-accent-foreground": "oklch(0.985 0 0)",
      "--sidebar-border": "oklch(1 0 0 / 10%)",
      "--sidebar-ring": "oklch(0.541 0.281 293.009)",
    }
  },
  {
    name: "Violet Bloom",
    value: "violet-bloom",
    description: "Rich violets with soft blooming pink accents",
    fontFamily: "Libre Baskerville, ui-serif, Georgia, Cambria, 'Times New Roman', Times, serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Libre+Baskerville:wght@400;500;600;700&display=swap",
    borderRadius: "0.75rem",
    style: "new-york",
    lightColors: {
      "--background": "oklch(0.98 0.02 320)",
      "--foreground": "oklch(0.1 0.05 320)",
      "--card": "oklch(0.98 0.02 320)",
      "--card-foreground": "oklch(0.1 0.05 320)",
      "--popover": "oklch(0.98 0.02 320)",
      "--popover-foreground": "oklch(0.1 0.05 320)",
      "--primary": "oklch(0.5 0.18 320)",
      "--primary-foreground": "oklch(0.98 0.02 320)",
      "--secondary": "oklch(0.93 0.03 320)",
      "--secondary-foreground": "oklch(0.5 0.18 320)",
      "--muted": "oklch(0.93 0.03 320)",
      "--muted-foreground": "oklch(0.55 0.12 320)",
      "--accent": "oklch(0.87 0.08 340)",
      "--accent-foreground": "oklch(0.5 0.18 320)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.86 0.04 320)",
      "--input": "oklch(0.86 0.04 320)",
      "--ring": "oklch(0.5 0.18 320)",
      "--chart-1": "oklch(0.6 0.2 320)",
      "--chart-2": "oklch(0.5 0.25 340)",
      "--chart-3": "oklch(0.4 0.2 300)",
      "--chart-4": "oklch(0.7 0.15 0)",
      "--chart-5": "oklch(0.8 0.1 20)",
      "--sidebar": "oklch(0.96 0.03 320)",
      "--sidebar-foreground": "oklch(0.1 0.05 320)",
      "--sidebar-primary": "oklch(0.5 0.18 320)",
      "--sidebar-primary-foreground": "oklch(0.98 0.02 320)",
      "--sidebar-accent": "oklch(0.93 0.03 320)",
      "--sidebar-accent-foreground": "oklch(0.5 0.18 320)",
      "--sidebar-border": "oklch(0.86 0.04 320)",
      "--sidebar-ring": "oklch(0.5 0.18 320)"
    },
    darkColors: {
      "--background": "oklch(0.05 0.03 320)",
      "--foreground": "oklch(0.92 0.05 320)",
      "--card": "oklch(0.08 0.05 320)",
      "--card-foreground": "oklch(0.92 0.05 320)",
      "--popover": "oklch(0.05 0.03 320)",
      "--popover-foreground": "oklch(0.92 0.05 320)",
      "--primary": "oklch(0.72 0.22 320)",
      "--primary-foreground": "oklch(0.05 0.03 320)",
      "--secondary": "oklch(0.15 0.05 320)",
      "--secondary-foreground": "oklch(0.92 0.05 320)",
      "--muted": "oklch(0.15 0.05 320)",
      "--muted-foreground": "oklch(0.65 0.12 320)",
      "--accent": "oklch(0.18 0.1 340)",
      "--accent-foreground": "oklch(0.92 0.05 320)",
      "--destructive": "oklch(0.62 0.25 29)",
      "--destructive-foreground": "oklch(0.98 0.013 17.38)",
      "--border": "oklch(0.25 0.05 320)",
      "--input": "oklch(0.25 0.05 320)",
      "--ring": "oklch(0.72 0.22 320)",
      "--chart-1": "oklch(0.7 0.25 320)",
      "--chart-2": "oklch(0.6 0.3 340)",
      "--chart-3": "oklch(0.5 0.25 300)",
      "--chart-4": "oklch(0.8 0.2 0)",
      "--chart-5": "oklch(0.9 0.15 20)",
      "--sidebar": "oklch(0.08 0.05 320)",
      "--sidebar-foreground": "oklch(0.92 0.05 320)",
      "--sidebar-primary": "oklch(0.72 0.22 320)",
      "--sidebar-primary-foreground": "oklch(0.05 0.03 320)",
      "--sidebar-accent": "oklch(0.15 0.05 320)",
      "--sidebar-accent-foreground": "oklch(0.92 0.05 320)",
      "--sidebar-border": "oklch(0.25 0.05 320)",
      "--sidebar-ring": "oklch(0.72 0.22 320)"
    }
  },
  {
    name: "Xanadu",
    value: "xanadu",
    description: "Xanadu theme with modern styling",
    fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    borderRadius: "0.3rem",
    style: "default",
    lightColors: {
      "--background": "oklch(0.9825 0.0032 146.74)",
      "--foreground": "oklch(0.2729 0.026 151.57)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.2729 0.026 151.57)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.2729 0.026 151.57)",
      "--primary": "oklch(0.1382 0.0185 151.74)",
      "--secondary": "oklch(0.9247 0.0127 148.2)",
      "--muted": "oklch(0.9676 0.0062 147.76)",
      "--muted-foreground": "oklch(0.4447 0.0357 152.52)",
      "--accent": "oklch(0.9676 0.0062 147.76)",
      "--accent-foreground": "oklch(0.2729 0.026 151.57)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.9247 0.0127 148.2)",
      "--input": "oklch(0.8683 0.0189 149.2)",
      "--ring": "oklch(0.2729 0.026 151.57)",
      "--chart-1": "oklch(0.1382 0.0185 151.74)",
      "--chart-2": "oklch(0.4447 0.0357 152.52)",
      "--chart-3": "oklch(0.2729 0.026 151.57)",
      "--chart-4": "oklch(0.7064 0.0283 150.5)",
      "--chart-5": "oklch(0.8683 0.0189 149.2)",
      "--sidebar": "oklch(1 0 0)",
      "--sidebar-foreground": "oklch(0.2729 0.026 151.57)",
      "--sidebar-primary": "oklch(0.1382 0.0185 151.74)",
      "--sidebar-primary-foreground": "oklch(1 0 0)",
      "--sidebar-accent": "oklch(0.9825 0.0032 146.74)",
      "--sidebar-accent-foreground": "oklch(0.2729 0.026 151.57)",
      "--sidebar-border": "oklch(0.9247 0.0127 148.2)",
      "--sidebar-ring": "oklch(0.2729 0.026 151.57)",
      "--primary-foreground": "oklch(1 0 0)",
      "--secondary-foreground": "oklch(0.2729 0.026 151.57)"
    },
    darkColors: {
      "--background": "oklch(0.1382 0.0185 151.74)",
      "--foreground": "oklch(0.9247 0.0127 148.2)",
      "--card": "oklch(0.2119 0.0213 152.25)",
      "--card-foreground": "oklch(0.9247 0.0127 148.2)",
      "--popover": "oklch(0.2119 0.0213 152.25)",
      "--popover-foreground": "oklch(0.9247 0.0127 148.2)",
      "--primary": "oklch(0.9825 0.0032 146.74)",
      "--primary-foreground": "oklch(0.2119 0.0213 152.25)",
      "--secondary": "oklch(0.3728 0.0314 151.95)",
      "--secondary-foreground": "oklch(0.9825 0.0032 146.74)",
      "--muted": "oklch(0.2729 0.026 151.57)",
      "--muted-foreground": "oklch(0.8683 0.0189 149.2)",
      "--accent": "oklch(0.2729 0.026 151.57)",
      "--accent-foreground": "oklch(0.9247 0.0127 148.2)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(0.2729 0.026 151.57)",
      "--input": "oklch(0.3728 0.0314 151.95)",
      "--ring": "oklch(0.9247 0.0127 148.2)",
      "--chart-1": "oklch(0.9825 0.0032 146.74)",
      "--chart-2": "oklch(0.7064 0.0283 150.5)",
      "--chart-3": "oklch(0.9247 0.0127 148.2)",
      "--chart-4": "oklch(0.4447 0.0357 152.52)",
      "--chart-5": "oklch(0.3728 0.0314 151.95)",
      "--sidebar": "oklch(0.2119 0.0213 152.25)",
      "--sidebar-foreground": "oklch(0.9247 0.0127 148.2)",
      "--sidebar-primary": "oklch(0.9825 0.0032 146.74)",
      "--sidebar-primary-foreground": "oklch(0.2119 0.0213 152.25)",
      "--sidebar-accent": "oklch(0.2729 0.026 151.57)",
      "--sidebar-accent-foreground": "oklch(0.9247 0.0127 148.2)",
      "--sidebar-border": "oklch(0.2729 0.026 151.57)",
      "--sidebar-ring": "oklch(0.9247 0.0127 148.2)"
    }
  },
  {
    name: "Yellow",
    value: "yellow",
    description: "Bright yellow theme with Nunito",
    fontFamily: "Nunito, ui-sans-serif, system-ui, sans-serif",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=Nunito:wght@400;500;600;700&display=swap",
    borderRadius: "0.65rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.141 0.005 285.823)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.141 0.005 285.823)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.141 0.005 285.823)",
      "--primary": "oklch(0.795 0.184 86.047)",
      "--primary-foreground": "oklch(0.421 0.095 57.708)",
      "--secondary": "oklch(0.967 0.001 286.375)",
      "--secondary-foreground": "oklch(0.21 0.006 285.885)",
      "--muted": "oklch(0.967 0.001 286.375)",
      "--muted-foreground": "oklch(0.552 0.016 285.938)",
      "--accent": "oklch(0.967 0.001 286.375)",
      "--accent-foreground": "oklch(0.21 0.006 285.885)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--border": "oklch(0.92 0.004 286.32)",
      "--input": "oklch(0.92 0.004 286.32)",
      "--ring": "oklch(0.795 0.184 86.047)",
      "--chart-1": "oklch(0.646 0.222 41.116)",
      "--chart-2": "oklch(0.6 0.118 184.704)",
      "--chart-3": "oklch(0.398 0.07 227.392)",
      "--chart-4": "oklch(0.828 0.189 84.429)",
      "--chart-5": "oklch(0.769 0.188 70.08)",
      "--sidebar": "oklch(0.985 0 0)",
      "--sidebar-foreground": "oklch(0.141 0.005 285.823)",
      "--sidebar-primary": "oklch(0.795 0.184 86.047)",
      "--sidebar-primary-foreground": "oklch(0.985 0 0)",
      "--sidebar-accent": "oklch(0.967 0.001 286.375)",
      "--sidebar-accent-foreground": "oklch(0.21 0.006 285.885)",
      "--sidebar-border": "oklch(0.92 0.004 286.32)",
      "--sidebar-ring": "oklch(0.795 0.184 86.047)",
    },
    darkColors: {
      "--background": "oklch(0.141 0.005 285.823)",
      "--foreground": "oklch(0.985 0 0)",
      "--card": "oklch(0.21 0.006 285.885)",
      "--card-foreground": "oklch(0.985 0 0)",
      "--popover": "oklch(0.21 0.006 285.885)",
      "--popover-foreground": "oklch(0.985 0 0)",
      "--primary": "oklch(0.795 0.184 86.047)",
      "--primary-foreground": "oklch(0.985 0 0)",
      "--secondary": "oklch(0.274 0.006 286.033)",
      "--secondary-foreground": "oklch(0.985 0 0)",
      "--muted": "oklch(0.274 0.006 286.033)",
      "--muted-foreground": "oklch(0.705 0.015 286.067)",
      "--accent": "oklch(0.274 0.006 286.033)",
      "--accent-foreground": "oklch(0.985 0 0)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--border": "oklch(1 0 0 / 10%)",
      "--input": "oklch(1 0 0 / 15%)",
      "--ring": "oklch(0.554 0.135 66.442)",
      "--chart-1": "oklch(0.488 0.243 264.376)",
      "--chart-2": "oklch(0.696 0.17 162.48)",
      "--chart-3": "oklch(0.769 0.188 70.08)",
      "--chart-4": "oklch(0.627 0.265 303.9)",
      "--chart-5": "oklch(0.645 0.246 16.439)",
      "--sidebar": "oklch(0.21 0.006 285.885)",
      "--sidebar-foreground": "oklch(0.985 0 0)",
      "--sidebar-primary": "oklch(0.795 0.184 86.047)",
      "--sidebar-primary-foreground": "oklch(0.985 0 0)",
      "--sidebar-accent": "oklch(0.274 0.006 286.033)",
      "--sidebar-accent-foreground": "oklch(0.985 0 0)",
      "--sidebar-border": "oklch(1 0 0 / 10%)",
      "--sidebar-ring": "oklch(0.554 0.135 66.442)",
    }
  },
  {
    name: "Zinc",
    value: "zinc",
    description: "Modern zinc theme with JetBrains Mono",
    fontFamily: "JetBrains Mono, ui-monospace, monospace",
    fontWeight: "400",
    googleFontUrl: "https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;600;700&display=swap",
    borderRadius: "0.75rem",
    style: "default",
    lightColors: {
      "--background": "oklch(1 0 0)",
      "--foreground": "oklch(0.157 0.004 285.845)",
      "--card": "oklch(1 0 0)",
      "--card-foreground": "oklch(0.157 0.004 285.845)",
      "--popover": "oklch(1 0 0)",
      "--popover-foreground": "oklch(0.157 0.004 285.845)",
      "--primary": "oklch(0.228 0.005 285.785)",
      "--primary-foreground": "oklch(0.984 0.001 285.882)",
      "--secondary": "oklch(0.969 0.001 285.882)",
      "--secondary-foreground": "oklch(0.228 0.005 285.785)",
      "--muted": "oklch(0.969 0.001 285.882)",
      "--muted-foreground": "oklch(0.556 0.014 285.8)",
      "--accent": "oklch(0.969 0.001 285.882)",
      "--accent-foreground": "oklch(0.228 0.005 285.785)",
      "--destructive": "oklch(0.577 0.245 27.325)",
      "--destructive-foreground": "oklch(0.984 0.001 285.882)",
      "--border": "oklch(0.921 0.003 285.847)",
      "--input": "oklch(0.921 0.003 285.847)",
      "--ring": "oklch(0.707 0.01 285.782)",
      "--chart-1": "oklch(0.646 0.222 41.116)",
      "--chart-2": "oklch(0.6 0.118 184.704)",
      "--chart-3": "oklch(0.398 0.07 227.392)",
      "--chart-4": "oklch(0.828 0.189 84.429)",
      "--chart-5": "oklch(0.769 0.188 70.08)",
      "--sidebar": "oklch(0.984 0.001 285.882)",
      "--sidebar-foreground": "oklch(0.157 0.004 285.845)",
      "--sidebar-primary": "oklch(0.228 0.005 285.785)",
      "--sidebar-primary-foreground": "oklch(0.984 0.001 285.882)",
      "--sidebar-accent": "oklch(0.969 0.001 285.882)",
      "--sidebar-accent-foreground": "oklch(0.228 0.005 285.785)",
      "--sidebar-border": "oklch(0.921 0.003 285.847)",
      "--sidebar-ring": "oklch(0.707 0.01 285.782)",
    },
    darkColors: {
      "--background": "oklch(0.157 0.004 285.845)",
      "--foreground": "oklch(0.984 0.001 285.882)",
      "--card": "oklch(0.228 0.005 285.785)",
      "--card-foreground": "oklch(0.984 0.001 285.882)",
      "--popover": "oklch(0.228 0.005 285.785)",
      "--popover-foreground": "oklch(0.984 0.001 285.882)",
      "--primary": "oklch(0.921 0.003 285.847)",
      "--primary-foreground": "oklch(0.228 0.005 285.785)",
      "--secondary": "oklch(0.278 0.006 285.823)",
      "--secondary-foreground": "oklch(0.984 0.001 285.882)",
      "--muted": "oklch(0.278 0.006 285.823)",
      "--muted-foreground": "oklch(0.707 0.01 285.782)",
      "--accent": "oklch(0.278 0.006 285.823)",
      "--accent-foreground": "oklch(0.984 0.001 285.882)",
      "--destructive": "oklch(0.704 0.191 22.216)",
      "--destructive-foreground": "oklch(0.984 0.001 285.882)",
      "--border": "oklch(1 0.001 285.882 / 10%)",
      "--input": "oklch(1 0.001 285.882 / 15%)",
      "--ring": "oklch(0.556 0.014 285.8)",
      "--chart-1": "oklch(0.488 0.243 264.376)",
      "--chart-2": "oklch(0.696 0.17 162.48)",
      "--chart-3": "oklch(0.769 0.188 70.08)",
      "--chart-4": "oklch(0.627 0.265 303.9)",
      "--chart-5": "oklch(0.645 0.246 16.439)",
      "--sidebar": "oklch(0.228 0.005 285.785)",
      "--sidebar-foreground": "oklch(0.984 0.001 285.882)",
      "--sidebar-primary": "oklch(0.488 0.243 264.376)",
      "--sidebar-primary-foreground": "oklch(0.984 0.001 285.882)",
      "--sidebar-accent": "oklch(0.278 0.006 285.823)",
      "--sidebar-accent-foreground": "oklch(0.984 0.001 285.882)",
      "--sidebar-border": "oklch(1 0.001 285.882 / 10%)",
      "--sidebar-ring": "oklch(0.445 0.008 285.803)",
    }
  }
]

// Utility functions for the new theme system
export function getCompleteTheme(themeName: ThemeName): CompleteTheme | undefined {
  return completeThemes.find(theme => theme.value === themeName)
}

export function getDefaultTheme(): CompleteTheme {
  return getCompleteTheme(defaultThemeName) || completeThemes[0]
}

// Apply complete theme function
export function applyCompleteTheme(theme: CompleteTheme, isDark: boolean = false, fontOverride?: string | null) {
  const root = document.documentElement
  const colors = isDark ? theme.darkColors : theme.lightColors
  
  // Apply all CSS variables
  Object.entries(colors).forEach(([key, value]) => {
    root.style.setProperty(key, value)
  })
  
  // Apply border radius
  root.style.setProperty('--radius', theme.borderRadius)
  
  // Determine which font to use - look up fontFamily from config, don't use raw value
  const fontConfig = fontOverride ? availableFonts.find(f => f.value === fontOverride) : null
  const fontToUse = fontConfig?.fontFamily || theme.fontFamily
  const fontWeight = fontConfig?.fontWeight || theme.fontWeight
  const googleFontUrl = fontConfig?.googleFontUrl || theme.googleFontUrl

  // Apply font family
  root.style.setProperty('--font-family', fontToUse)
  
  // Load Google Font if needed
  if (googleFontUrl && isRemoteFontLoadingEnabled()) {
    loadGoogleFont(googleFontUrl)
  }
  
  // Apply font to body
  document.body.style.fontFamily = fontToUse
  document.body.style.fontWeight = fontWeight
}

// Google Font loader
function loadGoogleFont(url: string) {
  // Check if font is already loaded
  const existingLink = document.querySelector(`link[href="${url}"]`)
  if (existingLink) return
  
  // Create and append link element
  const link = document.createElement('link')
  link.rel = 'stylesheet'
  link.href = url
  link.crossOrigin = 'anonymous'
  document.head.appendChild(link)
}
